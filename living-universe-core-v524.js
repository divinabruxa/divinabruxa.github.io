...
class LivingUniverseCoreV524 {
  constructor() {
    ...
    this.pixelRatio = 1;
    this.targetFps = reducedMotion() ? 24 : (constrained() ? 40 : (mobile ? 45 : 60));
  ...
  bind() {
    ...
    this.observer = new MutationObserver(records => {
      if (records.some(record => record.target === document.body && ['data-route','data-screen'].includes(record.attributeName))) {
    ...
    this.observer.observe(document.body, { childList:true, subtree:true, attributes:true, attributeFilter:['data-route','data-screen','data-skin'] });
    this.onViewportShift();
  }

  setRoute(route) {
    const next = normalizeRoute(route || routeNow());
