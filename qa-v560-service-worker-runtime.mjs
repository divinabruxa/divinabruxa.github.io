...
const inheritedBody=relative=>{
  if(relative.endsWith('.html'))return '<!doctype html><html><body><main>V559 inherited fixture · Música · Vídeos</main></body></html>';
  if(relative.endsWith('.js'))return '/* inherited V559 fixture */';
  if(relative.endsWith('.css'))return '/* inherited V559 fixture */';
  if(relative.endsWith('.json')||relative.endsWith('.webmanifest'))return '{}';
  return 'inherited-v559-fixture';
};
const fetchMock=async request=>{
  ...
  if(fs.existsSync(file)&&fs.statSync(file).isFile())body=fs.readFileSync(file);
    ...
    // O pacote V560 é um delta sobre a V559. Fixtures sintéticas representam
...
check(cacheStores.has('divina-bruxa-v560-shell'),'install-cache-v560');
check((await cacheStores.get('divina-bruxa-v560-shell').keys()).length>=125,'install-shell-atomico');
check(!cacheStores.has('divina-bruxa-v560-offline-core'),'install-sem-cache-opcional');

const shellCache=cacheStores.get('divina-bruxa-v560-shell');
let response=await shellCache.match(new Request(`${scope}page-design-supreme-v560.js`));
check(response?.status===200&&(await response.text()).includes('PAGE_DESIGN_SUPREME_CONTRACT_V560'),'install-modulo-v560-real');
response=await shellCache.match(new Request(`${scope}page-design-supreme-v560.css`));
check(response?.status===200&&(await response.text()).includes('.v560-system-state'),'install-css-v560-real');

const oldCache=await caches.open('divina-bruxa-v559-shell');
...
check(!cacheStores.has('divina-bruxa-v559-shell'),'activate-remove-cache-antigo');

let result=await message({type:'VERIFY_SHELL'});
check(result?.complete===true&&result?.atomic===true&&result?.version===560,'verify-shell-completo');
...
check(!cacheStores.has('divina-bruxa-v560-premium-static'),'premium-sem-cache-na-negacao');
...
check(response?.status===200,'navegacao-v560-valida');
const contentCache=cacheStores.get('divina-bruxa-v560-content');
check(Boolean(await contentCache?.match(new Request(`${scope}index.html?fresh=1`))),'navegacao-v560-cacheada');

await shellCache.delete(new Request(`${scope}page-design-supreme-v560.js`));
result=await message({type:'VERIFY_SHELL'});
check(result?.complete===false&&result?.missing?.includes('./page-design-supreme-v560.js'),'corrupcao-v560-detectada');
result=await message({type:'REPAIR_SHELL'});
check(result?.complete===true,'corrupcao-v560-reparada');

result=await message({type:'CLEAR_OPTIONAL_OFFLINE'});
check(result?.ok===true,'cache-opcional-limpo');
check(!cacheStores.has('divina-bruxa-v560-offline-core')&&!cacheStores.has('divina-bruxa-v560-premium-static'),'stores-opcionais-removidas');
check(result?.authorityDataRemoved===false,'nao-inventa-exclusao-privada');

const quotaCache=await caches.open('divina-bruxa-v560-content');quotaCache.failNextPut=true;
...
check(inheritedFixtures.size>0,'fixture-herdada-v559-utilizada');
const total=passed+failures.length;
console.log(`V560 Service Worker runtime: ${passed}/${total} verificações aprovadas (${inheritedFixtures.size} arquivos herdados simulados).`);
