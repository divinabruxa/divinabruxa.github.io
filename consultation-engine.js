/* DIVINA BRUXA — SANTUÁRIO DAS CONSULTAS V558
   Jornada móvel, pedido durável, confirmação dupla e rastreamento privado. */
import { store, escapeHTML } from './storage.js';
import { CONSULTATION_POLICY, consultationById, consultationPriceSnapshot } from './consultation-policy.js?v=558';
...
const CATALOG_KEY='consultation-catalog-v558';
...
export class ConsultationEngine{
  ...
  apiStatus(){
    if(this.apiState==='ready')return {className:'is-online',label:this.providerConfigured?'REGISTRO E E-MAIL CONECTADOS':'REGISTRO CONECTADO · E-MAIL PENDENTE'};
  ...
  serviceCard(service,index){
    ...
    return `<button type="button" class="consultation-v147-service${featured?' is-featured':''}" data-service="${escapeHTML(service.id)}" aria-describedby="service-${escapeHTML(service.id)}-detail">
      ...
      <strong>${moneyCents(service.priceCents)}</strong><small class="consultation-price-note">valor da consulta · sem cobrança agora</small><b>ESCOLHER ESTA CONSULTA <span aria-hidden="true">→</span></b>
    </button>`;
  }
  ...
  servicesMarkup(){
    return `<section class="consultation-flow-stage consultation-services-stage" aria-labelledby="consultationServicesTitle">
      ...
      <p class="consultation-human-note"><span aria-hidden="true">✦</span><span><b>Um pedido verdadeiro, atendido por uma pessoa.</b><small>Você escolhe, revisa e registra. A disponibilidade final é confirmada pelo canal oficial.</small></span></p>
    </section>`;
  }
  ...
  reviewMarkup(service){
    ...
    return `<section class="consultation-flow-stage consultation-review" aria-labelledby="consultationReviewTitle" aria-busy="${this.submitting}">
      ...
      <div class="consultation-review-actions"><button type="button" class="consultation-primary-action" data-confirm-request${this.submitting?' disabled':''}>${this.submitting?'REGISTRANDO COM SEGURANÇA…':apiMode?(this.providerConfigured?'REGISTRAR E GERAR PROTOCOLO':'REGISTRAR E PREPARAR E-MAIL'):'ABRIR E-MAIL PARA ENVIAR'}</button><button type="button" class="consultation-secondary-action" data-back-details${this.submitting?' disabled':''}>VOLTAR E EDITAR</button>${apiMode&&this.submitError?'<button type="button" class="consultation-link-button" data-email-fallback>USAR O E-MAIL OFICIAL</button>':''}</div>
    </section>`;
  }
  ...
  successMarkup(service){
    ...
    return `<section class="consultation-flow-stage consultation-success" aria-labelledby="consultationSuccessTitle">
      ...
      <h3 id="consultationSuccessTitle">${isApi?'Seu pedido atravessou o portal.':'Falta apenas enviar a mensagem.'}</h3><p>${isApi?(customerAccepted&&ownerAccepted?'Guardamos sua solicitação e os dois avisos foram aceitos pelo provedor de e-mail.':!ownerAccepted?'Guardamos sua solicitação e preparamos uma cópia para a equipe. No aplicativo de e-mail, revise e toque em Enviar.':'Guardamos sua solicitação; a sua confirmação por e-mail ainda precisa ser concluída.'):'Revise a mensagem aberta no seu aplicativo de e-mail e toque em Enviar.'}</p>
  ...
  scrollFlow(){this.root.querySelector('.consultation-flow-stage')?.scrollIntoView({behavior:'auto',block:'start'});}
  scrollTracker(){this.root.querySelector('[data-tracker]')?.scrollIntoView({behavior:'auto',block:'start'});}
}
