...
const inheritedBody=relative=>{
  if(relative.endsWith('.html'))return '<!doctype html><html><body><main>V561 inherited fixture · Música · Vídeos</main></body></html>';
  if(relative.endsWith('.js'))return '/* inherited V561 fixture */';
  if(relative.endsWith('.css'))return '/* inherited V561 fixture */';
  if(relative.endsWith('.json')||relative.endsWith('.webmanifest'))return '{}';
  return 'inherited-v561-fixture';
};
const fetchMock=async request=>{
  ...
  if(fs.existsSync(file)&&fs.statSync(file).isFile())body=fs.readFileSync(file);
    ...
    // O pacote V562 é um delta sobre a V561. Fixtures sintéticas representam
...
check(cacheStores.has('divina-bruxa-v562-shell'),'install-cache-v562');
check((await cacheStores.get('divina-bruxa-v562-shell').keys()).length>=126,'install-shell-atomico');
check(!cacheStores.has('divina-bruxa-v562-offline-core'),'install-sem-cache-opcional');

const shellCache=cacheStores.get('divina-bruxa-v562-shell');
...
check(result?.complete===true&&result?.atomic===true&&result?.version===562,'verify-shell-completo');
...
check(!cacheStores.has('divina-bruxa-v562-premium-static'),'premium-sem-cache-na-negacao');
...
check(response?.status===200,'navegacao-v562-valida');
const contentCache=cacheStores.get('divina-bruxa-v562-content');
check(Boolean(await contentCache?.match(new Request(`${scope}index.html?fresh=1`))),'navegacao-v562-cacheada');
...
check(!cacheStores.has('divina-bruxa-v562-offline-core')&&!cacheStores.has('divina-bruxa-v562-premium-static'),'stores-opcionais-removidas');
check(result?.authorityDataRemoved===false,'nao-inventa-exclusao-privada');

const quotaCache=await caches.open('divina-bruxa-v562-content');quotaCache.failNextPut=true;
...
console.log(`V562 Service Worker runtime: ${passed}/${total} verificações aprovadas (${inheritedFixtures.size} arquivos herdados simulados quando o teste roda no delta).`);
