/* DIVINA BRUXA V202 — CONSULTAS + RESEND TRANSACIONAL LOCALIZADO */
import "jsr:@supabase/functions-js/edge-runtime.d.ts";
import { createClient } from "npm:@supabase/supabase-js@2.112.4";
import { renderTransactionalEmailV202 } from "./transactional-email-templates-v202.ts";
...
async function ledgerFor(admin:any,consultationId:string,audience:"owner"|"customer"){
  const {data}=await admin.from("consultation_email_notifications")
    .select("delivery_status,provider_message_id,attempt_count,accepted_at,delivered_at,next_retry_at,updated_at")
...
async function recordEmail(admin:any,record:ConsultationRecord,audience:"owner"|"customer",recipient:string,status:string,details:Record<string,unknown>={}){
  const {data,error}=await admin.rpc("consultation_email_record_server_v202",{
...
function notificationSummary(value:any){
  ...
  return {
    status,
    accepted:["accepted","delivered"].includes(status),
    delivered:status==="delivered",
    blocked:["bounced","complained","suppressed"].includes(status)
  };
}
...
async function sendEmail(admin:any,record:ConsultationRecord,audience:"owner"|"customer",trackingToken:string){
  ...
  if(["bounced","complained","suppressed"].includes(String(existing?.delivery_status||"")))return {...notificationSummary(existing),blocked:true};

  const {data:suppressed,error:suppressionError}=await admin.rpc("transactional_email_is_suppressed_v202",{p_recipient_email:recipient});
  if(suppressionError){
    console.error("consultation email",{consultationId:record.id,audience,code:"SUPPRESSION_CHECK_FAILED"});
    return {status:"failed",accepted:false,delivered:false};
  }
  if(suppressed===true){
    const result=await recordEmail(admin,record,audience,recipient,"suppressed",{errorCode:"RECIPIENT_SUPPRESSED"});
    return {...notificationSummary(result),blocked:true};
  }
  ...
  const rendered=renderTransactionalEmailV202(
    isOwner?"consultation_owner_new":"consultation_customer_confirmation",
    "pt-BR",
    {
      personName:isOwner?"Proprietária":record.customer_name,
      protocol:record.protocol,
      serviceName:name,
      amountLabel:currency(record.price_brl_cents_snapshot),
      preferenceLabel:schedule,
      trackingCode:isOwner?"":trackingToken,
      customerName:isOwner?record.customer_name:"",
      customerEmail:isOwner?record.customer_email:"",
      customerPhone:isOwner?record.customer_phone:"",
      questionContext:isOwner?record.question_context:"",
      actionUrl:"https://divinabruxa.com.br/#consultations"
    }
  );
  ...
  try{
    const response=await fetch(RESEND_ENDPOINT,{
      ...
      headers:{
        ...
        "Idempotency-Key":`consultation/${record.id}/${audience}/v202`
      },
      body:JSON.stringify({
        from:sender,
        to:[recipient],
        ...
        html:rendered.html,
        text:rendered.text,
        headers:{"X-Entity-Ref-ID":`${record.id}:${audience}`},
        tags:[
          {name:"environment",value:"staging"},
          {name:"release",value:"v202"},
          {name:"purpose",value:isOwner?"consultation_owner":"consultation_customer"}
        ]
      })
    });
    if(!response.ok){
