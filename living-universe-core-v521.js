/* DIVINA BRUXA — UNIVERSO VIVO · MACROETAPA 1/4 · MOTOR RETINA CELESTIAL V521
   Um único céu procedural, tátil e supersampled respira atrás de todas as realidades.
   Nuvens filamentosas, estrelas espectrais, gravidade da Orbe e a Chama
   Celestial aprovada compartilham o mesmo canvas — sem foto ou motor extra. */

const VERSION = 521;
const STYLE_ID = 'divinaLivingUniverseV521Styles';
const ROOT_ID = 'divinaLivingUniverseV521';
...
function installStyle() {
  ...
  link.id = STYLE_ID;
  link.rel = 'stylesheet';
  link.href = './living-universe-core-v521.css?v=521';
...
class LivingUniverseCoreV521 {
  constructor() {
    ...
    this.root.className = 'db521-universe';
    this.root.setAttribute('aria-hidden', 'true');
    this.root.innerHTML = '<canvas data-living-universe-canvas></canvas><span class="db521-universe__veil"></span>';
    ...
    document.documentElement.dataset.livingUniverse = 'v521';
    document.body.classList.remove('db516-universe-active','db519-universe-active','db520-universe-active');
    document.body.classList.add('db521-universe-active');
    ...
    const readiness = Object.freeze({
      ...
      adaptivePixelBudget:true,
      spectralStarHalos:true,
      highFrequencyCloudFilaments:true,
    ...
    try {
      ...
      const fragment = this.createShader(this.gl.FRAGMENT_SHADER, `
        ...
        float starLayer(vec2 p,float scale,float seed,float time,out float giant){
          ...
          float visible=step(0.944-random*0.004,random);
          float pixel=scale/max(min(u_resolution.x,u_resolution.y),1.0);
          float radius=pixel*mix(0.58,1.12,hash21(id+4.7));
          float distanceToStar=length(delta);
          float core=(1.0-smoothstep(radius,radius+pixel*1.18,distanceToStar))*visible;
          float halo=(1.0-smoothstep(radius*1.25,radius+pixel*5.4,distanceToStar))*visible;
          float twinkle=0.54+0.46*sin(time*(0.7+hash21(id+9.1)*1.8)+random*29.0);
          twinkle*=twinkle;
          giant=step(0.994,random)*visible;
          float vertical=(1.0-smoothstep(pixel*0.48,pixel*1.16,abs(delta.x)))
            *(1.0-smoothstep(pixel*1.8,pixel*13.5,abs(delta.y)))*giant;
          float horizontal=(1.0-smoothstep(pixel*0.48,pixel*1.16,abs(delta.y)))
            *(1.0-smoothstep(pixel*1.8,pixel*13.5,abs(delta.x)))*giant;
          return core*(0.58+twinkle*0.92)+halo*(0.075+twinkle*0.10)
            +(vertical+horizontal)*0.62*twinkle;
        }
        ...
        void livingConstellations(vec2 point,float aspect,float time,out float lines,out float stars){
          ...
          if(abs(crown.x)<0.18*constellationScale.x+0.02
            ...
            stars+=constellationStar(crown,c0,0.78+0.22*sin(time*0.78+0.2));
            stars+=constellationStar(crown,c1,0.78+0.22*sin(time*0.71+1.3));
            stars+=constellationStar(crown,c2,0.78+0.22*sin(time*0.84+2.5));
            stars+=constellationStar(crown,c3,0.78+0.22*sin(time*0.67+3.4));
            stars+=constellationStar(crown,c4,0.78+0.22*sin(time*0.75+4.6));
          }
          ...
          if(abs(river.x)<0.145*constellationScale.x+0.02
            ...
            stars+=constellationStar(river,r0,0.78+0.22*sin(time*0.69+0.8));
            stars+=constellationStar(river,r1,0.78+0.22*sin(time*0.82+1.7));
            stars+=constellationStar(river,r2,0.78+0.22*sin(time*0.73+2.8));
            stars+=constellationStar(river,r3,0.78+0.22*sin(time*0.79+3.9));
            stars+=constellationStar(river,r4,0.78+0.22*sin(time*0.66+4.8));
            stars+=constellationStar(river,r5,0.78+0.22*sin(time*0.76+5.7));
          }
        }
        ...
        void main(){
          ...
          float velocityStrength=clamp(length(touchVelocity)*3.8,0.0,1.0);
          float touchField=exp(-touchDistance*3.65)*u_touch_energy;
          float touchWave=sin(touchDistance*23.0-time*2.55+u_seed*9.0);
          vec2 touchFlow=touchTangent*touchField*(0.010+velocityStrength*0.034)
            *(0.62+touchWave*0.38)
            +touchVelocity*touchField*0.12
            +touchNormal*touchWave*touchField*0.0075;
          ...
          float cloud=fbm(p*2.14+warp*2.22+slow*0.7);
          float detail=fbmDetail(p*5.28-warp*1.18-slow*1.35);
          float micro=noise2(p*18.6+warp*4.65-slow*2.1+u_seed*11.0);
          float microFine=noise2(p*39.0-warp*6.2+slow*3.1-u_seed*17.0);
          float curl=0.5+0.5*sin((p.y+warp.x*0.31)*38.0
            +(p.x-warp.y*0.22)*16.0-time*0.085);
          float ridge=1.0-abs(cloud*2.0-1.0);
          float edge=smoothstep(0.10,0.72,abs(uv.x-0.5)*2.0);
          float filament=pow(max(ridge,0.0),3.45);
          float silk=smoothstep(0.48,0.84,detail*0.50+micro*0.32+curl*0.18)
            *smoothstep(0.24,0.92,filament);
          float ribbonLeft=exp(-abs(p.x+aspect*0.37
            +sin(p.y*5.7+warp.x*2.5+time*0.026)*0.052)*18.0);
          float ribbonRight=exp(-abs(p.x-aspect*0.39
            +cos(p.y*5.1-warp.y*2.3-time*0.023)*0.058)*17.0);
          float ribbons=(ribbonLeft+ribbonRight)*(0.30+detail*0.46+micro*0.24);
          float strandWave=0.5+0.5*sin((p.y+warp.x*0.16)*76.0
            +(p.x-warp.y*0.11)*34.0-time*0.16);
          float strands=pow(strandWave,7.5)*ribbons*(0.28+microFine*0.72);
          float nebula=smoothstep(0.44,0.78,
            cloud*0.64+detail*0.20+filament*0.14+silk*0.10+edge*0.075);
          float darkDust=smoothstep(0.58,0.88,
            (1.0-detail)*0.48+(1.0-micro)*0.22+cloud*0.30);
          float cloudResponse=1.0+touchField*(0.12+0.08*sin(time*0.88-touchDistance*14.0));
          nebula=(nebula+ribbons*0.24+strands*0.16)
            *mix(0.46,1.16,edge)*u_density*(1.0-darkDust*0.34)*breath*cloudResponse;
          float hotDust=smoothstep(0.55,0.88,
            detail*0.42+filament*0.30+silk*0.22+micro*0.12+edge*0.055);

          vec3 color=vec3(0.0018,0.0005,0.0048);
          color+=u_accent*nebula*(0.19+filament*0.42+silk*0.22);
          color+=mix(u_accent,u_light,0.42)*hotDust*nebula
            *(0.14+u_warmth*0.20+silk*0.20);
          color+=u_gold*pow(max(hotDust*silk,0.0),2.35)*0.31*u_warmth;
          color+=mix(u_accent,u_light,0.58)*strands*nebula*0.20;
          float stellarMist=smoothstep(0.86,0.985,microFine)*nebula
            *(0.34+silk*0.66);
          color+=mix(u_light,u_gold,u_warmth)*stellarMist*0.13;
          color-=vec3(0.0018,0.0007,0.0025)*darkDust;
          ...
          color+=mix(u_accent,u_gold,0.38)*g1*0.44*u_depth;
          color+=mix(u_accent,u_light,0.28)*g2*0.38*u_depth;
          color+=mix(u_accent,u_gold,0.56)*g3*0.22*u_depth;
          ...
          float constellationBreath=0.76+0.24*sin(time*0.31+u_seed*6.283185);
          vec3 constellationColor=mix(u_light,u_gold,0.48+u_warmth*0.18);
          color+=constellationColor*constellationLines*0.115*u_depth*constellationBreath;
          color+=mix(constellationColor,vec3(1.0),0.26)*constellationStars*0.66*u_depth;
          ...
          float touchBreath=0.72+0.28*sin(time*1.12-touchDistance*16.0);
          color+=mix(u_accent,u_gold,u_warmth)*touchGlow*0.16;
          color+=mix(u_accent,u_light,0.5)*touchField*touchBreath*0.12;
  ...
  setPerformanceBudget({ fps, scale, profile } = {}) {
    ...
    const nextScale = clamp(Number(scale) || this.qualityCeiling, 0.50, 0.90);
  ...
  resize() {
    this.width = Math.max(1, document.documentElement.clientWidth || innerWidth);
    this.height = Math.max(1, innerHeight);
    const mobile = this.width < 700;
    const profile = this.qualityProfile || 'balanced';
    const ratioCeiling = constrained() || profile === 'protected'
      ? 1.45
      : profile === 'cinematic'
        ? (mobile ? 2.20 : 1.82)
        : (mobile ? 2.05 : 1.70);
    const pixelBudget = constrained() || profile === 'protected'
      ? 680000
      : profile === 'cinematic'
        ? (mobile ? 1500000 : 2400000)
        : (mobile ? 1120000 : 1850000);
    this.pixelRatio = Math.min(ratioCeiling, Math.max(1, devicePixelRatio || 1));
    const requestedRatio = this.pixelRatio * this.scale;
    const budgetRatio = Math.sqrt(pixelBudget / Math.max(1, this.width * this.height));
    const renderRatio = Math.max(0.72, Math.min(requestedRatio, budgetRatio));
  ...
  drawCanvasConstellations(context, time) {
    ...
    for (const [groupIndex,definition] of definitions.entries()) {
      ...
      const breathing = 0.68+Math.sin(time*0.31+definition.phase)**2*0.22;
      context.strokeStyle = `rgba(${color[0]},${color[1]},${color[2]},${0.18*breathing})`;
      ...
      points.forEach((point,index) => {
        const pulse = 0.56+Math.sin(time*(0.68+index*0.025)+index+definition.phase)**2*0.44;
        context.globalAlpha = breathing*(0.62+pulse*0.38);
        context.fillStyle = index % 3 === 0
          ? `rgb(${light[0]} ${light[1]} ${light[2]})`
          : `rgb(${color[0]} ${color[1]} ${color[2]})`;
        context.beginPath();
        context.arc(point.x,point.y,0.9+pulse*1.05,0,Math.PI*2);
        context.fill();
      });
  ...
  drawCanvas(timestamp) {
    ...
    for (let index=0; index<clouds; index+=1) {
      ...
      const source = this.warpCanvasPoint({
        x:this.width * (0.5 + Math.sin(phase + time * 0.018 * this.profile.drift) * (0.34 + index % 2 * 0.09) - this.camera.x * depth),
        y:this.height * (0.5 + Math.cos(phase * 0.74 - time * 0.014 * this.profile.drift) * 0.43 - this.camera.y * depth - this.scroll * 0.025 * depth)
      }, 1.2-depth*0.34);
      const x = source.x;
      const y = source.y;
      const breathing = 0.96+Math.sin(time*0.22+phase)*0.04+this.touch.energy*0.055;
  ...
  adapt(delta) {
    ...
    if (this.slowFrames >= 24 && this.scale > 0.50) {
      this.slowFrames = 0;
      this.calmFrames = 0;
      this.degraded = true;
      this.scale = Math.max(0.50, this.scale - 0.06);
      this.resize();
    ...
    if (this.degraded && this.calmFrames >= Math.round(this.targetFps * 10)) {
      this.calmFrames = 0;
      this.scale = Math.min(this.qualityCeiling, this.scale + 0.03);
      this.resize();
  ...
  frame(timestamp) {
    ...
    if (timestamp - this.lastDraw >= interval - 0.5) {
      ...
      if (this.touch.active) this.touch.target = 1;
  ...
  status() {
    return Object.freeze({
      ...
      staticUniverseImage:false,
      globalAcrossRoutes:true,
      oneUniverseCanvas:document.querySelectorAll('#divinaLivingUniverseV521 canvas').length === 1,
      targetFps:this.targetFps,
...
export function createLivingUniverseV521() {
  if (globalThis.divinaLivingUniverseV521?.status) return globalThis.divinaLivingUniverseV521;
  const former = globalThis.divinaLivingUniverseV520 || globalThis.divinaLivingUniverseV519 || globalThis.divinaLivingUniverseV516;
  ...
  document.body?.classList.remove('db516-universe-active','db519-universe-active','db520-universe-active');
  const universe = new LivingUniverseCoreV521();
