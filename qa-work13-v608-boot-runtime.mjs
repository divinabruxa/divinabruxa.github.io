/* DIVINA BRUXA — WORK13 V608 · QA DO BOOT RECUPERAVEL */
...
function harness() {
  ...
  vm.runInContext(bootScript, vm.createContext(context), { filename:'work13-inline-boot-v608.js' });
...
check('boot:work13-meta-v608', indexSource.includes('name="divina-work13" content="V608"'));
check('boot:work12-meta-v600', indexSource.includes('name="divina-work12" content="V600"'));
check('boot:app-v608', indexSource.includes('app-v208.js?v=608-work13-commerce-clarity'));
check('boot:worker-v608', indexSource.includes("register('./sw.js?v=608'"));
check('boot:v608-bootstrap', indexSource.includes("__divinaSWBootstrap='v608-work13-commerce-clarity-inline'"));
check('boot:v608-style', indexSource.includes('living-commerce-path-v608.css?v=608-work13-commerce-clarity'));
...
for (const id of [
  'divinaLivingCommercePathV608','divinaLivingWisdomPathV607','divinaCosmicSpreadReadingV605',
  'divinaCosmicDailyReadingV604','divinaCosmosRealityResonanceV603','divinaWork12FinalContinuityV598'
...
check('boot:watchdog-work13-v608', watchdog.root.dataset.work13Boot === 'released-v608');
...
check('boot:manual-work13', manual.root.dataset.work13Boot === 'released-v608');
...
check('boot:healthy-work13-ready', healthy.root.dataset.work13Boot === 'ready-v608');
check('boot:healthy-never-bypassed', !healthy.home.classList.contains('active'));
healthy.fire('load');
await Promise.resolve();
check('boot:registers-v608', healthy.registrations.some(item => item.url === './sw.js?v=608'));
...
console.log(JSON.stringify({
  release:'V608',
  work:'WORK13',
  macroStage:'8-of-10 / commerce-clarity-boot-runtime',
