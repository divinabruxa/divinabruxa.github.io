/* DIVINA BRUXA — WORK13 V602 · QA SERVICE WORKER ATOMICO */
...
const assets = Object.freeze({
  './index.html':'index',
  './app-v208.js?v=602-work13-context':'app',
...
const cacheName = 'divina-bruxa-work13-v602-context-memory';
...
check('install:twenty-seven-core-assets', cache?.size === 27, cache?.size);
...
harness.stores.set('divina-bruxa-work12-v600-one-style', new Map());
await harness.wait('activate');
check('activate:claims-clients', harness.counters().claimed === 1);
check('activate:keeps-v602', harness.stores.has(cacheName));
check('activate:deletes-v600-cache', !harness.stores.has('divina-bruxa-work12-v600-one-style'));
for (const type of [
  ...
  'DIVINA_WORK13_CONTEXT_ACTIVE','DIVINA_RELEASE_READY'
]) check(`activate:${type}`, harness.clientMessages.some(item => item.type === type && item.version === 602));
check('activate:context-base-v600', harness.clientMessages.some(item => item.type === 'DIVINA_WORK13_CONTEXT_ACTIVE' && item.base === 600));

const onlineNavigation = await harness.fetchEvent(new FakeRequest('https://divina.test/#daily', { mode:'navigate' }));
check('fetch:navigation-network', (await onlineNavigation.text()).includes('name="divina-work13" content="V602"'));
...
for (const [file, needle] of [
  ['app-v208.js?v=602-work13-context','divinaWork13Macro2V602'],
...
check('message:status-version', harness.sourceMessages.some(item => item.type === 'WORK12_STATUS' && item.version === 602));
check('message:status-twenty-seven-assets', harness.sourceMessages.some(item => item.type === 'WORK12_STATUS' && item.core?.length === 27));
await harness.message({ type:'CLEAR_DIVINA_CACHES' });
check('message:clear-caches', !harness.stores.has(cacheName));
check('message:clear-confirmed', harness.sourceMessages.some(item => item.type === 'DIVINA_CACHES_CLEARED' && item.version === 602));

const failures = checks.filter(item => !item.pass);
console.log(JSON.stringify({
  release:'V602',
  work:'WORK13',
  macroStage:'2-of-10 / context-service-worker-runtime',
