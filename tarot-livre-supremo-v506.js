/* DIVINA BRUXA — MACROETAPA V506 · TAROT LIVRE SUPREMO
...
class SupremeStellarFlameEngineV506 {
  constructor({ cosmos, effects, stage, orb, card }) {
    ...
    this.orb = orb;
    this.card = card;
    this.world = stage.closest('.tl506');
  ...
  drawFallbackCosmos(context, timestamp) {
    ...
    context.shadowBlur = constrained() ? 24 : 32;
    context.globalAlpha = 0.22;
    context.lineWidth = constrained() ? 52 : 68;
    trace(0, 0.1);
    context.stroke();

    context.strokeStyle = bodyGradient;
    context.shadowColor = '#ff38ca';
    context.shadowBlur = constrained() ? 16 : 22;
    context.globalAlpha = 0.9;
    context.lineWidth = constrained() ? 16 : 22;
    trace(0, 0.12);
    context.stroke();
    ...
    context.shadowBlur = constrained() ? 11 : 16;
    context.globalAlpha = 0.96;
    context.lineWidth = constrained() ? 3.6 : 5;
    trace(0, 0.1);
    context.stroke();
    ...
    for (const strand of strands) {
      ...
      context.shadowBlur = constrained() ? 13 : 18;
      context.strokeStyle = strand.color;
      context.globalAlpha = 0.88;
      context.lineWidth = constrained() ? 2.8 : 4.2;
      ...
      context.lineWidth = constrained() ? 0.7 : 1.05;
      trace(strand.phase, 1);
      context.stroke();
    }

    const tongueCount = constrained() ? 10 : 15;
    for (let index = 0; index < tongueCount; index += 1) {
      ...
      const reach = (constrained() ? 12 : 17) + (index % 4) * 3.5;
      const side = flicker * (7 + index % 3 * 2.5);
      context.beginPath();
      ...
      context.globalAlpha = 0.3 + 0.22 * (0.5 + 0.5 * flicker);
      context.lineWidth = constrained() ? 1.15 + index % 3 * 0.45 : 1.55 + index % 3 * 0.62;
    ...
    const starCount = constrained() ? 16 : 22;
  ...
  drawBirthWave(context, timestamp) {
    ...
    context.strokeStyle = `rgba(174,43,255,${0.38 + pulse * 0.1})`;
    context.lineWidth = constrained() ? 22 : 30;
    traceTail();
    context.stroke();
    context.shadowColor = '#ff5fcf';
    context.shadowBlur = 16;
    context.strokeStyle = `rgba(255,66,190,${0.72 + pulse * 0.08})`;
    context.lineWidth = constrained() ? 7.5 : 10.5;
    traceTail();
    context.stroke();
    context.globalCompositeOperation = 'lighter';
    context.shadowColor = '#ffae49';
    context.shadowBlur = 12;
    context.strokeStyle = `rgba(255,214,129,${0.88 + pulse * 0.07})`;
    context.lineWidth = constrained() ? 2.2 : 3.2;
    traceTail();
    context.stroke();
...
export class TarotLivreSupremeV506 {
  constructor(root, {
    storage = store,
    ...
    if (!orbCore?.claim || !orbCore?.orb) {
      throw new Error('A Orbe Suprema V501 precisa despertar antes do Tarot Livre V506.');
    }
    ...
    this.root.innerHTML = '';
    this.root.dataset.tarotWorld = 'supremo-v506';
    ...
    this.world.dataset.flameTexture = 'organic';
    this.render(false);
    this.fire = new SupremeStellarFlameEngineV506({
    ...
    document.documentElement.dataset.tarotLivre = 'v506';
    const readiness = Object.freeze({
      version: VERSION,
      engine: 'SupremeStellarFlameEngineV506',
  ...
  build() {
    this.root.innerHTML = `
      <div class="tl506" data-phase="idle" data-engine="pending">
        <header class="tl506__header">
          <div class="tl506__title">
            <h2>Tarot Livre</h2>
            <span class="tl506__title-star" aria-hidden="true"><i></i><b></b><i></i></span>
          </div>

          <a class="tl506__mesa" href="#spreads" data-go="spreads" aria-label="Abrir Tiragens e Mesa Real">
        ...
        <section class="tl506__stage" data-stage aria-label="Tarot Livre — Cosmos de Fogo">
          <canvas class="tl506__canvas tl506__cosmos" data-cosmos aria-hidden="true"></canvas>
          <canvas class="tl506__canvas tl506__effects" data-effects aria-hidden="true"></canvas>

          <div class="tl506__portal-crown" aria-hidden="true"><i></i><i></i><i></i></div>

          <div class="tl506__card-zone" data-card-zone tabindex="0"
               aria-label="Carta atual. Use as setas para navegar pelas cartas reveladas.">
            <div class="tl506__card" data-current-card data-empty="true">
              <canvas class="tl506__card-back" data-card-back aria-hidden="true"></canvas>
            </div>
          </div>

          <nav class="tl506__card-nav" data-card-nav aria-label="Navegar pelas cartas reveladas">
          ...
          <div class="tl506__orb-host" data-tarot-orb-host>
            <span class="tl506__orb-aura" aria-hidden="true"><i></i><i></i><i></i></span>
            <span class="tl506__orb-flare" aria-hidden="true"></span>
          </div>

          <p class="tl506__counter" aria-label="Cartas reveladas">
            <b data-count>0</b><span>/78</span>
          </p>
          <p class="tl506__signal" data-signal aria-live="polite">Toque na Orbe para revelar seu Tarot.</p>
        </section>

        <div class="tl506__toolbar" aria-label="Controles do Tarot Livre">
        ...
        <section class="tl506__constellation" aria-labelledby="tl506ConstellationTitle">
          <header>
            <div>
              <small>SEU CÍRCULO</small>
              <h3 id="tl506ConstellationTitle">Cartas reveladas</h3>
            </div>
            <span data-remaining>78 aguardam</span>
          </header>
          <div class="tl506__grid" data-grid role="grid"
               aria-label="Cartas reveladas, seis por fileira" aria-colcount="6"></div>
          <p class="tl506__empty-grid" data-empty-grid>Nenhuma carta foi revelada ainda.</p>
        </section>

        <p class="tl506__sr" data-live role="status" aria-live="polite" aria-atomic="true"></p>
      </div>`;

    this.world = this.root.querySelector('.tl506');
  ...
  enter() {
    ...
    this.orb.dataset.tarotReveal = 'v506';
  ...
  leave(nextRoute = 'home') {
    ...
    document.body?.classList.remove('db506-tarot-birthing');
    this.fire?.setActive(false);
    ...
    this.orbCore.settleRoute?.(nextRoute, 'tarot-leave-v506');
    return true;
  }
  ...
  setPhase(phase) {
    ...
    document.body?.classList.toggle('db506-tarot-birthing', this.active && ritualVisible);
    ...
    try {
      ...
      globalThis.dispatchEvent?.(new CustomEvent('tarot:rebirth-revealed', {
        detail: {
          ...
          engine: 'supreme-stellar-flame-v506'
        }
      }));
  ...
  show(index, animate = false) {
    ...
    this.card.innerHTML = `${cardImageMarkup(card, {
      ...
      priority: 'high'
    })}<div class="tl506__card-name"><b>${escapeText(card.name)}</b><small>${index + 1} / ${this.state.revealed.length}</small></div>`;
  ...
  render(animateCurrent = false) {
    ...
    if (!revealed) {
      ...
      this.card.innerHTML = '<canvas class="tl506__card-back" data-card-back aria-hidden="true"></canvas>';
  ...
  status() {
    return Object.freeze({
      ...
      organicCanvasFire: this.world.dataset.flameTexture === 'organic'
    });
  }
  ...
  destroy() {
    ...
    this.fire?.destroy();
    if (globalThis.divinaTarotLivreV506 === this) delete globalThis.divinaTarotLivreV506;
