...
export function grantCredits(amount, source = 'sandbox') { const credits = Math.max(0, Math.floor(Number(amount))); if (!credits) return false; const current = creditState(); store.set(KEY, {...current, remaining: current.remaining + credits, granted: current.granted + credits, entries: [...current.entries, { source, credits, at: new Date().toISOString() }].slice(-100)}); return true; }
