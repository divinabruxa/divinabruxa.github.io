/* DIVINA BRUXA — WORK13 V606 · QA DO BOOT RECUPERAVEL */
...
function harness() {
  ...
  vm.runInContext(bootScript, vm.createContext(context), { filename:'work13-inline-boot-v606.js' });
...
check('boot:work13-meta-v606', indexSource.includes('name="divina-work13" content="V606"'));
check('boot:work12-meta-v600', indexSource.includes('name="divina-work12" content="V600"'));
check('boot:app-v606', indexSource.includes('app-v208.js?v=606-work13-whit-timing'));
check('boot:worker-v606', indexSource.includes("register('./sw.js?v=606'"));
check('boot:v606-bootstrap', indexSource.includes("__divinaSWBootstrap='v606-work13-whit-timing-inline'"));
...
check('boot:watchdog-work13-v606', watchdog.root.dataset.work13Boot === 'released-v606');
...
check('boot:manual-work13', manual.root.dataset.work13Boot === 'released-v606');
...
check('boot:healthy-work13-ready', healthy.root.dataset.work13Boot === 'ready-v606');
check('boot:healthy-never-bypassed', !healthy.home.classList.contains('active'));
healthy.fire('load');
await Promise.resolve();
check('boot:registers-v606', healthy.registrations.some(item => item.url === './sw.js?v=606'));
...
console.log(JSON.stringify({
  release:'V606',
  work:'WORK13',
  macroStage:'6-of-10 / whit-silence-timing-boot-runtime',
