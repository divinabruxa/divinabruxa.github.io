/* DIVINA BRUXA — WORK12 · QA BOOT RECUPERÁVEL · AUDITORIA VIVA V599 */
...
function harness(){
  ...
  vm.runInContext(bootScript,vm.createContext(context),{filename:'work12-inline-boot-v599.js'});
...
check('boot:worker-v599',indexSource.includes("register('./sw.js?v=599'"));
...
check('boot:watchdog-v599',watchdog.root.dataset.work12Boot==='released-v599'&&watchdog.root.dataset.work12BootReason==='boot-watchdog');
check('boot:watchdog-event-v599',watchdog.events.some(event=>event.type==='divina:work12-boot-release'&&event.detail.version===599));
check('boot:recovery-source-v599',watchdog.events.some(event=>event.type==='divina:loading-bypass'&&event.detail.source==='work12-live-audit-v599'));
...
check('boot:healthy-never-bypassed',healthy.root.dataset.work12Boot==='ready-v599'&&!healthy.home.classList.contains('active'));

const failures=checks.filter(item=>!item.pass);
console.log(JSON.stringify({release:'V599',work:'WORK12',macroStage:'10-of-10 / live-audit-boot-runtime',state:failures.length?'FAIL':'PASS',passed:checks.length-failures.length,failed:failures.length,total:checks.length,failures:failures.map(({id,detail})=>({id,detail}))},null,2));
