...
const css = await readFile(new URL('./biblioteca-soul-v610.css', import.meta.url),'utf8');
...
for (const [id,width,height,dpr] of profiles) {
  ...
  const archiveWidth = portrait && width <= 430 ? width - 16 : Math.min(width - 28,1160);
  const discoveryHeight = portrait && width <= 430 ? Math.min(height * .52,450) : (height <= 520 ? 250 : Math.min(height * .60,650));
  const shelfWidth = portrait && width <= 430 ? Math.min(width * .78,284) : Math.min(archiveWidth * .48,360);
  ...
  ok(shelfWidth >= 280 || !portrait, `${id}: uma porta respira por vez`);
...
ok(css.includes('width:min(calc(100% - 28px),1160px)'), 'largura máxima da Biblioteca');
...
ok(css.includes('grid-auto-columns:min(78%,284px)'), 'uma carta por respiração');
...
console.log(`PASS ${checks}/${checks} — Biblioteca em 8 geometrias iPhone`);
