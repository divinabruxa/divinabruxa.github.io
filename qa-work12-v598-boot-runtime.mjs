/* DIVINA BRUXA — WORK12 · QA BOOT RECUPERÁVEL · V598 */
...
function harness(){
  ...
  vm.runInContext(bootScript,vm.createContext(context),{filename:'work12-inline-boot-v598.js'});
...
check('boot:worker-v598',indexSource.includes("register('./sw.js?v=598'"));
check('boot:intelligence-style-v597',indexSource.includes('experience-intelligence-v597.css?v=597-work12-intelligence'));
check('boot:final-style-v598',indexSource.includes('work12-final-continuity-v598.css?v=598-work12-final'));
...
check('boot:watchdog-v598',watchdog.root.dataset.work12Boot==='released-v598'&&watchdog.root.dataset.work12BootReason==='boot-watchdog');
check('boot:watchdog-event-v598',watchdog.events.some(event=>event.type==='divina:work12-boot-release'&&event.detail.version===598));
check('boot:recovery-source-v598',watchdog.events.some(event=>event.type==='divina:loading-bypass'&&event.detail.source==='work12-final-v598'));
...
check('boot:healthy-never-bypassed',healthy.root.dataset.work12Boot==='ready-v598'&&!healthy.home.classList.contains('active'));

const failures=checks.filter(item=>!item.pass);
console.log(JSON.stringify({release:'V598',work:'WORK12',macroStage:'10-of-10 / boot-runtime',state:failures.length?'FAIL':'PASS',passed:checks.length-failures.length,failed:failures.length,total:checks.length,failures:failures.map(({id,detail})=>({id,detail}))},null,2));
