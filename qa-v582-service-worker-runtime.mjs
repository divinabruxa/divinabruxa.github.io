/* DIVINA BRUXA 2.0 — FLUIDEZ SUPREMA · SERVICE WORKER RUNTIME V582 */
...
check(cacheStores.has('divina-bruxa-v582-shell'),'install:cache-v582');
const shellCache=cacheStores.get('divina-bruxa-v582-shell');
check((await shellCache.keys()).length>=127,'install:atomic-shell-size');
for(const file of [
  'app-v208.js','orb-engine-v208.js','whit-orb-soul-bridge-v581.js',
  'living-universe-core-v524.js','orb-persistent-journey-v565.js','tarot-livre-orbe-os-v517.js'
]){
  const response=await shellCache.match(new Request(`${scope}${file}`));
  check(response?.status===200,`install:critical:${file}`);
}

const oldCache=await caches.open('divina-bruxa-v581-shell');
...
check(!cacheStores.has('divina-bruxa-v581-shell'),'activate:remove-v581-cache');

let result=await message({type:'VERIFY_SHELL'});
check(result?.complete===true&&result?.atomic===true&&result?.version===582,'verify:complete-v582');
check(result?.ready===result?.total,'verify:complete-count');

online=false;
let response=await fetchEvent(new Request(`${scope}whit-orb-soul-bridge-v581.js?v=582-living-soul`));
check(response?.status===200&&(await response.text()).includes('ALMA WHIT V582'),'offline:soul-from-atomic-shell');
response=await fetchEvent(new Request(`${scope}orb-engine-v208.js?v=582-living-soul`));
check(response?.status===200&&(await response.text()).includes('ORB_SOUL_STATE_MAP_V582'),'offline:renderer-from-atomic-shell');
...
response=await fetchEvent({
  url:`${scope}index.html?v582=1`,method:'GET',mode:'navigate',destination:'document',
  cache:'default',headers:new Headers()
});
check(response?.status===200&&(await response.text()).includes('app-v208.js?v=582'),'navigation:v582-shell-valid');
const contentCache=cacheStores.get('divina-bruxa-v582-content');
check(Boolean(await contentCache?.match(new Request(`${scope}index.html?v582=1`))),'navigation:valid-shell-cached');

await shellCache.delete(new Request(`${scope}orb-engine-v208.js`));
result=await message({type:'VERIFY_SHELL'});
check(result?.complete===false&&result?.missing?.includes('./orb-engine-v208.js'),'repair:renderer-corruption-detected');
result=await message({type:'REPAIR_SHELL'});
check(result?.complete===true,'repair:atomic-shell-restored');

const total=passed+failures.length;
console.log(JSON.stringify({
  release:'V582',base:'V581',state:failures.length?'FAIL':'PASS',passed,failed:failures.length,total,failures
},null,2));
