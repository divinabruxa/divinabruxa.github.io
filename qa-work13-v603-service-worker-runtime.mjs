/* DIVINA BRUXA — WORK13 V603 · QA SERVICE WORKER ATOMICO */
...
const sourceFiles = Object.freeze({
  index:'./index.html',
  app:'./app-v208.js',
  context:'./cosmos-context-memory-v602.js',
  resonance:'./cosmos-reality-resonance-v603.js',
  'resonance-styles':'./cosmos-reality-resonance-v603.css',
  final:'./work12-final-continuity-v598.js',
  'final-styles':'./work12-final-continuity-v598.css',
  intelligence:'./experience-intelligence-v597.js',
  'intelligence-styles':'./experience-intelligence-v597.css',
  chambers:'./reality-chambers-v596.js',
  'chamber-styles':'./reality-chambers-v596.css',
  school:'./school-world-v306.js',
  journal:'./journal-world-v317.js',
  ritual:'./reading-ritual-core-v595.js',
  'ritual-styles':'./reading-ritual-core-v595.css',
  daily:'./daily-world-v509.js',
  whit:'./whit-living-presence-v594.js',
  menu:'./orbital-menu-v502.js',
  'menu-styles':'./orbital-menu-v502.css',
  navigation:'./navigation.js',
  foundation:'./work12-foundation-v589.js',
  'page-loader':'./page-loader-v1.js',
  universe:'./living-universe-core-v524.js',
  orb:'./supreme-orb-core-v501.js',
  renderer:'./orb-engine-v208.js',
  journey:'./orb-persistent-journey-v565.js',
  soul:'./whit-orb-soul-bridge-v581.js',
  shell:'./divina-shell-v180.css',
  bridge:'./cosmic-visual-atlas-v1.js'
});
const sources = Object.freeze(Object.fromEntries(
  Object.entries(sourceFiles).map(([key, file]) => [key, read(file)])
));
const assets = Object.freeze({
  './index.html':'index',
  './app-v208.js?v=603-work13-resonance':'app',
  './cosmos-context-memory-v602.js?v=602-work13-context':'context',
  './cosmos-reality-resonance-v603.js?v=603-work13-resonance':'resonance',
  './cosmos-reality-resonance-v603.css?v=603-work13-resonance':'resonance-styles',
...
function createHarness({ broken = null } = {}) {
  ...
  let claimed = 0;
  let skipped = 0;
  const paths = new Map(coreAssets.map(asset => [
    asset,
    broken === assets[asset] ? `/* broken ${broken} */` : sources[assets[asset]]
  ]));
  const caches = {
    async open(name) {
  ...
  const fetch = async input => {
    ...
    if (request.mode === 'navigate' || url.pathname === '/' || url.pathname.endsWith('/index.html')) {
      return new FakeResponse(sources.index);
    }
...
const cacheName = 'divina-bruxa-work13-v603-reality-resonance';
...
check('install:twenty-nine-core-assets', cache?.size === 29, cache?.size);
...
harness.stores.set('divina-bruxa-work13-v602-context-memory', new Map());
await harness.wait('activate');
check('activate:claims-clients', harness.counters().claimed === 1);
check('activate:keeps-v603', harness.stores.has(cacheName));
check('activate:deletes-v602-cache', !harness.stores.has('divina-bruxa-work13-v602-context-memory'));
for (const type of [
  ...
  'DIVINA_WORK13_CONTEXT_ACTIVE','DIVINA_WORK13_RESONANCE_ACTIVE','DIVINA_RELEASE_READY'
]) check(`activate:${type}`, harness.clientMessages.some(item => item.type === type && item.version === 603));
...
check('activate:resonance-base-v600', harness.clientMessages.some(item => item.type === 'DIVINA_WORK13_RESONANCE_ACTIVE' && item.base === 600));

const onlineNavigation = await harness.fetchEvent(new FakeRequest('https://divina.test/#daily', { mode:'navigate' }));
check('fetch:navigation-network', (await onlineNavigation.text()).includes('name="divina-work13" content="V603"'));
...
for (const [file, needle] of [
  ['app-v208.js?v=603-work13-resonance','divinaWork13Macro3V603'],
  ['cosmos-reality-resonance-v603.js?v=603-work13-resonance','COSMOS_REALITY_RESONANCE_CONTRACT_V603'],
  ['cosmos-reality-resonance-v603.css?v=603-work13-resonance','--db603-focus-x'],
...
check('message:status-version', harness.sourceMessages.some(item => item.type === 'WORK12_STATUS' && item.version === 603));
check('message:status-twenty-nine-assets', harness.sourceMessages.some(item => item.type === 'WORK12_STATUS' && item.core?.length === 29));
await harness.message({ type:'CLEAR_DIVINA_CACHES' });
check('message:clear-caches', !harness.stores.has(cacheName));
check('message:clear-confirmed', harness.sourceMessages.some(item => item.type === 'DIVINA_CACHES_CLEARED' && item.version === 603));

const failures = checks.filter(item => !item.pass);
console.log(JSON.stringify({
  release:'V603',
  work:'WORK13',
  macroStage:'3-of-10 / resonance-service-worker-runtime',
