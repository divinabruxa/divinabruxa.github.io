/* DIVINA BRUXA — WORK12 · QA SERVICE WORKER · V593 */
...
const sources=Object.freeze({
  index:read('./index.html'),app:read('./app-v208.js'),menu:read('./orbital-menu-v502.js'),menuStyles:read('./orbital-menu-v502.css'),
...
const coreAssets=[
  './index.html','./app-v208.js?v=593-work12-menu','./orbital-menu-v502.js?v=593-work12-menu','./orbital-menu-v502.css?v=593-work12-menu',
...
function createHarness({broken=null}={}){
  ...
  const paths=new Map([
    ...
    ['./app-v208.js?v=593-work12-menu',body('app',sources.app)],
  ...
  const fetch=async input=>{
    ...
    const routeSource={
      '/app-v208.js':sources.app,'/orbital-menu-v502.js':sources.menu,'/orbital-menu-v502.css':sources.menuStyles,
  ...
  vm.runInContext(source,vm.createContext({self,caches,fetch,Request:FakeRequest,URL,Object,Map,Set,Promise,Error,RegExp,console}),{filename:'sw-v593.js'});
...
const cacheName='divina-bruxa-work12-v593-menu';
...
check('install:fourteen-core-assets',cache?.size===14,cache?.size);
...
for(const component of ['index','app','menu','menu-styles','navigation','foundation','page-loader','universe','orb','renderer','journey','soul']){
...
harness.stores.set('divina-bruxa-work12-v592-navigation',new Map());
await harness.wait('activate');
check('activate:claims-clients',harness.counters().claimed===1);
check('activate:keeps-v593',harness.stores.has(cacheName));
check('activate:deletes-v592',!harness.stores.has('divina-bruxa-work12-v592-navigation'));
for(const type of ['DIVINA_WORK12_FOUNDATION_ACTIVE','DIVINA_WORK12_NAVIGATION_ACTIVE','DIVINA_WORK12_UNIVERSE_ACTIVE','DIVINA_WORK12_ORB_ACTIVE','DIVINA_WORK12_MENU_ACTIVE','DIVINA_RELEASE_READY']){
  check(`activate:${type}`,harness.clientMessages.some(item=>item.type===type&&item.version===593));
}

const onlineNavigation=await harness.fetchEvent(new FakeRequest('https://divina.test/#tarot',{mode:'navigate'}));
check('fetch:navigation-network',(await onlineNavigation.text()).includes('content="V593"'));
...
check('fetch:navigation-offline',(await offlineNavigation.text()).includes('content="V593"'));
harness.offline(false);

for(const [file,needle] of [
  ['app-v208.js?v=593-work12-menu','divinaWork12Macro5V593'],
...
check('message:status-version',harness.sourceMessages.some(item=>item.type==='WORK12_STATUS'&&item.version===593));
check('message:status-fourteen-assets',harness.sourceMessages.some(item=>item.type==='WORK12_STATUS'&&item.core?.length===14));
await harness.message({type:'CLEAR_DIVINA_CACHES'});
check('message:clear-caches',!harness.stores.has(cacheName));
check('message:clear-confirmed',harness.sourceMessages.some(item=>item.type==='DIVINA_CACHES_CLEARED'&&item.version===593));

const failures=checks.filter(item=>!item.pass);
console.log(JSON.stringify({
  release:'V593',work:'WORK12',macroStage:'5-of-10 / service-worker-runtime',
