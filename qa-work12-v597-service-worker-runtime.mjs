/* DIVINA BRUXA — WORK12 · QA SERVICE WORKER · V597 */
...
const sourceFiles=Object.freeze({
  index:'./index.html',app:'./app-v208.js',intelligence:'./experience-intelligence-v597.js',
...
const assets=Object.freeze({
  './index.html':'index',
  './app-v208.js?v=597-work12-intelligence':'app',
...
function createHarness({broken=null}={}){
  ...
  vm.runInContext(source,vm.createContext({self,caches,fetch,Request:FakeRequest,URL,Object,Map,Set,Promise,Error,RegExp,console}),{filename:'sw-v597.js'});
...
const cacheName='divina-bruxa-work12-v597-experience-intelligence';
...
check('install:twenty-four-core-assets',cache?.size===24,cache?.size);
...
harness.stores.set('divina-bruxa-work12-v596-reality-chambers',new Map());
await harness.wait('activate');
check('activate:claims-clients',harness.counters().claimed===1);
check('activate:keeps-v597',harness.stores.has(cacheName));
check('activate:deletes-v596',!harness.stores.has('divina-bruxa-work12-v596-reality-chambers'));
for(const type of ['DIVINA_WORK12_FOUNDATION_ACTIVE','DIVINA_WORK12_NAVIGATION_ACTIVE','DIVINA_WORK12_UNIVERSE_ACTIVE','DIVINA_WORK12_ORB_ACTIVE','DIVINA_WORK12_MENU_ACTIVE','DIVINA_WORK12_WHIT_ACTIVE','DIVINA_WORK12_RITUAL_ACTIVE','DIVINA_WORK12_CHAMBERS_ACTIVE','DIVINA_WORK12_INTELLIGENCE_ACTIVE','DIVINA_RELEASE_READY']){
  check(`activate:${type}`,harness.clientMessages.some(item=>item.type===type&&item.version===597));
}

const onlineNavigation=await harness.fetchEvent(new FakeRequest('https://divina.test/#tarot',{mode:'navigate'}));
check('fetch:navigation-network',(await onlineNavigation.text()).includes('content="V597"'));
...
check('fetch:navigation-offline',(await offlineNavigation.text()).includes('content="V597"'));
harness.offline(false);

for(const [file,needle] of [
  ['app-v208.js?v=597-work12-intelligence','divinaWork12Macro9V597'],
...
check('message:status-version',harness.sourceMessages.some(item=>item.type==='WORK12_STATUS'&&item.version===597));
check('message:status-twenty-four-assets',harness.sourceMessages.some(item=>item.type==='WORK12_STATUS'&&item.core?.length===24));
await harness.message({type:'CLEAR_DIVINA_CACHES'});
check('message:clear-caches',!harness.stores.has(cacheName));
check('message:clear-confirmed',harness.sourceMessages.some(item=>item.type==='DIVINA_CACHES_CLEARED'&&item.version===597));

const failures=checks.filter(item=>!item.pass);
console.log(JSON.stringify({release:'V597',work:'WORK12',macroStage:'9-of-10 / service-worker-runtime',state:failures.length?'FAIL':'PASS',passed:checks.length-failures.length,failed:failures.length,total:checks.length,failures:failures.map(({id,detail})=>({id,detail}))},null,2));
