/* DIVINA BRUXA — CONSTELAÇÃO DAS 30 SKINS V191
   A conta STAGING define propriedade; a troca visual continua atômica e sem recriar a aplicação. */
...
export class SkinsEngineV191 {
  ...
  render() {
    ...
    this.root.innerHTML = `
      <div class="skins-v191-shell" data-switching="${Boolean(this.pending)}">
  ...
  async switchTo(id) {
    ...
    this.announce(applied ? `${skin.name} agora vive em todas as suas Orbes.` : `A skin ${skin.name} não conseguiu atravessar o portal.`);
    return applied;
  }
  ...
  async sync(showFeedback = false) {
    ...
    if (result?.ok && result.body?.snapshot) {
      ...
      if (showFeedback) this.announce('Sua coleção foi restaurada pelo servidor STAGING.');
      return;
    }
