/* DIVINA BRUXA 2.0 — FLUIDEZ SUPREMA · SERVICE WORKER RUNTIME V585 */
...
check(stores.has('divina-bruxa-v585-shell'),'install:cache-v585');
const shellCache=stores.get('divina-bruxa-v585-shell');
check((await shellCache.keys()).length>=130,'install:atomic-shell-size',(await shellCache.keys()).length);
for(const file of [
...
const oldCache=await caches.open('divina-bruxa-v584-shell');
...
check(!stores.has('divina-bruxa-v584-shell'),'activate:remove-v584-cache');

let result=await message({type:'VERIFY_SHELL'});
check(result?.complete===true&&result?.atomic===true&&result?.version===585,'verify:complete-v585');
check(result?.ready===result?.total,'verify:complete-count');

online=false;
let response=await fetchEvent(new Request(`${scope}reality-intention-language-v585.js?v=585`));
check(response?.status===200&&(await response.text()).includes('REALITY_INTENTIONS_V585'),'offline:language-js-from-atomic-shell');
response=await fetchEvent(new Request(`${scope}reality-intention-language-v585.css?v=585`));
check(response?.status===200&&(await response.text()).includes('LINGUAGEM VIVA DAS REALIDADES V585'),'offline:language-css-from-atomic-shell');
...
response=await fetchEvent({
  url:`${scope}index.html?v585=1`,method:'GET',mode:'navigate',destination:'document',
  cache:'default',headers:new Headers()
});
check(response?.status===200&&(await response.text()).includes('app-v208.js?v=585-linguagem-viva'),'navigation:v585-shell-valid');
const contentCache=stores.get('divina-bruxa-v585-content');
check(Boolean(await contentCache?.match(new Request(`${scope}index.html?v585=1`))),'navigation:valid-shell-cached');

await shellCache.delete(new Request(`${scope}reality-intention-language-v585.js`));
result=await message({type:'VERIFY_SHELL'});
check(result?.complete===false&&result?.missing?.includes('./reality-intention-language-v585.js'),'repair:language-corruption-detected');
