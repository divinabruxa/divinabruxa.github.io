...
const css = await readFile(new URL('./diario-world-v619.css',import.meta.url),'utf8');
...
for (const [id,width,height,dpr] of profiles) {
  ...
  const worldWidth = width <= 430 ? width - 16 : Math.min(width - 28,1080);
  const paperWidth = Math.min(worldWidth,840);
  ...
  ok(worldWidth > 320 && worldWidth <= width, `${id}: câmara contida`);
  ok(paperWidth > 0 && paperWidth <= worldWidth, `${id}: papel cabe sem corte`);
...
ok(css.includes('width:min(calc(100% - 28px),1080px)'), 'largura máxima');
ok(css.includes('width:min(840px,100%)'), 'papel com limite legível');
...
ok(css.includes('scroll-snap-type:x mandatory') && css.includes('scroll-snap-align:center'), 'memórias em um passo por vez');
ok(css.includes('overscroll-behavior-inline:contain') && css.includes('-webkit-overflow-scrolling:touch'), 'timeline iOS contida');
ok(css.includes('content-visibility:auto') && css.includes('contain-intrinsic-size:680px'), 'Espelho abaixo da dobra adiado');
ok(css.includes('min-block-size:clamp(290px,48svh,450px)'), 'teclado mantém área de escrita');
...
console.log(`PASS ${checks}/${checks} — Câmara da Tinta Viva em 8 geometrias iPhone`);
