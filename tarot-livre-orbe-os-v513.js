/* DIVINA BRUXA — MACROETAPA V513 · ORBE OS / TAROT LIVRE GÊNESE
   ...
   master cósmico lossless + uma única cadência de animação: estrelas, plasma e
...
class OrbRealityEngineV513 {
  constructor({ cosmos, effects, stage, orb, card }) {
    ...
    this.orb = orb;
    this.card = card;
    this.world = stage.closest('.tl513');
    ...
    this.effectsScale = constrained() ? 1 : (innerWidth < 700 ? 1.5 : 1.35);
  ...
  syncPalette(force = false) {
    ...
    this.world.style.setProperty('--tl513-skin-accent', colorCss(accent));
    this.world.style.setProperty('--tl513-skin-light', colorCss(light));
    this.world.style.setProperty('--tl513-skin-gold', colorCss(gold));
    this.world.style.setProperty('--tl513-skin-hot', colorCss(this.palette.hot));
  ...
  buildParticleSprites() {
    ...
    this.particleSprites = {
      accent:make(this.palette.hot, this.palette.accent),
      gold:make(this.palette.hot, this.palette.gold),
      white:make({ r:255, g:253, b:242 }, this.palette.light)
    };
  }
  ...
  useCanvasFallback() {
    ...
    this.world.dataset.canvasArchitecture = 'lossless-master-plus-retina-live-layer';
  }
  ...
  /* Canvas V513: the lossless master carries microscopic detail while this
     Retina layer gives it breath. No galaxy texture is rebuilt per frame. */
  drawCanvasFlame(context, timestamp) {
    ...
    const cx = card.x * this.width;
    const cy = card.cssY * this.height;
    const amplitude = clamp(this.width * 0.12, 34, 94);
    const breath = 0.93 + Math.sin(time * 1.92) * 0.07;
    const awakening = clamp((this.energy - 0.72) / 1.12, 0, 1);

    const ribbonPoint = (progress, phase = 0, spread = 1) => {
      const eased = progress * progress * (3 - 2 * progress);
      const envelope = Math.sin(Math.PI * progress);
      const primary = Math.sin(progress * Math.PI * 2.08 - time * 0.76 + phase) * amplitude * envelope;
      const filament = Math.sin(progress * Math.PI * 10.6 + time * 1.12 - phase * 1.64)
        * amplitude * 0.135 * envelope * spread;
      return {
        x:ox + (cx - ox) * eased + primary + filament,
        y:oy + (cy - oy) * eased
          + Math.cos(progress * Math.PI * 4.1 - time * 0.66 + phase) * 7.2 * envelope
      };
    };
    const trace = (phase = 0, spread = 1) => {
      context.beginPath();
      for (let step = 0; step <= 40; step += 1) {
        const point = ribbonPoint(step / 40, phase, spread);
        if (step === 0) context.moveTo(point.x, point.y);
        else context.lineTo(point.x, point.y);
      }
    };
    ...
    context.lineJoin = 'round';
    context.globalCompositeOperation = 'source-over';
    context.globalAlpha = 0.15 + awakening * 0.08;
    context.strokeStyle = colorCss(palette.accent, 0.72);
    context.shadowColor = colorCss(palette.accent);
    context.shadowBlur = constrained() ? 19 : 30;
    context.lineWidth = (constrained() ? 44 : 58) * breath;
    trace(0, 0.25);
    context.stroke();

    context.globalAlpha = 0.12 + awakening * 0.07;
    context.strokeStyle = colorCss(mixColor(palette.accent, palette.light, 0.34), 0.8);
    context.shadowBlur = constrained() ? 12 : 20;
    context.lineWidth = (constrained() ? 24 : 32) * breath;
    trace(0.08, 0.46);
    context.stroke();

    context.globalCompositeOperation = 'lighter';
    const strands = [
      [-2.82, palette.accent, 4.4, 0.56],
      [-1.86, mixColor(palette.accent, palette.light, 0.2), 5.1, 0.58],
      [-0.9, palette.light, 3.8, 0.62],
      [0.08, mixColor(palette.accent, palette.gold, 0.32), 5.6, 0.68],
      [1.08, palette.gold, 4.2, 0.66],
      [2.04, palette.ember, 3.5, 0.62],
      [2.92, palette.hot, 2.2, 0.72]
    ];
    for (const [phase, color, width, alpha] of strands) {
      context.globalAlpha = alpha + awakening * 0.08;
      context.strokeStyle = colorCss(color);
      context.shadowColor = colorCss(color);
      context.shadowBlur = constrained() ? 8 : 13;
  ...
  drawBirthWave(context, timestamp) {
    ...
    context.strokeStyle = colorCss(palette.accent, 0.28 + pulse * 0.08);
    context.lineWidth = constrained() ? 20 : 27;
    traceTail();
    context.stroke();
    context.shadowColor = colorCss(mixColor(palette.accent, palette.light, 0.26));
    context.shadowBlur = 14;
    context.strokeStyle = colorCss(mixColor(palette.accent, palette.light, 0.22), 0.62 + pulse * 0.08);
    context.lineWidth = constrained() ? 6.4 : 9.2;
    traceTail();
    context.stroke();
    context.globalCompositeOperation = 'lighter';
    context.shadowColor = colorCss(palette.gold);
    context.shadowBlur = 10;
    context.strokeStyle = colorCss(palette.hot, 0.72 + pulse * 0.08);
    context.lineWidth = constrained() ? 1.45 : 2.1;
    traceTail();
    context.stroke();

    const flare = context.createRadialGradient(x, y, 0, x, y, radius * 2.8);
    flare.addColorStop(0, colorCss(palette.hot, 0.7 - progress * 0.18));
    flare.addColorStop(0.15, colorCss(palette.gold, 0.64 - progress * 0.24));
    flare.addColorStop(0.45, colorCss(palette.accent, 0.43 - progress * 0.17));
    flare.addColorStop(1, colorCss(palette.accent, 0));
...
export class TarotLivreOrbOSV513 {
  constructor(root, {
    storage = store,
    ...
    if (!orbCore?.claim || !orbCore?.orb) {
      throw new Error('A Orbe Suprema V501 precisa despertar antes do Tarot Livre V513.');
    }
    ...
    this.root.innerHTML = '';
    this.root.dataset.tarotWorld = 'orbe-os-v513';
    ...
    this.world.dataset.flameTexture = 'volumetric-star-plasma-v513';
    this.world.dataset.birthCorona = 'card-edge';
    this.world.dataset.birthJourney = 'orb-to-card';
    this.world.dataset.birthEasing = 'smoothstep-v513';
    this.world.dataset.responsiveAltar = 'v513';
    ...
    this.world.dataset.universeBackdrop = 'tarot-cosmos-master-v513';
    this.world.dataset.masterQuality = 'lossless-1536x2304';
    this.world.dataset.skinReactiveUniverse = '30-palettes';
    this.world.dataset.effectsResolution = 'retina-adaptive';
    this.world.dataset.frameArchitecture = 'single-raf';
    this.world.dataset.particleArchitecture = 'palette-sprite-atlas';
    this.render(false);
    this.fire = new OrbRealityEngineV513({
    ...
    document.documentElement.dataset.tarotLivre = 'v513';
    const readiness = Object.freeze({
      version: VERSION,
      engine: 'OrbRealityEngineV513',
  ...
  build() {
    this.root.innerHTML = `
      <div class="tl513" data-phase="idle" data-engine="pending">
        <header class="tl513__header">
          <div class="tl513__title">
            <h2>Tarot Livre</h2>
            <span class="tl513__title-star" aria-hidden="true"><i></i><b></b><i></i></span>
          </div>

          <a class="tl513__mesa" href="#spreads" data-go="spreads" aria-label="Abrir Tiragens e Mesa Real">
        ...
        <section class="tl513__stage" data-stage data-universe="alive" aria-label="Tarot Livre — universo vivo nascido da Orbe">
          <canvas class="tl513__canvas tl513__cosmos" data-cosmos aria-hidden="true"></canvas>
          <canvas class="tl513__canvas tl513__effects" data-effects aria-hidden="true"></canvas>

          <div class="tl513__portal-crown" aria-hidden="true"><i></i><i></i><i></i></div>

          <div class="tl513__card-zone" data-card-zone tabindex="0"
               aria-label="Carta atual. Use as setas para navegar pelas cartas reveladas.">
            <div class="tl513__card" data-current-card data-empty="true">
              <canvas class="tl513__card-back" data-card-back aria-hidden="true"></canvas>
            </div>
          </div>

          <nav class="tl513__card-nav" data-card-nav aria-label="Navegar pelas cartas reveladas">
          ...
          <div class="tl513__orb-host" data-tarot-orb-host>
            <span class="tl513__orb-aura" aria-hidden="true"><i></i><i></i><i></i></span>
            <span class="tl513__orb-flare" aria-hidden="true"></span>
          </div>

          <p class="tl513__counter" aria-label="Cartas reveladas">
            <b data-count>0</b><span>/78</span>
          </p>
          <p class="tl513__signal" data-signal aria-live="polite">Toque na Orbe para revelar seu Tarot.</p>
        </section>

        <div class="tl513__toolbar" aria-label="Controles do Tarot Livre">
        ...
        <section class="tl513__constellation" aria-labelledby="tl513ConstellationTitle">
          <header>
            <div>
              <small>SEU CÍRCULO</small>
              <h3 id="tl513ConstellationTitle">Cartas reveladas</h3>
            </div>
            <span data-remaining>78 aguardam</span>
          </header>
          <div class="tl513__grid" data-grid role="grid"
               aria-label="Cartas reveladas, seis por fileira" aria-colcount="6"></div>
          <p class="tl513__empty-grid" data-empty-grid>Nenhuma carta foi revelada ainda.</p>
        </section>

        <p class="tl513__sr" data-live role="status" aria-live="polite" aria-atomic="true"></p>
      </div>`;

    this.world = this.root.querySelector('.tl513');
  ...
  enter() {
    ...
    this.orb.dataset.tarotReveal = 'v513';
  ...
  leave(nextRoute = 'home') {
    ...
    document.body?.classList.remove('db513-tarot-birthing');
    this.fire?.setActive(false);
    ...
    this.orbCore.settleRoute?.(nextRoute, 'tarot-leave-v513');
    return true;
  }
  ...
  setPhase(phase) {
    ...
    document.body?.classList.toggle('db513-tarot-birthing', this.active && ritualVisible);
  ...
  async prepareCardContent(index) {
    ...
    cradle.innerHTML = `${cardImageMarkup(card, {
      ...
      priority: 'high'
    })}<div class="tl513__card-name"><b>${escapeText(card.name)}</b><small>${index + 1} / ${this.state.revealed.length}</small></div>`;
  ...
  async transitionTo(index, { onBirth } = {}) {
    ...
    veil.className = 'tl513__birth-veil';
    ...
    const journey = this.card.animate([
      ...
      duration,
      easing:'cubic-bezier(.22,.58,.24,1)',
      fill:'both'
    });
    ...
    try {
      ...
      globalThis.dispatchEvent?.(new CustomEvent('tarot:rebirth-revealed', {
        detail: {
          ...
          engine: 'living-universe-flame-v513'
        }
      }));
  ...
  render(animateCurrent = false) {
    ...
    if (!revealed) {
      ...
      this.card.innerHTML = '<canvas class="tl513__card-back" data-card-back aria-hidden="true"></canvas>';
  ...
  status() {
    return Object.freeze({
      ...
      organicCanvasFire: this.world.dataset.flameTexture === 'volumetric-star-plasma-v513',
      plasmaFilaments: true,
      cardEdgeBurst: this.world.dataset.birthCorona === 'card-edge',
      responsiveAltar: this.world.dataset.responsiveAltar === 'v513',
      ...
      optimizedCosmicBackdrop: this.world.dataset.universeBackdrop === 'tarot-cosmos-master-v513',
      losslessRetinaMaster: this.world.dataset.masterQuality === 'lossless-1536x2304',
      skinReactiveUniverse: this.world.dataset.skinReactiveUniverse === '30-palettes',
      retinaAdaptiveEffects: this.world.dataset.effectsResolution === 'retina-adaptive',
      spriteAtlasParticles: this.world.dataset.particleArchitecture === 'palette-sprite-atlas',
      smoothBirthJourney: this.world.dataset.birthEasing === 'smoothstep-v513',
  ...
  destroy() {
    ...
    this.fire?.destroy();
    if (globalThis.divinaTarotLivreV513 === this) delete globalThis.divinaTarotLivreV513;
