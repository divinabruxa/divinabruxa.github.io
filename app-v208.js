...
// A matéria existente não é reescrita. Uma única máquina coordena quando as
// quatro realidades podem aparecer, sempre abaixo da mesma Orbe persistente.
const realityChambers = safely('WORK12 · Realidades como Câmaras V596', () =>
  createRealityChambersV596({
    orbCore:supremeOrb,
    universe:livingUniverse,
    foundation:work12Foundation,
    presence:whitLivingPresence,
...
navigator.serviceWorker?.addEventListener('message', event => {
  if (event.data?.type === 'DIVINA_WORK13_PREMIUM_WORLD_ACTIVE') {
    document.documentElement.dataset.work13PremiumWorldWorker = `v${event.data.version || 623}`;
    return;
  }
  ...
  if (event.data?.type === 'DIVINA_WORK13_DAILY_WORLD_ACTIVE') {
    document.documentElement.dataset.work13DailyWorldWorker = `v${event.data.version || 616}`;
    return;
  }
  ...
  if (event.data?.type === 'DIVINA_WORK13_ENTRY_ACTIVE') {
    document.documentElement.dataset.work13EntryWorker = `v${event.data.version || 613}-global-menu`;
    return;
  }
  if (event.data?.type === 'DIVINA_WORK13_PENTAGRAM_MENU_ACTIVE') {
    document.documentElement.dataset.work13MenuWorker = `v${event.data.version || 613}-living-global-menu`;
    return;
  }
  ...
  if (event.data?.type === 'DIVINA_WORK13_CONTEXT_ACTIVE') {
    document.documentElement.dataset.work13ContextWorker = `v${event.data.contextVersion || event.data.version || 602}`;
    return;
  }
  ...
  if (event.data?.type === 'DIVINA_WORK12_CHAMBERS_ACTIVE') {
    document.documentElement.dataset.work12ChambersWorker = `v${event.data.version || 596}`;
    work12Foundation?.audit?.('chambers-worker-active');
    return;
  }
...
if ('serviceWorker' in navigator && !window.__divinaSWBootstrap) {
  window.__divinaSWBootstrap = 'v623-premium-sala-das-chaves-app';
  addEventListener('load', () => {
    navigator.serviceWorker.register('./sw.js?v=623-premium-sala-das-chaves', { updateViaCache:'none' })
      .then(async registration => {
        document.documentElement.dataset.releaseEpoch = 'v623-premium-sala-das-chaves';
        await registration.update().catch(() => null);
        registration.waiting?.postMessage?.({ type:'SKIP_WAITING' });
        console.info('[Divina] WORK13 V623 · Sala das Chaves; V622 e WORK12 V600 protegidos');
      })
...
document.documentElement.dataset.work12Macro = '8-realidades-como-camaras';
document.documentElement.dataset.work12Chambers = 'v596';
window.divinaWork12Macro8V596 = Object.freeze({
  version:596,
  base:'V595',
  work:'WORK12',
  stage:'realidades-como-camaras',
  ...
  coordinator:work12Foundation,
  status:() => {
    const chambers = realityChambers?.status?.() || null;
    const chamberAudit = realityChambers?.audit?.() || null;
    const ritual = readingRitual?.status?.() || null;
    const presence = whitLivingPresence?.status?.() || null;
    const orb = supremeOrb?.snapshot?.() || null;
    const journey = orbIOSJourney?.status?.() || null;
    return Object.freeze({
      ...
      orb,
      journey,
      chamberRoutes:Object.freeze(['school','journal','consultations','store']),
...
window.divinaFluidezSupremaV596 = window.divinaWork12Macro8V596;
const skinPerformanceCore = safely('Skins, desempenho e acabamento V518', () =>
  installSkinPerformanceCoreV518()
);
// A inteligência da experiência não interpreta pessoas. Ela coordena somente
// rota, viagem, rito, foco e visibilidade para o movimento sempre vir primeiro.
const experienceIntelligence = safely('WORK12 · Inteligência da Experiência V597', () =>
  createExperienceIntelligenceV597({
    universe:livingUniverse,
    foundation:work12Foundation,
    orbCore:supremeOrb,
    soul:whitOrbSoul,
    ...
    ritual:readingRitual,
    chambers:realityChambers,
    performance:skinPerformanceCore
  })
);
window.orbe.experienceIntelligence = experienceIntelligence;
window.orbe.intelligence = experienceIntelligence;
document.documentElement.dataset.work12Macro = '9-inteligencia-da-experiencia';
document.documentElement.dataset.work12Intelligence = 'v597';
window.divinaWork12Macro9V597 = Object.freeze({
  version:597,
  base:'V596',
  work:'WORK12',
  stage:'inteligencia-da-experiencia',
  ...
  status:() => {
    ...
    const intelligenceAudit = experienceIntelligence?.audit?.() || null;
    const performance = skinPerformanceCore?.status?.() || null;
    const presence = whitLivingPresence?.status?.() || null;
    const orb = supremeOrb?.snapshot?.() || null;
    const journey = orbIOSJourney?.status?.() || null;
    const universe = livingUniverse?.status?.() || null;
    return Object.freeze({
      ...
      journey,
      universe,
      contextModel:'route-and-public-state-only',
      ...
      newAnimationLoops:0,
      newMutationObservers:0,
      recoveredZeroByteApplication:true
    });
  }
});
...
// A V603 nao acrescenta materia ao cosmos. Ela reutiliza o mesmo veu do
// universo V524 e recebe da memoria somente rota e fase publicas. Assim cada
// realidade ganha uma respiracao propria sem tocar em conteudo ou chamar Whit.
const cosmosRealityResonance = safely('WORK13 · Ressonância Viva das Realidades V603', () =>
  createCosmosRealityResonanceV603({
    universe:livingUniverse,
    contextMemory:cosmosContextMemory
  })
);
window.orbe.resonance = cosmosRealityResonance;
window.orbe.realityResonance = cosmosRealityResonance;
document.documentElement.dataset.work13 = 'cosmos-vivo';
document.documentElement.dataset.work13Macro = '3-reality-resonance';
window.divinaWork13Macro3V603 = Object.freeze({
  version:603,
  base:'V602-context-memory-on-WORK12-V600-frozen-by-V601',
  work:'WORK13',
  stage:'universo-reage-a-cada-realidade',
  ...
  status:() => {
    ...
    const audit = cosmosRealityResonance?.audit?.() || null;
    const context = cosmosContextMemory?.status?.() || null;
    const universe = livingUniverse?.status?.() || null;
    const orb = supremeOrb?.snapshot?.() || null;
    const journey = orbIOSJourney?.status?.() || null;
    return Object.freeze({
      ...
      orb,
      journey,
      existingUniverseVeilOnly:true,
      respectsCurrentSkin:true,
      automaticNavigation:false,
      automaticWhitSpeech:false,
      onePhysicalOrb:orb?.oneLivingOrb === true,
      sameEntity:orb?.entityPreserved === true,
      oneUniverseCanvas:universe?.oneUniverseCanvas === true,
      travelerCopies:Number(journey?.travelerCopies || 0),
      teleport:journey?.teleportFallback === true,
      flicker:journey?.flicker === true,
      privateContentReads:0,
...
const cosmosEntryIntention = safely('WORK13 · Menu Global Vivo e Persistente V613', () =>
...
const cartaDoDiaWorld = safely('WORK13 · Carta do Dia · Santuário da Aurora V616', () =>
  createCartaDoDiaWorldV616({ orbCore:supremeOrb })
);
...
const whitWorld = safely('WORK13 · Whit · Presença Entre Mundos V620', () =>
  createWhitWorldV620({
    livingPresence:whitLivingPresence,
    silenceTiming:whitSilenceTiming,
    soul:whitOrbSoul
  })
);
...
const premiumWorld = safely('WORK13 · Premium · Sala das Chaves V623', () =>
  createPremiumWorldV623({ livingCommerce:livingCommercePath })
);
...
window.divinaWork13CartaDoDiaWorldV616 = Object.freeze({
  version:616,
  work:'WORK13',
  stage:'renovacao-dos-mundos-3-carta-do-dia',
  reality:'daily',
  universe:'santuario-da-aurora',
  world:cartaDoDiaWorld,
  status:() => cartaDoDiaWorld?.status?.() || null,
  work14:false
});
window.divinaWork13TiragensSoulV610 = Object.freeze({
  version:610,
  work:'WORK13',
...
window.divinaWork13DiarioSoulV610 = Object.freeze({
  version:610,
  work:'WORK13',
...
window.divinaWork13WhitWorldV620 = Object.freeze({
  version:620,
  work:'WORK13',
  stage:'renovacao-dos-mundos-7-whit',
  reality:'ai',
  universe:'presenca-entre-mundos',
  base:window.divinaWork13DiarioWorldV619,
  world:whitWorld,
...
window.divinaWork13PremiumWorldV623 = Object.freeze({
  version:623,
  work:'WORK13',
  stage:'renovacao-dos-mundos-10-premium',
  reality:'subscriptions',
  universe:'sala-das-chaves',
  base:window.divinaWork13LojaWorldV622,
  world:premiumWorld,
  status:() => premiumWorld?.status?.() || null,
  work14:false
});
document.documentElement.dataset.work13 = 'cosmos-vivo';
...
window.divinaWork13MenuGlobalVivoV613 = Object.freeze({
  version:613,
  work:'WORK13',
  correction:'menu-global-vivo-persistente',
  base:window.divinaWork13PentagramMenuV612,
  entryIntention:cosmosEntryIntention,
  status:() => Object.freeze({
    release:'V613',
...
document.documentElement.dataset.work13Macro = 'menu-global-vivo-persistente';
...
document.documentElement.dataset.work13Macro = 'premium-sala-das-chaves';
...
window.divinaCosmosVivo = window.divinaWork13PremiumWorldV623;
