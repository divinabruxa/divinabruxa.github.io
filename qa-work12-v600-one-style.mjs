/* DIVINA BRUXA — WORK12 · QA ESTRUTURAL · FECHAMENTO VIVO V600 */
...
check('release:meta-v600',index.includes('name="divina-fluidity-release" content="V600"'));
check('release:work12-v600',index.includes('name="divina-work12" content="V600"'));
check('release:live-audit-v600',index.includes('name="divina-live-audit" content="V600"'));
check('release:macro-ten',index.includes('data-macroetapa="work12-10-fechamento-vivo"'));
...
check('release:one-style-identity',occurrences(index,/id="divinaWork12FinalContinuityV598"/g)===1,occurrences(index,/id="divinaWork12FinalContinuityV598"/g));
check('release:no-shadow-style-identity',!index.includes('id="divinaWork12FinalContinuityV599"'));
check('release:runtime-style-identity-match',final.includes("const STYLE_ID = 'divinaWork12FinalContinuityV598'"));
check('release:worker-query',index.includes("register('./sw.js?v=600'"));
check('release:inline-bootstrap',index.includes("__divinaSWBootstrap='v600-work12-inline'"));
check('release:inline-ready',index.includes("work12Boot='ready-v600'"));
check('release:inline-recovery',index.includes("work12Boot='released-v600'"));
check('release:inline-recovery-source',index.includes("source:'work12-one-style-v600'"));
check('release:app-bootstrap',app.includes("__divinaSWBootstrap = 'v598-work12-app'"));
check('release:worker-version',worker.includes('const VERSION = 600;'));
...
check('worker:cache-v600',worker.includes('divina-bruxa-work12-v600-one-style'));
...
const coordinates=await import(`${pathToFileURL(path.join(root,'universe-coordinate-law-v583.js')).href}?qa=v600-one-style`);
...
const vector=coordinates.universeJourneyVectorV583('home','tarot',600);
...
console.log(JSON.stringify({
  release:'V600',work:'WORK12',macroStage:'10-of-10 / Fechamento Vivo',
