/* DIVINA BRUXA — WORK12 · QA BOOT RECUPERÁVEL · V596 */
...
function harness(){
  ...
  vm.runInContext(bootScript,vm.createContext(context),{filename:'work12-inline-boot-v596.js'});
...
check('boot:app-v596',indexSource.includes('app-v208.js?v=596-work12-chambers'));
check('boot:worker-v596',indexSource.includes("register('./sw.js?v=596'"));
...
check('boot:watchdog-v596',watchdog.root.dataset.work12Boot==='released-v596'&&watchdog.root.dataset.work12BootReason==='boot-watchdog');
check('boot:watchdog-event-v596',watchdog.events.some(event=>event.type==='divina:work12-boot-release'&&event.detail.version===596));
...
check('boot:healthy-never-bypassed',healthy.root.dataset.work12Boot==='ready-v596'&&!healthy.home.classList.contains('active'));

const failures=checks.filter(item=>!item.pass);
console.log(JSON.stringify({release:'V596',work:'WORK12',macroStage:'8-of-10 / boot-runtime',state:failures.length?'FAIL':'PASS',passed:checks.length-failures.length,failed:failures.length,total:checks.length,failures:failures.map(({id,detail})=>({id,detail}))},null,2));
