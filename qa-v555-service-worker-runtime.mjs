...
check(cacheStores.has('divina-bruxa-v555-shell'),'install-cache-v555');
check((await cacheStores.get('divina-bruxa-v555-shell').keys()).length>=123,'install-shell-atomico');
check(!cacheStores.has('divina-bruxa-v555-offline-core'),'install-sem-cache-opcional');
...
check(!cacheStores.has('divina-bruxa-v555-premium-static'),'premium-sem-cache-na-negacao');
...
online=true;
const shellCache=cacheStores.get('divina-bruxa-v555-shell');
await shellCache.delete(new Request(`${scope}security-privacy-core-v547.js`));
result=await message({type:'VERIFY_SHELL'});
check(result?.complete===false&&result?.missing?.includes('./security-privacy-core-v547.js'),'corrupcao-v555-detectada');
result=await message({type:'REPAIR_SHELL'});
check(result?.complete===true,'corrupcao-v555-reparada');

result=await message({type:'CLEAR_OPTIONAL_OFFLINE'});
check(result?.ok===true,'cache-opcional-limpo');
check(!cacheStores.has('divina-bruxa-v555-offline-core')&&!cacheStores.has('divina-bruxa-v555-premium-static'),'stores-opcionais-removidas');
check(result?.authorityDataRemoved===false,'nao-inventa-exclusao-privada');

const contentCache=await caches.open('divina-bruxa-v555-content');contentCache.failNextPut=true;
...
console.log(`V555 Service Worker runtime: ${passed}/${total} verificações aprovadas.`);
