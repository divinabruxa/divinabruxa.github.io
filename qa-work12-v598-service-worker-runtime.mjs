/* DIVINA BRUXA — WORK12 · QA SERVICE WORKER · V598 */
...
const assets=Object.freeze({
  ...
  './work12-final-continuity-v598.css?v=598-work12-final':'final-styles',
...
function createHarness({broken=null}={}){
  ...
  vm.runInContext(source,vm.createContext({self,caches,fetch,Request:FakeRequest,URL,Object,Map,Set,Promise,Error,RegExp,console}),{filename:'sw-v598.js'});
...
const cacheName='divina-bruxa-work12-v598-final-fluidity';
...
harness.stores.set('divina-bruxa-work12-v597-experience-intelligence',new Map());
await harness.wait('activate');
check('activate:claims-clients',harness.counters().claimed===1);
check('activate:keeps-v598',harness.stores.has(cacheName));
check('activate:deletes-v597',!harness.stores.has('divina-bruxa-work12-v597-experience-intelligence'));
for(const type of ['DIVINA_WORK12_FOUNDATION_ACTIVE','DIVINA_WORK12_NAVIGATION_ACTIVE','DIVINA_WORK12_UNIVERSE_ACTIVE','DIVINA_WORK12_ORB_ACTIVE','DIVINA_WORK12_MENU_ACTIVE','DIVINA_WORK12_WHIT_ACTIVE','DIVINA_WORK12_RITUAL_ACTIVE','DIVINA_WORK12_CHAMBERS_ACTIVE','DIVINA_WORK12_INTELLIGENCE_ACTIVE','DIVINA_WORK12_FINAL_ACTIVE','DIVINA_RELEASE_READY']){
  check(`activate:${type}`,harness.clientMessages.some(item=>item.type===type&&item.version===598));
}

const onlineNavigation=await harness.fetchEvent(new FakeRequest('https://divina.test/#tarot',{mode:'navigate'}));
check('fetch:navigation-network',(await onlineNavigation.text()).includes('content="V598"'));
...
check('fetch:navigation-offline',(await offlineNavigation.text()).includes('content="V598"'));
...
for(const [file,needle] of [
  ...
  ['work12-final-continuity-v598.css?v=598-work12-final','body[data-screen="home"] .app-header'],
...
check('message:status-version',harness.sourceMessages.some(item=>item.type==='WORK12_STATUS'&&item.version===598));
...
check('message:clear-confirmed',harness.sourceMessages.some(item=>item.type==='DIVINA_CACHES_CLEARED'&&item.version===598));

const failures=checks.filter(item=>!item.pass);
console.log(JSON.stringify({release:'V598',work:'WORK12',macroStage:'10-of-10 / service-worker-runtime',state:failures.length?'FAIL':'PASS',passed:checks.length-failures.length,failed:failures.length,total:checks.length,failures:failures.map(({id,detail})=>({id,detail}))},null,2));
