/* DIVINA BRUXA — WORK12 · QA BOOT RECUPERÁVEL · V595 */
...
function harness(){
  const timers=new Map();const listeners=new Map();const events=[];let timerId=0;
  ...
  vm.runInContext(bootScript,vm.createContext(context),{filename:'work12-inline-boot-v595.js'});
...
check('boot:app-v595',indexSource.includes('app-v208.js?v=595-work12-ritual'));
check('boot:worker-v595',indexSource.includes("register('./sw.js?v=595'"));
...
check('boot:watchdog-v595',watchdog.root.dataset.work12Boot==='released-v595'&&watchdog.root.dataset.work12BootReason==='boot-watchdog');
check('boot:watchdog-event-v595',watchdog.events.some(event=>event.type==='divina:work12-boot-release'&&event.detail.version===595));
...
check('boot:healthy-never-bypassed',healthy.root.dataset.work12Boot==='ready-v595'&&!healthy.home.classList.contains('active'));

const failures=checks.filter(item=>!item.pass);
console.log(JSON.stringify({
  release:'V595',work:'WORK12',macroStage:'7-of-10 / boot-runtime',
