/* DIVINA BRUXA — MACROETAPA V514 · ORBE OS / TAROT LIVRE GÊNESE
   A única Orbe Suprema V501 deixa a Home, ocupa o altar e materializa cada
   carta em uma viagem física sincronizada com fogo celestial volumétrico.
   O universo profundo e as chamas compartilham uma única cadência. Não existe
   linha elétrica entre Orbe e carta: o fogo é uma massa de volumes, línguas,
   estrelas e brasas. 78 diretas, zero repetição e seis por fileira permanecem
   leis imutáveis. */
...
class CelestialFireUniverseEngineV514 {
  constructor({ cosmos, effects, stage, orb, card }) {
    ...
    this.orb = orb;
    this.card = card;
    this.world = stage.closest('.tl514');
  ...
  syncPalette(force = false) {
    ...
    this.world.style.setProperty('--tl514-skin-accent', colorCss(accent));
    this.world.style.setProperty('--tl514-skin-light', colorCss(light));
    this.world.style.setProperty('--tl514-skin-gold', colorCss(gold));
    this.world.style.setProperty('--tl514-skin-hot', colorCss(this.palette.hot));
  ...
  initializeRenderer() {
    // V514 deliberately uses a sprite-volume field. The former WebGL shader
  ...
  /* V514 Celestial Fire Volume Field. Every visible part is an overlapping
...
export class TarotLivreOrbOSV514 {
  constructor(root, {
    storage = store,
    ...
    if (!orbCore?.claim || !orbCore?.orb) {
      throw new Error('A Orbe Suprema V501 precisa despertar antes do Tarot Livre V514.');
    }
    ...
    this.root.innerHTML = '';
    this.root.dataset.tarotWorld = 'orbe-os-v514';
    ...
    this.world.dataset.flameTexture = 'celestial-fire-volume-v514';
    this.world.dataset.flameArchitecture = 'sprite-billows-and-tongues';
    ...
    this.world.dataset.birthEasing = 'smoothstep-v514';
    this.world.dataset.responsiveAltar = 'v514';
    ...
    this.world.dataset.universeBackdrop = 'tarot-cosmos-deep-v514';
    ...
    this.render(false);
    this.fire = new CelestialFireUniverseEngineV514({
    ...
    document.documentElement.dataset.tarotLivre = 'v514';
    const readiness = Object.freeze({
      version: VERSION,
      engine: 'CelestialFireUniverseEngineV514',
  ...
  build() {
    this.root.innerHTML = `
      <div class="tl514" data-phase="idle" data-engine="pending">
        <header class="tl514__header">
          <div class="tl514__title">
            <h2>Tarot Livre</h2>
            <span class="tl514__title-star" aria-hidden="true"><i></i><b></b><i></i></span>
          </div>

          <a class="tl514__mesa" href="#spreads" data-go="spreads" aria-label="Abrir Tiragens e Mesa Real">
        ...
        <section class="tl514__stage" data-stage data-universe="alive" aria-label="Tarot Livre — universo vivo nascido da Orbe">
          <div class="tl514__deep-space" aria-hidden="true"></div>
          <canvas class="tl514__canvas tl514__cosmos" data-cosmos aria-hidden="true"></canvas>
          <canvas class="tl514__canvas tl514__effects" data-effects aria-hidden="true"></canvas>

          <div class="tl514__portal-crown" aria-hidden="true"><i></i><i></i><i></i></div>

          <div class="tl514__card-zone" data-card-zone tabindex="0"
               aria-label="Carta atual. Use as setas para navegar pelas cartas reveladas.">
            <div class="tl514__card" data-current-card data-empty="true">
              <canvas class="tl514__card-back" data-card-back aria-hidden="true"></canvas>
            </div>
          </div>

          <nav class="tl514__card-nav" data-card-nav aria-label="Navegar pelas cartas reveladas">
          ...
          <div class="tl514__orb-host" data-tarot-orb-host>
            <span class="tl514__orb-aura" aria-hidden="true"><i></i><i></i><i></i></span>
            <span class="tl514__orb-flare" aria-hidden="true"></span>
          </div>

          <p class="tl514__counter" aria-label="Cartas reveladas">
            <b data-count>0</b><span>/78</span>
          </p>
          <p class="tl514__signal" data-signal aria-live="polite">Toque na Orbe para revelar seu Tarot.</p>
        </section>

        <div class="tl514__toolbar" aria-label="Controles do Tarot Livre">
        ...
        <section class="tl514__constellation" aria-labelledby="tl514ConstellationTitle">
          <header>
            <div>
              <small>SEU CÍRCULO</small>
              <h3 id="tl514ConstellationTitle">Cartas reveladas</h3>
            </div>
            <span data-remaining>78 aguardam</span>
          </header>
          <div class="tl514__grid" data-grid role="grid"
               aria-label="Cartas reveladas, seis por fileira" aria-colcount="6"></div>
          <p class="tl514__empty-grid" data-empty-grid>Nenhuma carta foi revelada ainda.</p>
        </section>

        <p class="tl514__sr" data-live role="status" aria-live="polite" aria-atomic="true"></p>
      </div>`;

    this.world = this.root.querySelector('.tl514');
  ...
  enter() {
    ...
    this.orb.dataset.tarotReveal = 'v514';
  ...
  leave(nextRoute = 'home') {
    ...
    document.body?.classList.remove('db514-tarot-birthing');
    this.fire?.setActive(false);
    ...
    this.orbCore.settleRoute?.(nextRoute, 'tarot-leave-v514');
    return true;
  }
  ...
  setPhase(phase) {
    ...
    document.body?.classList.toggle('db514-tarot-birthing', this.active && ritualVisible);
  ...
  async prepareCardContent(index) {
    ...
    cradle.innerHTML = `${cardImageMarkup(card, {
      ...
      priority: 'high'
    })}<div class="tl514__card-name"><b>${escapeText(card.name)}</b><small>${index + 1} / ${this.state.revealed.length}</small></div>`;
  ...
  async transitionTo(index, { onBirth } = {}) {
    ...
    veil.className = 'tl514__birth-veil';
    ...
    try {
      ...
      globalThis.dispatchEvent?.(new CustomEvent('tarot:rebirth-revealed', {
        detail: {
          ...
          engine: 'living-universe-flame-v514'
        }
      }));
  ...
  render(animateCurrent = false) {
    ...
    if (!revealed) {
      ...
      this.card.innerHTML = '<canvas class="tl514__card-back" data-card-back aria-hidden="true"></canvas>';
  ...
  status() {
    return Object.freeze({
      ...
      organicCanvasFire: this.world.dataset.flameTexture === 'celestial-fire-volume-v514',
      trueCelestialFire: this.world.dataset.flameArchitecture === 'sprite-billows-and-tongues',
      lightningStrokes: this.world.dataset.lightningStrokes !== 'none',
      volumetricBillows: Array.isArray(this.fire?.fireVolumes),
      flameTongues: Boolean(this.fire?.flameSprites?.tongueAccent),
      cardEdgeBurst: this.world.dataset.birthCorona === 'card-edge',
      responsiveAltar: this.world.dataset.responsiveAltar === 'v514',
      volumetricFire: this.world.dataset.visualCalibration === 'orbe-os-fluid-universe',
      cosmicNebula: true,
      ...
      optimizedCosmicBackdrop: this.world.dataset.universeBackdrop === 'tarot-cosmos-deep-v514',
      ...
      smoothBirthJourney: this.world.dataset.birthEasing === 'smoothstep-v514',
  ...
  destroy() {
    ...
    this.fire?.destroy();
    if (globalThis.divinaTarotLivreV514 === this) delete globalThis.divinaTarotLivreV514;
