/* DIVINA BRUXA — WORK13 V607 · QA DO SERVICE WORKER ATOMICO */
...
const cacheName = 'divina-bruxa-work13-v607-living-wisdom';
const harness = createHarness();

check('core:thirty-six-assets-declared', coreAssets.length === 36, coreAssets.length);
check('core:unique-assets', new Set(coreAssets).size === coreAssets.length);
check('core:app-v607', coreAssets.includes('./app-v208.js?v=607-work13-living-wisdom'));
check('core:living-wisdom-js', coreAssets.includes('./living-wisdom-path-v607.js?v=607-work13-living-wisdom'));
check('core:living-wisdom-css', coreAssets.includes('./living-wisdom-path-v607.css?v=607-work13-living-wisdom'));
...
check('install:thirty-six-core-assets', cache?.size === 36, cache?.size);
...
harness.stores.set('divina-bruxa-work13-v606-whit-silence-timing', new Map());
await harness.wait('activate');
check('activate:claims-clients', harness.counters().claimed === 1);
check('activate:keeps-v607', harness.stores.has(cacheName));
check('activate:deletes-v606-cache', !harness.stores.has('divina-bruxa-work13-v606-whit-silence-timing'));
for (const type of [
...
]) check(`activate:${type}`, harness.clientMessages.some(item => item.type === type && item.version === 607));
...
check('activate:wisdom-version-v607', harness.clientMessages.some(item => item.type === 'DIVINA_WORK13_LIVING_WISDOM_ACTIVE' && item.livingWisdomVersion === 607));

const onlineNavigation = await harness.fetchEvent(new FakeRequest('https://divina.test/index.html#journal', { mode:'navigate' }));
check('fetch:navigation-network', (await onlineNavigation.text()).includes('name="divina-work13" content="V607"'));
harness.offline(true);
const offlineNavigation = await harness.fetchEvent(new FakeRequest('https://divina.test/index.html#journal', { mode:'navigate' }));
...
for (const [file, needle] of [
  ['app-v208.js?v=607-work13-living-wisdom','divinaWork13Macro7V607'],
  ['living-wisdom-path-v607.js?v=607-work13-living-wisdom','LIVING_WISDOM_PATH_CONTRACT_V607'],
  ['living-wisdom-path-v607.css?v=607-work13-living-wisdom','[data-living-wisdom="v607"]'],
...
check('message:status-version', harness.sourceMessages.some(item => item.type === 'WORK12_STATUS' && item.version === 607));
check('message:status-thirty-six-assets', harness.sourceMessages.some(item => item.core?.length === 36));
await harness.message({ type:'CLEAR_DIVINA_CACHES' });
check('message:clear-caches', !harness.stores.has(cacheName));
check('message:clear-confirmed', harness.sourceMessages.some(item => item.type === 'DIVINA_CACHES_CLEARED' && item.version === 607));

const failures = checks.filter(item => !item.pass);
console.log(JSON.stringify({
  release:'V607',
  work:'WORK13',
  macroStage:'7-of-10 / living-wisdom-service-worker-runtime',
