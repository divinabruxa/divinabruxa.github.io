/* DIVINA BRUXA 4.0 — FLUIDEZ SUPREMA · MACROETAPA 1/14 · CORTE V549
...
import { createPwaPerformanceRecoveryCoreV546 } from './pwa-performance-recovery-core-v546.js?v=549';
...
const copy=()=>({
  pt:{
    ...
    verifying:'Verificando o núcleo instalado…',healthy:'Núcleo V549 íntegro e pronto.',
  ...
  en:{
    ...
    verifying:'Checking the installed core…',healthy:'V549 core is intact and ready.',
  ...
  es:{
    ...
    verifying:'Verificando el núcleo instalado…',healthy:'El núcleo V549 está íntegro y listo.',
...
const installStyle=()=>{
  ...
  link.id=STYLE_ID;link.rel='stylesheet';link.href='./pwa-resilience-v324.css?v=549';
...
const setupServiceWorker=()=>{
  ...
  if(String(globalThis.__divinaSWBootstrap||'').startsWith('v549')){
    navigator.serviceWorker.ready.then(announceRegistration).catch(()=>{});
    return;
  }
  globalThis.__divinaSWBootstrap='v549-pwa';
  navigator.serviceWorker.register('./sw.js?v=549',{updateViaCache:'none'})
    .then(registration=>{
      announceRegistration(registration);
    })
    .catch(error=>console.error('[Divina] PWA V549 isolado indisponível',error));
};
