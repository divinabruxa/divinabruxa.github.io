/* DIVINA BRUXA 3.0 — LOJA AMAZON · CORTE V543 / PWA V324
...
const setupServiceWorker=()=>{
  ...
  globalThis.__divinaSWBootstrap='v543';
  const register=()=>navigator.serviceWorker.register('./sw.js?v=543',{updateViaCache:'none'})
    .then(registration=>{
      dispatchEvent(new CustomEvent('divina:pwa-ready',{detail:{scope:registration.scope,version:VERSION,recovery:'loja-amazon-v543'}}));
      registration.update().catch(()=>{});
      return registration;
    })
    .catch(error=>console.error('[Divina] PWA V543 isolado indisponível',error));
