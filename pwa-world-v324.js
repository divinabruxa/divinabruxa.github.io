/* DIVINA BRUXA 4.0 — FLUIDEZ SUPREMA · MACROETAPA 2/14 · CORTE V550
...
import { createPwaPerformanceRecoveryCoreV546 } from './pwa-performance-recovery-core-v546.js?v=550';
...
const copy=()=>({
  pt:{
    ...
    verifying:'Verificando o núcleo instalado…',healthy:'Núcleo V550 íntegro e pronto.',
  ...
  en:{
    ...
    verifying:'Checking the installed core…',healthy:'V550 core is intact and ready.',
  ...
  es:{
    ...
    verifying:'Verificando el núcleo instalado…',healthy:'El núcleo V550 está íntegro y listo.',
...
const installStyle=()=>{
  ...
  link.id=STYLE_ID;link.rel='stylesheet';link.href='./pwa-resilience-v324.css?v=550';
...
const setupServiceWorker=()=>{
  ...
  if(String(globalThis.__divinaSWBootstrap||'').startsWith('v550')){
    navigator.serviceWorker.ready.then(announceRegistration).catch(()=>{});
    return;
  }
  globalThis.__divinaSWBootstrap='v550-pwa';
  navigator.serviceWorker.register('./sw.js?v=550',{updateViaCache:'none'})
    .then(registration=>{
      announceRegistration(registration);
    })
    .catch(error=>console.error('[Divina] PWA V550 isolado indisponível',error));
};
