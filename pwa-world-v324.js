/* DIVINA BRUXA 4.0 — FLUIDEZ SUPREMA · MACROETAPA 8/14 · CORTE V556
...
import { createPwaPerformanceRecoveryCoreV546 } from './pwa-performance-recovery-core-v546.js?v=556';
...
const copy=()=>({
  pt:{
    ...
    verifying:'Verificando o núcleo instalado…',healthy:'Núcleo V556 íntegro e pronto.',
  ...
  en:{
    ...
    verifying:'Checking the installed core…',healthy:'V556 core is intact and ready.',
  ...
  es:{
    ...
    verifying:'Verificando el núcleo instalado…',healthy:'El núcleo V556 está íntegro y listo.',
...
const installStyle=()=>{
  ...
  link.id=STYLE_ID;link.rel='stylesheet';link.href='./pwa-resilience-v324.css?v=556';
...
const setupServiceWorker=()=>{
  ...
  if(String(globalThis.__divinaSWBootstrap||'').startsWith('v556')){
    navigator.serviceWorker.ready.then(announceRegistration).catch(()=>{});
    return;
  }
  globalThis.__divinaSWBootstrap='v556-pwa';
  navigator.serviceWorker.register('./sw.js?v=556',{updateViaCache:'none'})
    .then(registration=>{
      announceRegistration(registration);
    })
    .catch(error=>console.error('[Divina] PWA V556 isolado indisponível',error));
};
