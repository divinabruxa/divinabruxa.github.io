/* DIVINA BRUXA — WORK13 V608 · QA DO SERVICE WORKER ATOMICO */
...
const cacheName = 'divina-bruxa-work13-v608-commerce-clarity';
const harness = createHarness();

check('core:thirty-eight-assets-declared', coreAssets.length === 38, coreAssets.length);
check('core:unique-assets', new Set(coreAssets).size === coreAssets.length);
check('core:app-v608', coreAssets.includes('./app-v208.js?v=608-work13-commerce-clarity'));
check('core:commerce-js', coreAssets.includes('./living-commerce-path-v608.js?v=608-work13-commerce-clarity'));
check('core:commerce-css', coreAssets.includes('./living-commerce-path-v608.css?v=608-work13-commerce-clarity'));
...
check('install:thirty-eight-core-assets', cache?.size === 38, cache?.size);
...
harness.stores.set('divina-bruxa-work13-v607-living-wisdom', new Map());
await harness.wait('activate');
check('activate:claims-clients', harness.counters().claimed === 1);
check('activate:keeps-v608', harness.stores.has(cacheName));
check('activate:deletes-v607-cache', !harness.stores.has('divina-bruxa-work13-v607-living-wisdom'));
for (const type of [
  ...
  'DIVINA_WORK13_COMMERCE_CLARITY_ACTIVE','DIVINA_RELEASE_READY'
]) check(`activate:${type}`, harness.clientMessages.some(item => item.type === type && item.version === 608));
...
const onlineNavigation = await harness.fetchEvent(new FakeRequest('https://divina.test/index.html#store', { mode:'navigate' }));
check('fetch:navigation-network', (await onlineNavigation.text()).includes('name="divina-work13" content="V608"'));
harness.offline(true);
const offlineNavigation = await harness.fetchEvent(new FakeRequest('https://divina.test/index.html#store', { mode:'navigate' }));
...
for (const [file, needle] of [
  ['app-v208.js?v=608-work13-commerce-clarity','divinaWork13Macro8V608'],
  ['living-commerce-path-v608.js?v=608-work13-commerce-clarity','LIVING_COMMERCE_PATH_CONTRACT_V608'],
  ['living-commerce-path-v608.css?v=608-work13-commerce-clarity','[data-living-commerce-path="v608"]'],
...
check('message:status-version', harness.sourceMessages.some(item => item.type === 'WORK12_STATUS' && item.version === 608));
check('message:status-thirty-eight-assets', harness.sourceMessages.some(item => item.core?.length === 38));
await harness.message({ type:'CLEAR_DIVINA_CACHES' });
check('message:clear-caches', !harness.stores.has(cacheName));
check('message:clear-confirmed', harness.sourceMessages.some(item => item.type === 'DIVINA_CACHES_CLEARED' && item.version === 608));

const failures = checks.filter(item => !item.pass);
console.log(JSON.stringify({
  release:'V608',
  work:'WORK13',
  macroStage:'8-of-10 / commerce-clarity-service-worker-runtime',
