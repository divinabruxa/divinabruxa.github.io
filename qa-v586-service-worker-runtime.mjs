/* DIVINA BRUXA 2.0 — FLUIDEZ SUPREMA · SERVICE WORKER RUNTIME V586 */
...
const self={
  ...
  clients:{
    ...
    matchAll:async()=>[{url:scope,postMessage:value=>messages.push(value),focus:async()=>{}}],
...
const fetchEvent=async request=>{
  let responseTask;
  listeners.get('fetch')?.({request,preloadResponse:undefined,respondWith:value=>{responseTask=Promise.resolve(value);}});
  return responseTask?responseTask:null;
};

await waitEvent('install');
check(skipWaitingCalls===1,'install:skip-waiting');
check(stores.has('divina-bruxa-v586-shell'),'install:cache-v586');
const shellCache=stores.get('divina-bruxa-v586-shell');
check((await shellCache.keys()).length>=132,'install:atomic-shell-size',(await shellCache.keys()).length);
for(const file of [
  'app-v208.js','magical-bubble-system-v586.js','magical-bubble-system-v586.css',
  'reality-intention-language-v585.js','reality-intention-language-v585.css',
  'universe-coordinate-law-v583.js','orb-persistent-journey-v565.js','orbital-menu-v502.js',
...
const oldCache=await caches.open('divina-bruxa-v585-shell');
...
check(!stores.has('divina-bruxa-v585-shell'),'activate:remove-v585-cache');
check(messages.some(value=>value?.type==='DIVINA_RELEASE_READY'&&value?.version===586),'activate:announce-v586');

let result=await message({type:'VERIFY_SHELL'});
check(result?.complete===true&&result?.atomic===true&&result?.version===586,'verify:complete-v586');
check(result?.ready===result?.total,'verify:complete-count');

online=false;
let response=await fetchEvent(new Request(`${scope}magical-bubble-system-v586.js?v=586`));
check(response?.status===200&&(await response.text()).includes('MAGICAL_BUBBLE_PROFILES_V586'),'offline:bubble-js-from-atomic-shell');
response=await fetchEvent(new Request(`${scope}magical-bubble-system-v586.css?v=586`));
check(response?.status===200&&(await response.text()).includes('BALÕES MÁGICOS VIVOS V586'),'offline:bubble-css-from-atomic-shell');
response=await fetchEvent(new Request(`${scope}reality-intention-language-v585.js?v=585`));
check(response?.status===200&&(await response.text()).includes('REALITY_INTENTIONS_V585'),'offline:v585-language-preserved');
...
response=await fetchEvent({
  url:`${scope}index.html?v586=1`,method:'GET',mode:'navigate',destination:'document',
  cache:'default',headers:new Headers()
});
check(response?.status===200&&(await response.text()).includes('app-v208.js?v=586-baloes-vivos'),'navigation:v586-shell-valid');
const contentCache=stores.get('divina-bruxa-v586-content');
check(Boolean(await contentCache?.match(new Request(`${scope}index.html?v586=1`))),'navigation:valid-shell-cached');

await shellCache.delete(new Request(`${scope}magical-bubble-system-v586.js`));
result=await message({type:'VERIFY_SHELL'});
check(result?.complete===false&&result?.missing?.includes('./magical-bubble-system-v586.js'),'repair:bubble-corruption-detected');
