/* DIVINA BRUXA — WORK13 V605 · QA BOOT RECUPERAVEL */
...
function harness() {
  ...
  vm.runInContext(bootScript, vm.createContext(context), { filename:'work13-inline-boot-v605.js' });
...
check('boot:work13-meta-v605', indexSource.includes('name="divina-work13" content="V605"'));
check('boot:work12-meta-v600', indexSource.includes('name="divina-work12" content="V600"'));
check('boot:app-v605', indexSource.includes('app-v208.js?v=605-work13-spread-reading'));
check('boot:worker-v605', indexSource.includes("register('./sw.js?v=605'"));
check('boot:v605-bootstrap', indexSource.includes("__divinaSWBootstrap='v605-work13-spread-reading-inline'"));
check('boot:v605-style', indexSource.includes('cosmic-spread-reading-v605.css?v=605-work13-spread-reading'));
...
check('boot:one-v605-style', [...indexSource.matchAll(/id="divinaCosmicSpreadReadingV605"/g)].length === 1);
...
check('boot:watchdog-work13-v605', watchdog.root.dataset.work13Boot === 'released-v605');
...
check('boot:manual-work13', manual.root.dataset.work13Boot === 'released-v605');
...
check('boot:healthy-work13-ready', healthy.root.dataset.work13Boot === 'ready-v605');
check('boot:healthy-never-bypassed', !healthy.home.classList.contains('active'));
healthy.fire('load');
check('boot:registers-v605', healthy.registrations.some(item => item.url === './sw.js?v=605'));
...
console.log(JSON.stringify({
  release:'V605',
  work:'WORK13',
  macroStage:'5-of-10 / cards-converse-boot-runtime',
