/* DIVINA BRUXA — WORK13 V603 · QA BOOT RECUPERAVEL */
...
function harness() {
  ...
  vm.runInContext(bootScript, vm.createContext(context), { filename:'work13-inline-boot-v603.js' });
...
check('boot:work13-meta-v603', indexSource.includes('name="divina-work13" content="V603"'));
check('boot:work12-meta-v600', indexSource.includes('name="divina-work12" content="V600"'));
check('boot:app-v603', indexSource.includes('app-v208.js?v=603-work13-resonance'));
check('boot:worker-v603', indexSource.includes("register('./sw.js?v=603'"));
check('boot:resonance-bootstrap', indexSource.includes("__divinaSWBootstrap='v603-work13-resonance-inline'"));
check('boot:resonance-style-v603', indexSource.includes('cosmos-reality-resonance-v603.css?v=603-work13-resonance'));
...
check('boot:one-resonance-style', [...indexSource.matchAll(/id="divinaCosmosRealityResonanceV603"/g)].length === 1);
...
check('boot:watchdog-work13-v603', watchdog.root.dataset.work13Boot === 'released-v603');
...
check('boot:manual-work13', manual.root.dataset.work13Boot === 'released-v603');
...
check('boot:healthy-work13-ready', healthy.root.dataset.work13Boot === 'ready-v603');
check('boot:healthy-never-bypassed', !healthy.home.classList.contains('active'));
healthy.fire('load');
check('boot:registers-v603', healthy.registrations.some(item => item.url === './sw.js?v=603'));
...
console.log(JSON.stringify({
  release:'V603',
  work:'WORK13',
  macroStage:'3-of-10 / resonance-boot-runtime',
