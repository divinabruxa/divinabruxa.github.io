/* DIVINA BRUXA — MACROETAPA V510 · UNIVERSO VIVO E FLUIDEZ SUPREMA
   A mesma Orbe Suprema V501 deixa a Home, ocupa o altar do Tarot e revela
   cartas por um motor híbrido de universo vivo e plasma estelar. Galáxias,
   nébulas e filamentos nascem dela; nenhuma carta decorativa vaga sem origem.
   Regras: 78 diretas, zero repetição e seis por fileira. */
...
class SupremeVolumetricFlameEngineV510 {
  constructor({ cosmos, effects, stage, orb, card }) {
    ...
    this.orb = orb;
    this.card = card;
    this.world = stage.closest('.tl510');
    ...
    this.height = 1;
    this.pixelRatio = 1;
    this.renderScale = constrained() ? 0.5 : (innerWidth < 700 ? 0.62 : 0.82);
    this.effectsScale = constrained() ? 0.9 : (innerWidth < 700 ? 1.12 : 1.32);
    this.targetFps = reducedMotion() ? 24 : (constrained() ? 36 : 60);
    this.energy = 0.82;
    this.touchPower = 0;
  ...
  spawnAmbient(timestamp) {
    const orb = this.elementCenter(this.orb);
    const card = this.elementCenter(this.card);
    const budget = constrained() ? 130 : 230;
    if (this.sparks.length >= budget) return;
    const frequency = reducedMotion() ? 0.08 : (constrained() ? 0.28 : 0.48);
  ...
  adapt(deltaMilliseconds) {
    ...
    if (this.slowFrames < 16) return;
    this.slowFrames = 0;
    this.renderScale = Math.max(0.46, this.renderScale * 0.84);
    this.effectsScale = Math.max(0.92, this.effectsScale * 0.9);
    this.targetFps = Math.max(42, this.targetFps - 6);
    this.resize();
  ...
  ignite(strength = 1) {
    ...
    const cx = card.x * this.width;
    const cy = card.cssY * this.height;
    const count = reducedMotion() ? 12 : (constrained() ? 34 : 64);
    ...
    const budget = constrained() ? 160 : 280;
...
export class TarotLivreSupremeV510 {
  constructor(root, {
    storage = store,
    ...
    if (!orbCore?.claim || !orbCore?.orb) {
      throw new Error('A Orbe Suprema V501 precisa despertar antes do Tarot Livre V510.');
    }
    ...
    this.root.innerHTML = '';
    this.root.dataset.tarotWorld = 'supremo-v510';
    ...
    this.world.dataset.responsiveAltar = 'v510';
    this.world.dataset.visualCalibration = 'fluid-living-universe';
    this.world.dataset.cardOrigin = 'supreme-orb-v501';
    this.world.dataset.decorativeCards = 'none';
    this.render(false);
    this.fire = new SupremeVolumetricFlameEngineV510({
    ...
    document.documentElement.dataset.tarotLivre = 'v510';
    const readiness = Object.freeze({
      version: VERSION,
      engine: 'SupremeVolumetricFlameEngineV510',
  ...
  build() {
    this.root.innerHTML = `
      <div class="tl510" data-phase="idle" data-engine="pending">
        <header class="tl510__header">
          <div class="tl510__title">
            <h2>Tarot Livre</h2>
            <span class="tl510__title-star" aria-hidden="true"><i></i><b></b><i></i></span>
          </div>

          <a class="tl510__mesa" href="#spreads" data-go="spreads" aria-label="Abrir Tiragens e Mesa Real">
        ...
        <section class="tl510__stage" data-stage aria-label="Tarot Livre — universo vivo nascido da Orbe">
          <canvas class="tl510__canvas tl510__cosmos" data-cosmos aria-hidden="true"></canvas>
          <canvas class="tl510__canvas tl510__effects" data-effects aria-hidden="true"></canvas>

          <div class="tl510__portal-crown" aria-hidden="true"><i></i><i></i><i></i></div>

          <div class="tl510__card-zone" data-card-zone tabindex="0"
               aria-label="Carta atual. Use as setas para navegar pelas cartas reveladas.">
            <div class="tl510__card" data-current-card data-empty="true">
              <canvas class="tl510__card-back" data-card-back aria-hidden="true"></canvas>
            </div>
          </div>

          <nav class="tl510__card-nav" data-card-nav aria-label="Navegar pelas cartas reveladas">
          ...
          <div class="tl510__orb-host" data-tarot-orb-host>
            <span class="tl510__orb-aura" aria-hidden="true"><i></i><i></i><i></i></span>
            <span class="tl510__orb-flare" aria-hidden="true"></span>
          </div>

          <p class="tl510__counter" aria-label="Cartas reveladas">
            <b data-count>0</b><span>/78</span>
          </p>
          <p class="tl510__signal" data-signal aria-live="polite">Toque na Orbe para revelar seu Tarot.</p>
        </section>

        <div class="tl510__toolbar" aria-label="Controles do Tarot Livre">
        ...
        <section class="tl510__constellation" aria-labelledby="tl510ConstellationTitle">
          <header>
            <div>
              <small>SEU CÍRCULO</small>
              <h3 id="tl510ConstellationTitle">Cartas reveladas</h3>
            </div>
            <span data-remaining>78 aguardam</span>
          </header>
          <div class="tl510__grid" data-grid role="grid"
               aria-label="Cartas reveladas, seis por fileira" aria-colcount="6"></div>
          <p class="tl510__empty-grid" data-empty-grid>Nenhuma carta foi revelada ainda.</p>
        </section>

        <p class="tl510__sr" data-live role="status" aria-live="polite" aria-atomic="true"></p>
      </div>`;

    this.world = this.root.querySelector('.tl510');
  ...
  enter() {
    ...
    this.orb.dataset.tarotReveal = 'v510';
  ...
  leave(nextRoute = 'home') {
    ...
    document.body?.classList.remove('db510-tarot-birthing');
    this.fire?.setActive(false);
    ...
    this.orbCore.settleRoute?.(nextRoute, 'tarot-leave-v510');
    return true;
  }
  ...
  setPhase(phase) {
    ...
    document.body?.classList.toggle('db510-tarot-birthing', this.active && ritualVisible);
  ...
  async prepareCardContent(index) {
    ...
    cradle.innerHTML = `${cardImageMarkup(card, {
      ...
      priority: 'high'
    })}<div class="tl510__card-name"><b>${escapeText(card.name)}</b><small>${index + 1} / ${this.state.revealed.length}</small></div>`;
  ...
  async transitionTo(index, { onBirth } = {}) {
    ...
    onBirth?.();
    if (!reducedMotion()) {
      const birth = this.card.animate([
        { opacity: 0.22, transform: 'translate3d(0,68px,0) scale(.68)', filter: 'blur(7px) saturate(1.35) brightness(1.08)', clipPath: 'inset(42% 18% round 34px)' },
        { opacity: 1, transform: 'translate3d(0,-5px,0) scale(1.018)', filter: 'blur(0) saturate(1.08) brightness(1.08)', clipPath: 'inset(0 round 18px)', offset: 0.72 },
        { opacity: 1, transform: 'translate3d(0,0,0) scale(1)', filter: 'blur(0) saturate(1) brightness(1)', clipPath: 'inset(0 round 18px)' }
      ], { duration: REVEAL_SETTLE_MS, easing: 'cubic-bezier(.13,.84,.17,1)' });
  ...
  async draw() {
    ...
    try {
      ...
      const born = await this.transitionTo(result.position, { onBirth: () => {
      ...
      globalThis.dispatchEvent?.(new CustomEvent('tarot:rebirth-revealed', {
        detail: {
          ...
          engine: 'living-universe-flame-v510'
        }
      }));
  ...
  async show(index, animate = false, direction = 0) {
    ...
    try {
      ...
      if (animate && !reducedMotion()) {
        ...
        ], { duration: 250, easing: 'cubic-bezier(.16,.82,.18,1)' });
  ...
  render(animateCurrent = false) {
    ...
    if (!revealed) {
      ...
      this.card.innerHTML = '<canvas class="tl510__card-back" data-card-back aria-hidden="true"></canvas>';
  ...
  status() {
    return Object.freeze({
      ...
      responsiveAltar: this.world.dataset.responsiveAltar === 'v510',
      volumetricPlasma: this.world.dataset.visualCalibration === 'fluid-living-universe',
      cosmicNebula: true,
  ...
  destroy() {
    ...
    this.fire?.destroy();
    if (globalThis.divinaTarotLivreV510 === this) delete globalThis.divinaTarotLivreV510;
