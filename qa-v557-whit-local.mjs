...
check(app.startsWith('/* DIVINA BRUXA 4.0 — FLUIDEZ SUPREMA · MACROETAPA 9/14 · V557'),'release:app-v557');
check(app.includes('window.divinaWhitLocalReleaseV557')&&app.includes("currentMacroStage:'9-of-14'"),'release:contrato-v557');
...
check(index.includes('WHIT LOCAL SUPREMA · V557')&&index.includes('app-v208.js?v=557'),'release:index');
check(index.includes('sw.js?v=557')&&index.includes('manifest.webmanifest?v=557'),'release:cache');
check(sw.includes('const VERSION=557')&&sw.includes('divina-bruxa-v557-shell')&&sw.includes("'./whit-local-supreme-v557.css'"),'release:sw-atomico');
check(sw.includes("'./whit-local-guide-v540.js'")&&sw.includes("'./ai-engine.js'"),'offline:whit-local-completa');
check(pwa.includes('const VERSION=557')&&manifest.includes('?v=557'),'release:pwa-manifest');
check(fs.readdirSync(root).filter(file=>file.endsWith('.html')).length===321,'regressao:321-paginas-reais');

const total=passed+failures.length;
console.log(`V557 Whit Local Suprema: ${passed}/${total} verificações aprovadas.`);
