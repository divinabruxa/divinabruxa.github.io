/* DIVINA BRUXA — MACROETAPA V504 · TAROT LIVRE SUPREMO
...
class SupremeStellarFlameEngineV504 {
  constructor({ cosmos, effects, stage, orb, card }) {
    ...
    this.orb = orb;
    this.card = card;
    this.world = stage.closest('.tl504');
    ...
    try {
      ...
      const fragmentSource = `
        ...
        void main(){
          ...
          float sway=(sin(ribbonT*7.1-time*1.54)+0.52*sin(ribbonT*14.6+time*1.03))*0.047*taper;
          sway+=sideNoise*0.035*taper;
          ...
          float width=(0.043+0.025*(1.0-ribbonT))*breath*(0.84+0.16*u_energy);
          ...
          for(int strand=0;strand<3;strand++){
            ...
            vec2 strandCenter=center+normal*strandWave*(0.015+fi*0.004)*taper;
            float strandDistance=abs(dot(uv-strandCenter,normal));
            float filament=exp(-strandDistance*178.0)*segment;
            color+=mix(vec3(0.87,0.08,1.0),vec3(1.0,0.66,0.22),fi/2.0)*filament*(0.48+u_energy*0.21);
            alpha+=filament*0.48;
          }

          color+=vec3(0.43,0.025,1.0)*outer*(0.84+u_energy*0.25);
          color+=vec3(1.0,0.09,0.72)*middle*(0.91+u_energy*0.32);
          color+=vec3(1.0,0.48,0.10)*middle*0.78;
          color+=vec3(1.0,0.91,0.62)*core*(1.22+u_energy*0.54);
  ...
  drawFallbackCosmos(context, timestamp) {
    ...
    const amplitude = clamp(this.width * 0.095, 30, 76);
    const trace = (phase = 0, widthScale = 1) => {
      context.beginPath();
      for (let step = 0; step <= 34; step += 1) {
        const progress = step / 34;
        const eased = progress * progress * (3 - 2 * progress);
        const envelope = Math.sin(Math.PI * progress);
        const mainS = Math.sin(progress * Math.PI * 2.04 - time * 0.92 + phase) * amplitude * envelope;
        const braid = Math.sin(progress * Math.PI * 7.1 + time * 1.37 + phase * 1.8) * amplitude * 0.18 * envelope;
        const x = ox + (cx - ox) * eased + (mainS + braid) * widthScale;
        const y = oy + (cy - oy) * eased + Math.cos(progress * Math.PI * 4.2 - time + phase) * 7 * envelope;
        if (step === 0) context.moveTo(x, y);
        else context.lineTo(x, y);
      }
    };
    const gradient = context.createLinearGradient(ox, oy, cx, cy);
    gradient.addColorStop(0, 'rgba(116,34,255,0)');
    gradient.addColorStop(0.12, `rgba(137,38,255,${0.46 + this.energy * 0.06})`);
    gradient.addColorStop(0.43, `rgba(239,55,224,${0.58 + this.energy * 0.07})`);
    gradient.addColorStop(0.73, `rgba(255,120,35,${0.72 + this.energy * 0.06})`);
    gradient.addColorStop(0.96, 'rgba(255,244,205,.82)');
    gradient.addColorStop(1, 'rgba(255,244,205,0)');

    context.save();
    context.globalCompositeOperation = 'lighter';
    const strands = [
      { phase: -1.72, scale: 0.86, glow: '#7c2cff' },
      { phase: 0.08, scale: 1, glow: '#ff45db' },
      { phase: 1.86, scale: 0.9, glow: '#ff9d35' }
    ];
    for (const strand of strands) {
      ...
      context.lineWidth = constrained() ? 31 : 43;
      trace(strand.phase, strand.scale);
      context.stroke();

      context.shadowBlur = 22;
      context.globalAlpha = 0.72;
      context.lineWidth = constrained() ? 8 : 12;
      trace(strand.phase, strand.scale);
      context.stroke();
      ...
      context.lineWidth = constrained() ? 1.6 : 2.4;
      trace(strand.phase, strand.scale);
      context.stroke();
    }
    context.restore();
  ...
  drawBirthWave(context, timestamp) {
    ...
    const amplitude = clamp(this.width * 0.09, 28, 70);
    const ox = orb.x * this.width;
    const oy = orb.cssY * this.height;
    const cx = card.x * this.width;
    const cy = card.cssY * this.height;
    const x = ox + (cx - ox) * eased + Math.sin(eased * Math.PI * 2.05 - timestamp * 0.0009) * amplitude * envelope;
    const y = oy + (cy - oy) * eased;
    const pulse = (1 - progress) * this.birthStrength;
    const radius = 18 + pulse * 24;
    ...
    if (progress > 0.68) {
      ...
      context.fillRect(cx - 1.2, cy - crownRadius * 0.9, 2.4, crownRadius * 1.8);
      context.fillRect(cx - crownRadius * 0.9, cy - 1.2, crownRadius * 1.8, 2.4);
    }
    context.restore();
  }
  ...
  birth(strength = 1) {
    ...
    this.birthDuration = reducedMotion() ? 160 : 690;
...
export class TarotLivreSupremeV504 {
  constructor(root, {
    storage = store,
    ...
    if (!orbCore?.claim || !orbCore?.orb) {
      throw new Error('A Orbe Suprema V501 precisa despertar antes do Tarot Livre V504.');
    }
    ...
    this.root.innerHTML = '';
    this.root.dataset.tarotWorld = 'supremo-v504';
    ...
    this.build();
    this.render(false);
    this.fire = new SupremeStellarFlameEngineV504({
    ...
    document.documentElement.dataset.tarotLivre = 'v504';
    const readiness = Object.freeze({
      version: VERSION,
      engine: 'SupremeStellarFlameEngineV504',
  ...
  build() {
    this.root.innerHTML = `
      <div class="tl504" data-phase="idle" data-engine="pending">
        <header class="tl504__header">
          <div class="tl504__title">
            <h2>Tarot Livre</h2>
            <span class="tl504__title-star" aria-hidden="true"><i></i><b></b><i></i></span>
          </div>

          <a class="tl504__mesa" href="#spreads" data-go="spreads" aria-label="Abrir Tiragens e Mesa Real">
        ...
        <section class="tl504__stage" data-stage aria-label="Tarot Livre — Cosmos de Fogo">
          <canvas class="tl504__canvas tl504__cosmos" data-cosmos aria-hidden="true"></canvas>
          <canvas class="tl504__canvas tl504__effects" data-effects aria-hidden="true"></canvas>

          <div class="tl504__portal-crown" aria-hidden="true"><i></i><i></i><i></i></div>

          <div class="tl504__card-zone" data-card-zone tabindex="0"
               aria-label="Carta atual. Use as setas para navegar pelas cartas reveladas.">
            <div class="tl504__card" data-current-card data-empty="true">
              <canvas class="tl504__card-back" data-card-back aria-hidden="true"></canvas>
            </div>
          </div>

          <nav class="tl504__card-nav" data-card-nav aria-label="Navegar pelas cartas reveladas">
          ...
          <div class="tl504__orb-host" data-tarot-orb-host>
            <span class="tl504__orb-aura" aria-hidden="true"><i></i><i></i><i></i></span>
            <span class="tl504__orb-flare" aria-hidden="true"></span>
          </div>

          <p class="tl504__counter" aria-label="Cartas reveladas">
            <b data-count>0</b><span>/78</span>
          </p>
          <p class="tl504__signal" data-signal aria-live="polite">Toque na Orbe para revelar seu Tarot.</p>
        </section>

        <div class="tl504__toolbar" aria-label="Controles do Tarot Livre">
        ...
        <section class="tl504__constellation" aria-labelledby="tl504ConstellationTitle">
          <header>
            <div>
              <small>SEU CÍRCULO</small>
              <h3 id="tl504ConstellationTitle">Cartas reveladas</h3>
            </div>
            <span data-remaining>78 aguardam</span>
          </header>
          <div class="tl504__grid" data-grid role="grid"
               aria-label="Cartas reveladas, seis por fileira" aria-colcount="6"></div>
          <p class="tl504__empty-grid" data-empty-grid>Nenhuma carta foi revelada ainda.</p>
        </section>

        <p class="tl504__sr" data-live role="status" aria-live="polite" aria-atomic="true"></p>
      </div>`;

    this.world = this.root.querySelector('.tl504');
  ...
  enter() {
    ...
    this.orb.dataset.tarotReveal = 'v504';
  ...
  leave(nextRoute = 'home') {
    ...
    document.body?.classList.remove('db504-tarot-birthing');
    this.fire?.setActive(false);
    ...
    this.orbCore.settleRoute?.(nextRoute, 'tarot-leave-v504');
    return true;
  }
  ...
  setPhase(phase) {
    ...
    document.body?.classList.toggle('db504-tarot-birthing', this.active && ritualVisible);
    ...
    try {
      ...
      globalThis.dispatchEvent?.(new CustomEvent('tarot:rebirth-revealed', {
        detail: {
          ...
          engine: 'supreme-stellar-flame-v504'
        }
      }));
  ...
  show(index, animate = false) {
    ...
    this.card.innerHTML = `${cardImageMarkup(card, {
      ...
      priority: 'high'
    })}<div class="tl504__card-name"><b>${escapeText(card.name)}</b><small>${index + 1} / ${this.state.revealed.length}</small></div>`;
  ...
  render(animateCurrent = false) {
    ...
    if (!revealed) {
      ...
      this.card.innerHTML = '<canvas class="tl504__card-back" data-card-back aria-hidden="true"></canvas>';
  ...
  destroy() {
    ...
    this.fire?.destroy();
    if (globalThis.divinaTarotLivreV504 === this) delete globalThis.divinaTarotLivreV504;
