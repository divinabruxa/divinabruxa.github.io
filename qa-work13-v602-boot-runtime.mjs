/* DIVINA BRUXA — WORK13 V602 · QA BOOT RECUPERAVEL */
...
function harness() {
  ...
  vm.runInContext(bootScript, vm.createContext(context), { filename:'work13-inline-boot-v602.js' });
...
check('boot:work13-meta-v602', indexSource.includes('name="divina-work13" content="V602"'));
check('boot:work12-meta-v600', indexSource.includes('name="divina-work12" content="V600"'));
check('boot:app-v602', indexSource.includes('app-v208.js?v=602-work13-context'));
check('boot:worker-v602', indexSource.includes("register('./sw.js?v=602'"));
check('boot:context-bootstrap', indexSource.includes("__divinaSWBootstrap='v602-work13-context-inline'"));
...
check('boot:watchdog-work13-v602', watchdog.root.dataset.work13Boot === 'released-v602');
...
check('boot:manual-work13', manual.root.dataset.work13Boot === 'released-v602');
...
check('boot:healthy-work13-ready', healthy.root.dataset.work13Boot === 'ready-v602');
check('boot:healthy-never-bypassed', !healthy.home.classList.contains('active'));
healthy.fire('load');
check('boot:registers-v602', healthy.registrations.some(item => item.url === './sw.js?v=602'));
...
console.log(JSON.stringify({
  release:'V602',
  work:'WORK13',
  macroStage:'2-of-10 / context-boot-runtime',
