...
for (const key of ['shell','ritual','chambers','intelligence','finalContinuity','daily','spread','wisdom','commerce','mediaSkins','entry','worldPresence','spreadsSoul','schoolSoul','librarySoul','journalSoul']) {
  check(`portrait:${key}`, /@media\s*\(?max-width\s*:\s*430px\)?/.test(styles[key]), key);
}
for (const key of ['shell','orbitalMenu','ritual','chambers','daily','spread','spreadsSoul','schoolSoul','librarySoul','journalSoul']) {
...
for (const key of ['shell','ritual','chambers','finalContinuity','spread','wisdom','commerce','mediaSkins','schoolSoul','librarySoul','journalSoul']) {
...
check('offline:cache-v610', worker.includes("divina-bruxa-work13-v610-diario-soul"));
check('offline:fifty-seven-core-assets', (worker.match(/const CORE = Object\.freeze\(\[([\s\S]*?)\]\);/)?.[1].match(/'[^']+'/g) || []).length === 57);
