...
orb.closest=selector=>selector==='#orb'?orb:null;
...
ok(controller.status().version===632,'autoridade V632');
...
ok(orb.getAttribute('aria-label')==='Orbe central. Toque para voltar ao Início','Orbe anuncia retorno ao Início');
fire(entry,'click');
...
fire(doc,'divina:route-ready',{id:'notifications'});
menu={...menu,state:'opening',targetOpen:true};
fire(doc,'divina:menu-state',{state:'opening'});
let prevented=0;let stopped=0;
const capturedClick=documentHandlers.get('click')[0];
capturedClick({target:orb,preventDefault(){prevented+=1;},stopImmediatePropagation(){stopped+=1;}});
ok(prevented===0&&stopped===0,'menu aberto recebe o toque da Orbe sem interceptação');
ok(controller.status().menuOpenOrbDelegatesToMenu===true,'delegação publicada no contrato');
ok(controller.status().menuOpenOrbAction==='home','ação da Orbe aberta é Início');
...
ok(prevented===0&&stopped===0,'menu aberto recebe teclado sem interceptação');
menu={...menu,state:'closed',targetOpen:false};
fire(doc,'divina:menu-state',{state:'closed'});
capturedClick({target:orb,preventDefault(){prevented+=1;},stopImmediatePropagation(){stopped+=1;}});
ok(prevented===1&&stopped===1,'mundo fechado mantém a Orbe como abertura do menu');
...
ok(opens===routes.length+1,'uma abertura por mundo e prova fechada adicional');
...
controller.destroy();
console.log(`V632 entrada/runtime: ${checks}/${checks} PASS`);
