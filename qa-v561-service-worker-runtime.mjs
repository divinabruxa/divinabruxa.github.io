...
const inheritedBody=relative=>{
  if(relative.endsWith('.html'))return '<!doctype html><html><body><main>V560 inherited fixture · Música · Vídeos</main></body></html>';
  if(relative.endsWith('.js'))return '/* inherited V560 fixture */';
  if(relative.endsWith('.css'))return '/* inherited V560 fixture */';
  if(relative.endsWith('.json')||relative.endsWith('.webmanifest'))return '{}';
  return 'inherited-v560-fixture';
};
const fetchMock=async request=>{
  ...
  if(fs.existsSync(file)&&fs.statSync(file).isFile())body=fs.readFileSync(file);
    ...
    // O pacote V561 é um delta sobre a V560. Fixtures sintéticas representam
...
check(cacheStores.has('divina-bruxa-v561-shell'),'install-cache-v561');
check((await cacheStores.get('divina-bruxa-v561-shell').keys()).length>=125,'install-shell-atomico');
check(!cacheStores.has('divina-bruxa-v561-offline-core'),'install-sem-cache-opcional');

const shellCache=cacheStores.get('divina-bruxa-v561-shell');
let response=await shellCache.match(new Request(`${scope}ethical-return-core-v561.js`));
check(response?.status===200&&(await response.text()).includes('ETHICAL_RETURN_CONTRACT_V561'),'install-modulo-v561-real');
response=await shellCache.match(new Request(`${scope}ethical-return-core-v561.css`));
check(response?.status===200&&(await response.text()).includes('.erv561__hero'),'install-css-v561-real');

const oldCache=await caches.open('divina-bruxa-v560-shell');
...
check(!cacheStores.has('divina-bruxa-v560-shell'),'activate-remove-cache-antigo');

let result=await message({type:'VERIFY_SHELL'});
check(result?.complete===true&&result?.atomic===true&&result?.version===561,'verify-shell-completo');
...
check(!cacheStores.has('divina-bruxa-v561-premium-static'),'premium-sem-cache-na-negacao');
...
check(response?.status===200,'navegacao-v561-valida');
const contentCache=cacheStores.get('divina-bruxa-v561-content');
check(Boolean(await contentCache?.match(new Request(`${scope}index.html?fresh=1`))),'navegacao-v561-cacheada');

await shellCache.delete(new Request(`${scope}ethical-return-core-v561.js`));
result=await message({type:'VERIFY_SHELL'});
check(result?.complete===false&&result?.missing?.includes('./ethical-return-core-v561.js'),'corrupcao-v561-detectada');
result=await message({type:'REPAIR_SHELL'});
check(result?.complete===true,'corrupcao-v561-reparada');

result=await message({type:'CLEAR_OPTIONAL_OFFLINE'});
check(result?.ok===true,'cache-opcional-limpo');
check(!cacheStores.has('divina-bruxa-v561-offline-core')&&!cacheStores.has('divina-bruxa-v561-premium-static'),'stores-opcionais-removidas');
check(result?.authorityDataRemoved===false,'nao-inventa-exclusao-privada');

const quotaCache=await caches.open('divina-bruxa-v561-content');quotaCache.failNextPut=true;
...
check(inheritedFixtures.size>0,'fixture-herdada-v560-utilizada');
const total=passed+failures.length;
console.log(`V561 Service Worker runtime: ${passed}/${total} verificações aprovadas (${inheritedFixtures.size} arquivos herdados simulados).`);
