...
export function cardImageMarkup(card, { alt = card.name, priority = 'lazy', decorative = false } = {}) {
  ...
  return `<img src="${escapeAttribute(card.imageSources.medium)}" data-card-index="${card.index}" width="300" height="450" loading="${eager ? 'eager' : 'lazy'}" decoding="async" fetchpriority="${eager ? 'high' : 'low'}" alt="${decorative ? '' : escapeAttribute(alt)}">`;
}
