/* DIVINA BRUXA — MACROETAPA V508 · CHAMA SOBERANA VOLUMÉTRICA
   A mesma Orbe Suprema V501 deixa a Home, ocupa o altar do Tarot e revela
   cartas por um motor híbrido de plasma volumétrico. Nébulas, células vivas,
   filamentos e fagulhas Canvas. Regras: 78 diretas, zero repetição e seis por fileira. */
...
class SupremeVolumetricFlameEngineV508 {
  constructor({ cosmos, effects, stage, orb, card }) {
    ...
    this.orb = orb;
    this.card = card;
    this.world = stage.closest('.tl508');
    ...
    try {
      ...
      const fragmentSource = `
        ...
        void main(){
          ...
          float width=(0.041+0.022*(1.0-flameT))*breath*(0.88+0.12*u_energy);
          ...
          alpha+=outer*0.55+body*0.52+core*0.38;
  ...
  drawFallbackCosmos(context, timestamp) {
    ...
    context.globalCompositeOperation = 'lighter';
    const plasmaCellCount = constrained() ? 18 : 28;
    ...
    context.lineWidth = constrained() ? 14 : 19;
    trace(0, 0.28);
    context.stroke();
  ...
  ignite(strength = 1) {
    ...
    const cx = card.x * this.width;
    const cy = card.cssY * this.height;
    const count = reducedMotion() ? 42 : (constrained() ? 112 : 190);
  ...
  spawnCardBurst(strength = 1) {
    ...
    const count = constrained() ? 74 : 126;
    ...
    const budget = constrained() ? 380 : 680;
    if (this.sparks.length > budget) this.sparks.splice(0, this.sparks.length - budget);
  }
...
export class TarotLivreSupremeV508 {
  constructor(root, {
    storage = store,
    ...
    if (!orbCore?.claim || !orbCore?.orb) {
      throw new Error('A Orbe Suprema V501 precisa despertar antes do Tarot Livre V508.');
    }
    ...
    this.root.innerHTML = '';
    this.root.dataset.tarotWorld = 'supremo-v508';
    ...
    this.world.dataset.flameTexture = 'volumetric-plasma';
    this.world.dataset.birthCorona = 'card-edge';
    this.world.dataset.responsiveAltar = 'v508';
    this.world.dataset.visualCalibration = 'sovereign-volume';
    this.render(false);
    this.fire = new SupremeVolumetricFlameEngineV508({
    ...
    document.documentElement.dataset.tarotLivre = 'v508';
    const readiness = Object.freeze({
      version: VERSION,
      engine: 'SupremeVolumetricFlameEngineV508',
  ...
  build() {
    this.root.innerHTML = `
      <div class="tl508" data-phase="idle" data-engine="pending">
        <header class="tl508__header">
          <div class="tl508__title">
            <h2>Tarot Livre</h2>
            <span class="tl508__title-star" aria-hidden="true"><i></i><b></b><i></i></span>
          </div>

          <a class="tl508__mesa" href="#spreads" data-go="spreads" aria-label="Abrir Tiragens e Mesa Real">
        ...
        <section class="tl508__stage" data-stage aria-label="Tarot Livre — Cosmos de Fogo">
          <canvas class="tl508__canvas tl508__cosmos" data-cosmos aria-hidden="true"></canvas>
          <canvas class="tl508__canvas tl508__effects" data-effects aria-hidden="true"></canvas>

          <div class="tl508__portal-crown" aria-hidden="true"><i></i><i></i><i></i></div>

          <div class="tl508__card-zone" data-card-zone tabindex="0"
               aria-label="Carta atual. Use as setas para navegar pelas cartas reveladas.">
            <div class="tl508__card" data-current-card data-empty="true">
              <canvas class="tl508__card-back" data-card-back aria-hidden="true"></canvas>
            </div>
          </div>

          <nav class="tl508__card-nav" data-card-nav aria-label="Navegar pelas cartas reveladas">
          ...
          <div class="tl508__orb-host" data-tarot-orb-host>
            <span class="tl508__orb-aura" aria-hidden="true"><i></i><i></i><i></i></span>
            <span class="tl508__orb-flare" aria-hidden="true"></span>
          </div>

          <p class="tl508__counter" aria-label="Cartas reveladas">
            <b data-count>0</b><span>/78</span>
          </p>
          <p class="tl508__signal" data-signal aria-live="polite">Toque na Orbe para revelar seu Tarot.</p>
        </section>

        <div class="tl508__toolbar" aria-label="Controles do Tarot Livre">
        ...
        <section class="tl508__constellation" aria-labelledby="tl508ConstellationTitle">
          <header>
            <div>
              <small>SEU CÍRCULO</small>
              <h3 id="tl508ConstellationTitle">Cartas reveladas</h3>
            </div>
            <span data-remaining>78 aguardam</span>
          </header>
          <div class="tl508__grid" data-grid role="grid"
               aria-label="Cartas reveladas, seis por fileira" aria-colcount="6"></div>
          <p class="tl508__empty-grid" data-empty-grid>Nenhuma carta foi revelada ainda.</p>
        </section>

        <p class="tl508__sr" data-live role="status" aria-live="polite" aria-atomic="true"></p>
      </div>`;

    this.world = this.root.querySelector('.tl508');
  ...
  enter() {
    ...
    this.orb.dataset.tarotReveal = 'v508';
  ...
  leave(nextRoute = 'home') {
    ...
    document.body?.classList.remove('db508-tarot-birthing');
    this.fire?.setActive(false);
    ...
    this.orbCore.settleRoute?.(nextRoute, 'tarot-leave-v508');
    return true;
  }
  ...
  setPhase(phase) {
    ...
    document.body?.classList.toggle('db508-tarot-birthing', this.active && ritualVisible);
    ...
    try {
      ...
      globalThis.dispatchEvent?.(new CustomEvent('tarot:rebirth-revealed', {
        detail: {
          ...
          engine: 'supreme-volumetric-flame-v508'
        }
      }));
  ...
  show(index, animate = false) {
    ...
    this.card.innerHTML = `${cardImageMarkup(card, {
      ...
      priority: 'high'
    })}<div class="tl508__card-name"><b>${escapeText(card.name)}</b><small>${index + 1} / ${this.state.revealed.length}</small></div>`;
  ...
  render(animateCurrent = false) {
    ...
    if (!revealed) {
      ...
      this.card.innerHTML = '<canvas class="tl508__card-back" data-card-back aria-hidden="true"></canvas>';
  ...
  status() {
    return Object.freeze({
      ...
      organicCanvasFire: this.world.dataset.flameTexture === 'volumetric-plasma',
      plasmaFilaments: true,
      cardEdgeBurst: this.world.dataset.birthCorona === 'card-edge',
      responsiveAltar: this.world.dataset.responsiveAltar === 'v508',
      volumetricPlasma: this.world.dataset.visualCalibration === 'sovereign-volume',
      cosmicNebula: true
    });
  }
  ...
  destroy() {
    ...
    this.fire?.destroy();
    if (globalThis.divinaTarotLivreV508 === this) delete globalThis.divinaTarotLivreV508;
