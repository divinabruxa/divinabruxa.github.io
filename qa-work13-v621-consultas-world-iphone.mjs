...
const css = await readFile(new URL('./consultas-world-v621.css',import.meta.url),'utf8');
...
for (const [id,width,height,dpr] of profiles) {
  ...
  const appWidth = width <= 430 ? width - 16 : Math.min(width - 16,960);
  const flowWidth = Math.min(appWidth,820);
  const servicesUseGrid = width >= 760;
  ...
  ok(appWidth > 320 && appWidth <= width, `${id}: templo contido`);
  ok(flowWidth > 0 && flowWidth <= appWidth, `${id}: pedido cabe sem corte`);
  ok(servicesUseGrid === (width >= 760), `${id}: quatro leituras usam a geometria prevista`);
...
ok(css.includes('width:min(calc(100% - 28px),1040px)'), 'largura máxima do templo');
ok(css.includes('width:min(820px,100%)'), 'pedido com limite legível');
ok(css.includes('width:min(780px,100%)'), 'protocolo com limite legível');
ok(css.includes('grid-template-columns:repeat(2,minmax(0,1fr))'), 'quatro leituras respiram em duas colunas');
ok(css.includes('flex-basis:min(86%,350px)'), 'uma leitura por gesto no retrato');
ok(css.includes('min-block-size:48px'), 'alvo de toque');
ok(css.includes('min-block-size:52px'), 'campos confortáveis');
...
ok(css.includes('content-visibility:auto') && css.includes('contain-intrinsic-size:420px'), 'protocolo abaixo da dobra adiado');
ok(css.includes('min-block-size:clamp(112px,20svh,176px)'), 'teclado mantém a pergunta visível');
ok(css.includes('@media(min-width:760px)'), 'corte de grade');
...
console.log(`PASS ${checks}/${checks} — Templo do Encontro em 8 geometrias iPhone`);
