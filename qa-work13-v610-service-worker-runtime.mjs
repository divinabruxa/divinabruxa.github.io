/* DIVINA BRUXA — WORK13 V613 · QA DO SERVICE WORKER ATÔMICO */
...
const cacheName = 'divina-bruxa-work13-v613-menu-global-vivo';
...
check('core:app-v613', coreAssets.includes('./app-v208.js?v=613-menu-global-vivo'));
...
check('activate:keeps-v613', harness.stores.has(cacheName));
...
]) check(`activate:${type}`, harness.clientMessages.some(item => item.type === type && item.version === 613));
...
check('fetch:navigation-network', (await onlineNavigation.text()).includes('name="divina-work13" content="V613"'));
...
for (const [file, needle] of [
  ['app-v208.js?v=613-menu-global-vivo','divinaWork13MenuGlobalVivoV613'],
...
check('message:status-version', harness.sourceMessages.some(item => item.type === 'WORK12_STATUS' && item.version === 613));
...
check('message:clear-confirmed', harness.sourceMessages.some(item => item.type === 'DIVINA_CACHES_CLEARED' && item.version === 613));

const failures = checks.filter(item => !item.pass);
console.log(JSON.stringify({
  release:'V613',
  work:'WORK13',
  macroStage:'WORK13 / menu-global-vivo-service-worker-runtime',
