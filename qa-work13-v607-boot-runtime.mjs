/* DIVINA BRUXA — WORK13 V607 · QA DO BOOT RECUPERAVEL */
...
function harness() {
  ...
  vm.runInContext(bootScript, vm.createContext(context), { filename:'work13-inline-boot-v607.js' });
...
check('boot:work13-meta-v607', indexSource.includes('name="divina-work13" content="V607"'));
check('boot:work12-meta-v600', indexSource.includes('name="divina-work12" content="V600"'));
check('boot:app-v607', indexSource.includes('app-v208.js?v=607-work13-living-wisdom'));
check('boot:worker-v607', indexSource.includes("register('./sw.js?v=607'"));
check('boot:v607-bootstrap', indexSource.includes("__divinaSWBootstrap='v607-work13-living-wisdom-inline'"));
check('boot:v607-style', indexSource.includes('living-wisdom-path-v607.css?v=607-work13-living-wisdom'));
...
for (const id of [
  'divinaLivingWisdomPathV607','divinaCosmicSpreadReadingV605',
  'divinaCosmicDailyReadingV604','divinaCosmosRealityResonanceV603',
  'divinaWork12FinalContinuityV598'
...
check('boot:watchdog-work13-v607', watchdog.root.dataset.work13Boot === 'released-v607');
...
check('boot:manual-work13', manual.root.dataset.work13Boot === 'released-v607');
...
check('boot:healthy-work13-ready', healthy.root.dataset.work13Boot === 'ready-v607');
check('boot:healthy-never-bypassed', !healthy.home.classList.contains('active'));
healthy.fire('load');
await Promise.resolve();
check('boot:registers-v607', healthy.registrations.some(item => item.url === './sw.js?v=607'));
...
console.log(JSON.stringify({
  release:'V607',
  work:'WORK13',
  macroStage:'7-of-10 / living-wisdom-boot-runtime',
