/*
 * DIVINA BRUXA — ORBE 2.0 V207 · LIVING MATTER + CICLO DE VIDA
 *
...
export const ORB_LIFE_STATES_V207 = Object.freeze({
  BOOT: 'BOOT',
  IDLE: 'IDLE',
  TOUCH: 'TOUCH',
  HOLD: 'HOLD',
...
const ORB_INSTANCE_KEY = Symbol.for('divina.reality.orb.instance.v207');
...
export class RealityOrbEngine {
  constructor(canvas, { onOpen } = {}) {
    ...
    this.lifeState = ORB_LIFE_STATES_V207.BOOT;
    this.resumeState = ORB_LIFE_STATES_V207.IDLE;
    if (this.shell) this.shell.dataset.orbLife = ORB_LIFE_STATES_V207.BOOT;
  ...
  setLifeState(next) {
    if (!Object.values(ORB_LIFE_STATES_V207).includes(next) || this.lifeState === next) return;
    this.lifeState = next;
  ...
  settleToIdle() {
    if (this.destroyed || this.lifeState === ORB_LIFE_STATES_V207.FALLBACK) return;
    if (this.lifeState === ORB_LIFE_STATES_V207.SUSPENDED) {
      this.resumeState = ORB_LIFE_STATES_V207.IDLE;
      return;
    }
    this.setLifeState(ORB_LIFE_STATES_V207.IDLE);
  }

  suspend() {
    if (this.destroyed || this.lifeState === ORB_LIFE_STATES_V207.FALLBACK) return;
    if (this.lifeState !== ORB_LIFE_STATES_V207.SUSPENDED) {
      this.resumeState = this.lifeState === ORB_LIFE_STATES_V207.PORTAL
        ? ORB_LIFE_STATES_V207.IDLE
        : this.lifeState;
    }
    if (this.down) this.cancelActivePointer();
    this.lastFrame = 0;
    this.setLifeState(ORB_LIFE_STATES_V207.SUSPENDED);
  }

  resume() {
    if (this.destroyed || this.lifeState !== ORB_LIFE_STATES_V207.SUSPENDED) return;
    const next = this.resumeState === ORB_LIFE_STATES_V207.TOUCH || this.resumeState === ORB_LIFE_STATES_V207.HOLD
      ? ORB_LIFE_STATES_V207.IDLE
      : this.resumeState;
    this.setLifeState(next || ORB_LIFE_STATES_V207.IDLE);
    this.lastFrame = 0;
    this.requestFrame();
  }
  ...
  bind() {
    ...
    if ('IntersectionObserver' in window) {
      this.intersectionObserver = new IntersectionObserver(entries => {
        ...
        this.visible = next;
        if (next) this.resume('intersection');
  ...
  moveTarget(event) {
    ...
    this.setCssVariable('--orb-touch-x', `${(point.x * 100).toFixed(2)}%`);
    this.setCssVariable('--orb-touch-y', `${(point.y * 100).toFixed(2)}%`);
    return point;
  }
  ...
  pointerDown(event) {
    ...
    this.setLifeState(ORB_LIFE_STATES_V207.TOUCH);
  ...
  releaseTouch(point) {
    ...
    this.ripple = { x: point.x, y: point.y, age: 0 };
    this.shell?.classList.remove('is-touching');
    this.setLifeState(ORB_LIFE_STATES_V207.RELEASE);
  ...
  open() {
    ...
    this.ripple = { x: this.pointer.targetX, y: this.pointer.targetY, age: 0 };
    this.setLifeState(ORB_LIFE_STATES_V207.PORTAL);
  ...
  update(time, seconds, elapsed) {
    ...
    if (this.down) {
      ...
      if (held > .72 && this.lifeState !== ORB_LIFE_STATES_V207.HOLD) {
        this.setLifeState(ORB_LIFE_STATES_V207.HOLD);
  ...
  fallback(reason = 'indisponível') {
    ...
    this.setLifeState(ORB_LIFE_STATES_V207.FALLBACK);
    this.announce('A Orbe está respirando', 'RESPIRA');
  }

  destroy() {
    if (this.destroyed) return;
    this.setLifeState(ORB_LIFE_STATES_V207.DESTROYED);
    this.destroyed = true;
    this.ready = false;
    this.stop();
    clearTimeout(this.phaseTimer);
