/* DIVINA BRUXA — FECHAMENTO SUPREMO · QA RUNTIME · MENU V631 */
...
globalThis.matchMedia=()=>({matches:prefersReducedMotion});
...
const menuModule=await import(`${pathToFileURL(path.join(root,'orbital-menu-v502.js')).href}?qa=v631-runtime-${Date.now()}`);
...
function makeFixture(screen='home'){
  ...
  const entryButton=new FakeElement('pentagram-entry');
  const legacy=new FakeElement('legacy-menu');
  ...
  Object.assign(instance,{
    core,go:()=>{},menuButton,entryButton,homeButton,homeButtonLabel:null,legacy,legacySnapshot:null,dockOrb:null,
    ...
    navigating:false,cycleBusy:false,intentTimer:0,discoveryCueTimer:0,discoveryCueReleaseTimer:0,discoveryCueShown:false,
    gesture:null,suppressSkyClickUntil:0,
    backgroundSnapshots:[],backgroundLocked:false,motionLocks:new Set(),menuMotionPaused:false
  });
  instance.silenceLegacyMenu();
  return {
    instance,documentTarget,appNode,dock,drawer,rootNode,host,homeHost,journeyHost,orb,buttons,intention,live,entryButton,legacy,
...
check('status:v631',status.release==='V631-FECHAMENTO-SUPREMO'&&status.version===631);
...
check('status:no-copy',status.oneLivingOrb===true&&status.newCanvases===0&&status.javascriptAnimationLoops===0&&status.permanentAnimationLoops===0);
check('status:work13-frozen',status.work13==='concluido-e-congelado'&&status.work14===false);
check('status:no-technical-copy',status.technicalMenuCopy===0);
check('status:legacy-silenced',status.legacyMenuSilenced===true);
check('status:origin-not-duplicated',status.duplicateOriginLabel===false&&status.visibleAdministrativeHeader===false);
...
const legacyState=makeFixture('home');
check('legacy:hidden-by-runtime',legacyState.legacy.hidden&&legacyState.legacy.inert&&legacyState.legacy.getAttribute('aria-hidden')==='true');
check('legacy:authority-recorded',legacyState.legacy.dataset.retiredBy==='v631-fechamento-supremo',legacyState.legacy.dataset.retiredBy);
check('legacy:restores-original-state',legacyState.instance.restoreLegacyMenu()===true&&!legacyState.legacy.hidden&&!legacyState.legacy.inert&&!legacyState.legacy.hasAttribute('aria-hidden'));

prefersReducedMotion=false;
const discovery=makeFixture('home');
check('discovery:open',await discovery.instance.open()===true);
await new Promise(resolve=>setTimeout(resolve,300));
check('discovery:one-shot-appears',discovery.instance.discoveryCueShown&&discovery.rootNode.classList.contains('is-discovery-cue'));
await new Promise(resolve=>setTimeout(resolve,210));
check('discovery:returns-to-silence',!discovery.rootNode.classList.contains('is-discovery-cue'));
check('discovery:does-not-repeat',discovery.instance.queueDiscoveryCue()===false);
await discovery.instance.close({reason:'qa-discovery-close'});
prefersReducedMotion=true;

for(const [index,route] of ROUTES.entries()){
  const reality=makeFixture('home');
  await reality.instance.open();
  reality.instance.setFrame(Math.floor(index/2),{reason:'qa-matrix'});
  const portal=reality.buttons.find(button=>button.dataset.v502Route===route);
  check(`matrix:${route}:visible`,portal?.hidden===false&&portal?.dataset.v593Visible==='true');
  check(`matrix:${route}:travels`,await reality.instance.activate(portal)===true&&reality.lastNavigation?.route===route);
  check(`matrix:${route}:same-orb`,reality.journeyHost.contains(reality.orb)&&reality.instance.releaseOrb===null);
  clearTimeout(reality.instance.intentTimer);
}

const stress=makeFixture('home');
let stressPasses=0;
for(let index=0;index<30;index+=1){
  const opened=await stress.instance.open();
  const closed=await stress.instance.close({reason:`qa-stress-${index}`});
  if(opened&&closed&&stress.homeHost.contains(stress.orb)&&stress.instance.motionLocks.size===0)stressPasses+=1;
}
check('stress:thirty-open-close',stressPasses===30,stressPasses);
check('stress:no-timer-leak',stress.instance.discoveryCueTimer===0&&stress.instance.discoveryCueReleaseTimer===0);
clearTimeout(stress.instance.intentTimer);

check('performance:all-pauses-resumed',universePauses===universeStarts,`${universePauses}/${universeStarts}`);

const failures=checks.filter(item=>!item.pass);
console.log(JSON.stringify({
  release:'V631',phase:'FECHAMENTO-SUPREMO',scope:'menu-runtime',
