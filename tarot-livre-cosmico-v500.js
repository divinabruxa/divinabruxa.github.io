/* DIVINA BRUXA — TAROT LIVRE COSMOS DE FOGO V500
   Um novo mundo construído para a rota Tarot Livre.
   WebGL adaptativo + partículas Canvas, com fallback 2D e continuidade local.
   Regras imutáveis: 78 cartas diretas, nenhuma repetição e seis por fileira. */
...
class CosmicFireEngineV500 {
  constructor({ cosmos, effects, stage, orb, card }) {
    ...
    this.orb = orb;
    this.card = card;
    this.world = stage.closest('.tl500');
    ...
    this.intersectionObserver = globalThis.IntersectionObserver
      ? new IntersectionObserver(entries => {
          ...
          this.visible = Boolean(entry?.isIntersecting) && !document.hidden;
          if (this.visible) this.start();
          else this.pause();
        }, { rootMargin: '120px' })
      : null;
...
export class TarotLivreCosmicoV500 {
  constructor(root, { storage = store } = {}) {
    ...
    this.root.innerHTML = '';
    this.root.dataset.tarotWorld = 'cosmos-v500';
    ...
    this.bind();
    this.render(false);
    this.fire = new CosmicFireEngineV500({
    ...
    document.documentElement.dataset.tarotLivre = 'v500';
    document.dispatchEvent(new CustomEvent('divina:tarot-cosmico-ready', {
      detail: Object.freeze({
        version: VERSION,
        engine: 'CosmicFireEngineV500',
  ...
  build() {
    this.root.innerHTML = `
      <div class="tl500" data-phase="idle" data-engine="pending">
        <header class="tl500__header">
          <div class="tl500__title">
            <small>ORBE DAS REALIDADES</small>
            <h2>Tarot Livre</h2>
          </div>

          <a class="tl500__mesa" href="#spreads" data-go="spreads" aria-label="Abrir Tiragens e Mesa Real">
            <span aria-hidden="true">✦</span>
            Mesa Real
          </a>

          <div class="tl500__toolbar" aria-label="Controles do Tarot Livre">
        ...
        <section class="tl500__stage" data-stage aria-label="Tarot Livre — Cosmos de Fogo">
          <canvas class="tl500__canvas tl500__cosmos" data-cosmos aria-hidden="true"></canvas>
          <canvas class="tl500__canvas tl500__effects" data-effects aria-hidden="true"></canvas>

          <div class="tl500__portal-crown" aria-hidden="true"><i></i><i></i><i></i></div>

          <div class="tl500__card-zone" data-card-zone tabindex="0"
               aria-label="Carta atual. Use as setas para navegar pelas cartas reveladas.">
            <div class="tl500__card" data-current-card data-empty="true">
              <canvas class="tl500__card-back" data-card-back aria-hidden="true"></canvas>
            </div>
          </div>

          <nav class="tl500__card-nav" data-card-nav aria-label="Navegar pelas cartas reveladas">
          ...
          <button class="tl500__orb" type="button" data-reveal aria-label="Revelar uma carta pela Orbe">
            <span class="tl500__orb-image" data-orb-surface="tarot"></span>
            <span class="tl500__orb-light" aria-hidden="true"></span>
            <span class="tl500__orb-touch" aria-hidden="true"></span>
          </button>

          <p class="tl500__counter" aria-label="Cartas reveladas">
            <b data-count>0</b><span>/78</span>
          </p>
          <p class="tl500__signal" data-signal aria-live="polite">Toque na Orbe para revelar seu Tarot.</p>
        </section>

        <section class="tl500__constellation" aria-labelledby="tl500ConstellationTitle">
          <header>
            <div>
              <small>SEU CÍRCULO</small>
              <h3 id="tl500ConstellationTitle">Cartas reveladas</h3>
            </div>
            <span data-remaining>78 aguardam</span>
          </header>
          <div class="tl500__grid" data-grid role="grid"
               aria-label="Cartas reveladas, seis por fileira" aria-colcount="6"></div>
          <p class="tl500__empty-grid" data-empty-grid>Nenhuma carta foi revelada ainda.</p>
        </section>

        <p class="tl500__sr" data-live role="status" aria-live="polite" aria-atomic="true"></p>
      </div>`;

    this.world = this.root.querySelector('.tl500');
  ...
  bind() {
    ...
    this.revealButton.addEventListener('pointerdown', event => {
      ...
      this.revealButton.style.setProperty('--tl500-touch-x', `${x}%`);
      this.revealButton.style.setProperty('--tl500-touch-y', `${y}%`);
  ...
  async transitionTo(index) {
    ...
    if (previous && !reducedMotion()) {
      ...
      ], { duration: 250, easing: 'cubic-bezier(.55,.02,.78,.35)', fill: 'forwards' });
  ...
  async draw() {
    ...
    try {
      ...
      this.fire?.ignite(0.78);
      await this.transitionTo(result.position);
  ...
  show(index, animate = false) {
    ...
    this.card.innerHTML = `${cardImageMarkup(card, {
      ...
      priority: 'high'
    })}<div class="tl500__card-name"><b>${escapeText(card.name)}</b><small>${index + 1} / ${this.state.revealed.length}</small></div>`;
  ...
  render(animateCurrent = false) {
    ...
    if (!revealed) {
      ...
      this.card.innerHTML = '<canvas class="tl500__card-back" data-card-back aria-hidden="true"></canvas>';
