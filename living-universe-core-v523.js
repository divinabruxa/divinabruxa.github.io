/* DIVINA BRUXA — UNIVERSO VIVO · MACROETAPA 1/4 · ATMOSFERA VIVA RETINA V523
   Um único céu procedural respira atrás de todas as realidades. Nuvens de voz,
   galáxias, poeira e constelações ocupam profundidades diferentes e respondem
   ao toque sem sacrificar a resolução estável nem fazer as estrelas piscarem. */

const VERSION = 523;
const STYLE_ID = 'divinaLivingUniverseV523Styles';
const ROOT_ID = 'divinaLivingUniverseV523';
...
function installStyle() {
  ...
  link.id = STYLE_ID;
  link.rel = 'stylesheet';
  link.href = './living-universe-core-v523.css?v=523';
...
class LivingUniverseCoreV523 {
  constructor() {
    ...
    this.root.className = 'db523-universe';
    this.root.setAttribute('aria-hidden', 'true');
    this.root.innerHTML = '<canvas data-living-universe-canvas></canvas><span class="db523-universe__veil"></span>';
    ...
    document.documentElement.dataset.livingUniverse = 'v523';
    document.body.classList.remove('db516-universe-active','db519-universe-active','db520-universe-active','db521-universe-active','db522-universe-active');
    document.body.classList.add('db523-universe-active');
    ...
    try {
      ...
      const fragment = this.createShader(this.gl.FRAGMENT_SHADER, `
        ...
        void main(){
          ...
          vec2 touchFlow=touchTangent*touchField*(0.020+velocityStrength*0.064)
            *(0.76+touchWave*0.24)
            +touchVelocity*touchField*0.22
          ...
          float cosmosCurrentA=noise2(
            cosmosDomain*1.42+vec2(time*0.015,-time*0.011)+u_seed*5.1
          );
          float cosmosCurrentB=noise2(
            cosmosDomain*1.63+vec2(-time*0.012,time*0.017)+u_seed*8.7
          );
          cosmosUv+=vec2(
            cosmosCurrentA-cosmosCurrentB,
            cosmosCurrentA+cosmosCurrentB-1.0
          )*(0.0072+u_density*0.0018);
          cosmosUv-=vec2(camera.x/max(aspect,0.08),camera.y)*0.18;
          cosmosUv+=vec2(touchFlow.x/max(aspect,0.08),touchFlow.y)*0.30;
          cosmosUv+=vec2(
            sin(time*0.021+u_seed*8.0),
            cos(time*0.017+u_seed*5.0)
          )*0.006*u_drift;
          ...
          vec3 cosmosTint=cosmosSample*(0.84+u_accent*0.22);
          cosmosTint+=u_gold*smoothstep(0.28,0.92,cosmosSample.r)*0.025*u_warmth;
          cosmosSample=mix(cosmosSample,cosmosTint,0.28);
          ...
          vec3 color=vec3(0.0022,0.00065,0.0062);
  ...
  status() {
    return Object.freeze({
      ...
      retinaTextureSize:this.cosmos.image?.naturalWidth
        ...
        : null,
      globalAcrossRoutes:true,
      oneUniverseCanvas:document.querySelectorAll('#divinaLivingUniverseV523 canvas').length === 1,
      targetFps:this.targetFps,
...
export function createLivingUniverseV523() {
  if (globalThis.divinaLivingUniverseV523?.status) return globalThis.divinaLivingUniverseV523;
  const former = globalThis.divinaLivingUniverseV522 || globalThis.divinaLivingUniverseV521 || globalThis.divinaLivingUniverseV520 || globalThis.divinaLivingUniverseV519 || globalThis.divinaLivingUniverseV516;
  ...
  document.body?.classList.remove('db516-universe-active','db519-universe-active','db520-universe-active','db521-universe-active','db522-universe-active');
  const universe = new LivingUniverseCoreV523();
