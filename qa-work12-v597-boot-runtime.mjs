/* DIVINA BRUXA — WORK12 · QA BOOT RECUPERÁVEL · V597 */
...
function harness(){
  ...
  vm.runInContext(bootScript,vm.createContext(context),{filename:'work12-inline-boot-v597.js'});
...
check('boot:app-v597',indexSource.includes('app-v208.js?v=597-work12-intelligence'));
check('boot:worker-v597',indexSource.includes("register('./sw.js?v=597'"));
...
check('boot:watchdog-v597',watchdog.root.dataset.work12Boot==='released-v597'&&watchdog.root.dataset.work12BootReason==='boot-watchdog');
check('boot:watchdog-event-v597',watchdog.events.some(event=>event.type==='divina:work12-boot-release'&&event.detail.version===597));
check('boot:recovery-source-v597',watchdog.events.some(event=>event.type==='divina:loading-bypass'&&event.detail.source==='work12-intelligence-v597'));
...
check('boot:healthy-never-bypassed',healthy.root.dataset.work12Boot==='ready-v597'&&!healthy.home.classList.contains('active'));

const failures=checks.filter(item=>!item.pass);
console.log(JSON.stringify({release:'V597',work:'WORK12',macroStage:'9-of-10 / boot-runtime',state:failures.length?'FAIL':'PASS',passed:checks.length-failures.length,failed:failures.length,total:checks.length,failures:failures.map(({id,detail})=>({id,detail}))},null,2));
