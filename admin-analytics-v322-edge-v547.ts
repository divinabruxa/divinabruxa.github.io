/* DIVINA BRUXA — ADMIN ANALYTICS V322 · HARDENING V547
   Owner-only aggregate snapshot. No private text, contact data, prompts or questions returned. */
...
async function snapshot(context:any){
  const db=context.db;
  const [profilesCount,dailyRows,aiRows,purchaseRows,consultRows,notificationRows,videoRows,eventRows]=await Promise.all([
  ...
  return {
    release:'V322',securityRelease:'V547',environment:'staging',generatedAt:new Date().toISOString(),sanitized:true,privateContentIncluded:false,
    coverage:{analyticsEvents:events.length,analyticsSampleLimited:events.length>=5000,countryEvents:countries.length,countrySignalAvailable:countries.length>0,regionSignalAvailable:false,observedAccountActivitySources:['daily_cards','ai_usage','purchases','consultation_requests']},
    audience:{registeredAccounts:integer(profilesCount.count),observedActiveAccounts:{d1:active(1),d7:active(7),d30:active(30)},officialDauWauMauAvailable:false},
    ...
    analytics:{eventsByKey:sortedPairs(group(events,'event_key'),30),funnel:sortedPairs(group(events,'funnel_stage'),20),platforms:sortedPairs(group(events,'platform'),10),locales:sortedPairs(group(events,'locale'),10),products:sortedPairs(group(events,'product_key'),20)},
    ...
    privacy:{journalBodies:false,journalQuestions:false,aiPrompts:false,aiResponses:false,consultationQuestions:false,names:false,emails:false,phones:false,preciseLocation:false}
  };
}
...
Deno.serve(async(request:Request)=>{
  ...
  if(!['v322','v547'].includes(String(request.headers.get('x-divina-admin-request')||'')))return json(403,{error:'request_guard_denied'},origin);
  try{
    ...
    await context.db.from('admin_audit_events').insert({actor_user_id:context.user.id,action:'analytics-snapshot-read',module_id:'analytics',result:'allowed',metadata:{release:'V322',securityRelease:'V547',sanitized:true}});
    return json(200,data,origin);
  }catch(error){console.error('admin-analytics-v547',error instanceof Error?error.name:'unknown');return json(500,{error:'analytics_snapshot_failed'},origin);}
});
