...
ok(html.includes('name="divina-work13-correction" content="V621-CONSULTAS-TEMPLO-DO-ENCONTRO"'), 'renovação permanece no WORK13');
...
ok(html.includes('app-v208.js?v=621-consultas-templo-do-encontro'), 'aplicação ligada');
ok(html.includes("./sw.js?v=621-consultas-templo-do-encontro"), 'worker ligado');
...
ok(count(core, /^\s*'\.\//gm) === 70, '70 ativos centrais');
ok(sw.includes("CACHE_NAME = 'divina-bruxa-work13-v621-consultas-templo-do-encontro'"), 'cache isolado');
...
console.log(`PASS ${checks}/${checks} — menu V613 preservado dentro do WORK13 V621`);
