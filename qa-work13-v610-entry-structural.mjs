...
ok(html.includes('name="divina-work13-correction" content="V620-WHIT-PRESENCA-ENTRE-MUNDOS"'), 'renovação permanece no WORK13');
...
ok(html.includes('app-v208.js?v=620-whit-presenca-entre-mundos'), 'aplicação ligada');
ok(html.includes("./sw.js?v=620-whit-presenca-entre-mundos"), 'worker ligado');
...
ok(count(core, /^\s*'\.\//gm) === 68, '68 ativos centrais');
ok(sw.includes("CACHE_NAME = 'divina-bruxa-work13-v620-whit-presenca-entre-mundos'"), 'cache isolado');
...
console.log(`PASS ${checks}/${checks} — menu V613 preservado dentro do WORK13 V620`);
