...
ok(html.includes('name="divina-work13-correction" content="V619-DIARIO-CAMARA-DA-TINTA-VIVA"'), 'renovação permanece no WORK13');
...
ok(html.includes('app-v208.js?v=619-diario-camara-da-tinta-viva'), 'aplicação ligada');
ok(html.includes("./sw.js?v=619-diario-camara-da-tinta-viva"), 'worker ligado');
...
ok(count(core, /^\s*'\.\//gm) === 66, '66 ativos centrais');
ok(sw.includes("CACHE_NAME = 'divina-bruxa-work13-v619-diario-camara-tinta-viva'"), 'cache isolado');
...
console.log(`PASS ${checks}/${checks} — menu V613 preservado dentro do WORK13 V619`);
