...
const css = await readFile(new URL('./loja-world-v622.css',import.meta.url),'utf8');
...
for (const [id,width,height,dpr] of profiles) {
  ...
  const appWidth = width <= 430 ? width - 16 : Math.min(width - 28,1120);
  const catalogWidth = Math.min(appWidth,1080);
  const productColumns = width <= 720 ? 1 : 2;
  const intentionGrid = width >= 760;
  ...
  ok(appWidth > 320 && appWidth <= width, `${id}: casa contida`);
  ok(catalogWidth > 0 && catalogWidth <= appWidth, `${id}: catálogo cabe sem corte`);
  ok([1,2].includes(productColumns), `${id}: produtos usam a geometria prevista`);
  ok(intentionGrid === (width >= 760), `${id}: caminhos preservam um gesto por vez no retrato`);
...
ok(css.includes('width:min(calc(100% - 32px),780px)'), 'título com limite legível');
ok(css.includes('width:min(calc(100% - 28px),1120px)'), 'largura máxima da casa');
ok(css.includes('width:min(1080px,100%)'), 'catálogo com limite legível');
ok(css.includes('grid-template-columns:repeat(2,minmax(0,1fr))'), 'caminhos e catálogo respiram em duas colunas');
ok(css.includes('grid-template-columns:1fr!important'), 'produtos voltam a uma coluna no iPhone retrato');
ok(css.includes('flex-basis:min(86%,350px)'), 'um caminho por gesto no retrato');
ok(css.includes('min-block-size:48px'), 'alvo de toque');
ok(css.includes('min-block-size:50px') && css.includes('min-block-size:52px'), 'busca e passagem confortáveis');
...
ok(css.includes('content-visibility:auto') && css.includes('contain-intrinsic-size:440px'), 'produtos abaixo da dobra adiados');
ok(css.includes('@media(min-width:760px)'), 'corte de grade de caminhos');
ok(css.includes('@media(max-width:720px)'), 'corte de catálogo móvel');
ok(css.includes('@media(max-width:430px)'), 'corte iPhone');
...
console.log(`PASS ${checks}/${checks} — Casa das Escolhas Vivas em 8 geometrias iPhone`);
