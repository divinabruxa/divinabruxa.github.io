/* DIVINA BRUXA — WORK12 · QA SERVICE WORKER · V596 */
...
const sourceFiles=Object.freeze({
  index:'./index.html',app:'./app-v208.js',chambers:'./reality-chambers-v596.js',
  'chamber-styles':'./reality-chambers-v596.css',school:'./school-world-v306.js',
  journal:'./journal-world-v317.js',ritual:'./reading-ritual-core-v595.js',
  'ritual-styles':'./reading-ritual-core-v595.css',daily:'./daily-world-v509.js',
  whit:'./whit-living-presence-v594.js',menu:'./orbital-menu-v502.js',
  'menu-styles':'./orbital-menu-v502.css',navigation:'./navigation.js',
  foundation:'./work12-foundation-v589.js','page-loader':'./page-loader-v1.js',
  universe:'./living-universe-core-v524.js',orb:'./supreme-orb-core-v501.js',
  renderer:'./orb-engine-v208.js',journey:'./orb-persistent-journey-v565.js',
  soul:'./whit-orb-soul-bridge-v581.js',shell:'./divina-shell-v180.css',
...
const assets=Object.freeze({
  './index.html':'index',
  './app-v208.js?v=596-work12-chambers':'app',
...
function createHarness({broken=null}={}){
  ...
  const paths=new Map(coreAssets.map(path=>[path,broken===assets[path]?`/* broken ${broken} */`:sources[assets[path]]]));
  const caches={
  ...
  vm.runInContext(source,vm.createContext({self,caches,fetch,Request:FakeRequest,URL,Object,Map,Set,Promise,Error,RegExp,console}),{filename:'sw-v596.js'});
...
const cacheName='divina-bruxa-work12-v596-reality-chambers';
...
check('install:twenty-two-core-assets',cache?.size===22,cache?.size);
...
harness.stores.set('divina-bruxa-work12-v595-reading-ritual',new Map());
await harness.wait('activate');
check('activate:claims-clients',harness.counters().claimed===1);
check('activate:keeps-v596',harness.stores.has(cacheName));
check('activate:deletes-v595',!harness.stores.has('divina-bruxa-work12-v595-reading-ritual'));
for(const type of ['DIVINA_WORK12_FOUNDATION_ACTIVE','DIVINA_WORK12_NAVIGATION_ACTIVE','DIVINA_WORK12_UNIVERSE_ACTIVE','DIVINA_WORK12_ORB_ACTIVE','DIVINA_WORK12_MENU_ACTIVE','DIVINA_WORK12_WHIT_ACTIVE','DIVINA_WORK12_RITUAL_ACTIVE','DIVINA_WORK12_CHAMBERS_ACTIVE','DIVINA_RELEASE_READY']){
  check(`activate:${type}`,harness.clientMessages.some(item=>item.type===type&&item.version===596));
}

const onlineNavigation=await harness.fetchEvent(new FakeRequest('https://divina.test/#school',{mode:'navigate'}));
check('fetch:navigation-network',(await onlineNavigation.text()).includes('content="V596"'));
...
check('fetch:navigation-offline',(await offlineNavigation.text()).includes('content="V596"'));
harness.offline(false);

for(const [file,needle] of [
  ['app-v208.js?v=596-work12-chambers','divinaWork12Macro8V596'],
  ['reality-chambers-v596.js?v=596-work12-chambers','REALITY_CHAMBERS_CONTRACT_V596'],
  ['reality-chambers-v596.css?v=596-work12-chambers','db596-school-paths'],
  ['school-world-v306.js?v=596-work12-chambers',"dataset.realityChambers==='v596'"],
  ['journal-world-v317.js?v=596-work12-chambers',"dataset.realityChambers === 'v596'"],
  ...
  ['page-loader-v1.js?v=592-work12-navigation','school-world-v306.js?v=596-work12-chambers'],
...
check('message:status-version',harness.sourceMessages.some(item=>item.type==='WORK12_STATUS'&&item.version===596));
check('message:status-twenty-two-assets',harness.sourceMessages.some(item=>item.type==='WORK12_STATUS'&&item.core?.length===22));
await harness.message({type:'CLEAR_DIVINA_CACHES'});
check('message:clear-caches',!harness.stores.has(cacheName));
check('message:clear-confirmed',harness.sourceMessages.some(item=>item.type==='DIVINA_CACHES_CLEARED'&&item.version===596));

const failures=checks.filter(item=>!item.pass);
console.log(JSON.stringify({release:'V596',work:'WORK12',macroStage:'8-of-10 / service-worker-runtime',state:failures.length?'FAIL':'PASS',passed:checks.length-failures.length,failed:failures.length,total:checks.length,failures:failures.map(({id,detail})=>({id,detail}))},null,2));
