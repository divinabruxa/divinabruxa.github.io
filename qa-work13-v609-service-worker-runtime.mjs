/* DIVINA BRUXA — WORK13 V609 · QA DO SERVICE WORKER ATÔMICO */
...
const cacheName = 'divina-bruxa-work13-v609-media-skins';
const harness = createHarness();

check('core:forty-assets-declared', coreAssets.length === 40, coreAssets.length);
check('core:unique-assets', new Set(coreAssets).size === coreAssets.length);
check('core:app-v609', coreAssets.includes('./app-v208.js?v=609-work13-media-skins'));
...
check('install:forty-core-assets', cache?.size === 40, cache?.size);
...
harness.stores.set('divina-bruxa-work13-v608-commerce-clarity', new Map());
await harness.wait('activate');
check('activate:claims-clients', harness.counters().claimed === 1);
check('activate:keeps-v609', harness.stores.has(cacheName));
check('activate:deletes-v608-cache', !harness.stores.has('divina-bruxa-work13-v608-commerce-clarity'));
for (const type of [
...
]) check(`activate:${type}`, harness.clientMessages.some(item => item.type === type && item.version === 609));
...
check('fetch:navigation-network', (await onlineNavigation.text()).includes('name="divina-work13" content="V609"'));
...
for (const [file, needle] of [
  ['app-v208.js?v=609-work13-media-skins','divinaWork13Macro9V609'],
...
check('message:status-version', harness.sourceMessages.some(item => item.type === 'WORK12_STATUS' && item.version === 609));
check('message:status-forty-assets', harness.sourceMessages.some(item => item.core?.length === 40));
await harness.message({ type:'CLEAR_DIVINA_CACHES' });
check('message:clear-caches', !harness.stores.has(cacheName));
check('message:clear-confirmed', harness.sourceMessages.some(item => item.type === 'DIVINA_CACHES_CLEARED' && item.version === 609));

const failures = checks.filter(item => !item.pass);
console.log(JSON.stringify({
  release:'V609',
  work:'WORK13',
  macroStage:'9-of-10 / media-skins-service-worker-runtime',
