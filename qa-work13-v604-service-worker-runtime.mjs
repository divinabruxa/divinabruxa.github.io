/* DIVINA BRUXA — WORK13 V604 · QA SERVICE WORKER ATOMICO */
...
function createHarness({ brokenAsset = '' } = {}) {
  ...
  const paths = new Map(coreAssets.map(asset => [
    asset,
    asset === brokenAsset ? `/* broken ${asset} */` : sources[asset]
  ]));
  const caches = {
    async open(name) {
  ...
  const fetch = async input => {
    ...
    if (request.mode === 'navigate' || url.pathname === '/' || url.pathname.endsWith('/index.html')) {
      return new FakeResponse(sources['./index.html']);
    }
...
const cacheName = 'divina-bruxa-work13-v604-layered-daily-reading';
const harness = createHarness();

check('core:thirty-one-assets-declared', coreAssets.length === 31, coreAssets.length);
check('core:app-v604', coreAssets.includes('./app-v208.js?v=604-work13-daily-reading'));
check('core:reading-js', coreAssets.includes('./cosmic-daily-reading-v604.js?v=604-work13-daily-reading'));
check('core:reading-css', coreAssets.includes('./cosmic-daily-reading-v604.css?v=604-work13-daily-reading'));
...
check('install:thirty-one-core-assets', cache?.size === 31, cache?.size);
...
const validatedAssets = coreAssets.filter(asset => ![
...
harness.stores.set('divina-bruxa-work13-v603-reality-resonance', new Map());
await harness.wait('activate');
check('activate:claims-clients', harness.counters().claimed === 1);
check('activate:keeps-v604', harness.stores.has(cacheName));
check('activate:deletes-v603-cache', !harness.stores.has('divina-bruxa-work13-v603-reality-resonance'));
for (const type of [
  ...
  'DIVINA_WORK13_DAILY_READING_ACTIVE','DIVINA_RELEASE_READY'
]) check(`activate:${type}`, harness.clientMessages.some(item => item.type === type && item.version === 604));
...
check('activate:reading-version-v604', harness.clientMessages.some(item => item.type === 'DIVINA_WORK13_DAILY_READING_ACTIVE' && item.dailyReadingVersion === 604));
check('activate:ritual-version-v595', harness.clientMessages.some(item => item.type === 'DIVINA_WORK13_DAILY_READING_ACTIVE' && item.ritualVersion === 595));

const onlineNavigation = await harness.fetchEvent(new FakeRequest('https://divina.test/#daily', { mode:'navigate' }));
check('fetch:navigation-network', (await onlineNavigation.text()).includes('name="divina-work13" content="V604"'));
...
for (const [file, needle] of [
  ['app-v208.js?v=604-work13-daily-reading','divinaWork13Macro4V604'],
  ['cosmic-daily-reading-v604.js?v=604-work13-daily-reading','COSMIC_DAILY_READING_CONTRACT_V604'],
  ['cosmic-daily-reading-v604.css?v=604-work13-daily-reading','db604-daily-essence'],
  ...
  ['daily-world-v509.js?v=595-work12-ritual','readingRitualAuthority'],
...
check('message:status-version', harness.sourceMessages.some(item => item.type === 'WORK12_STATUS' && item.version === 604));
check('message:status-thirty-one-assets', harness.sourceMessages.some(item => item.type === 'WORK12_STATUS' && item.core?.length === 31));
await harness.message({ type:'CLEAR_DIVINA_CACHES' });
check('message:clear-caches', !harness.stores.has(cacheName));
check('message:clear-confirmed', harness.sourceMessages.some(item => item.type === 'DIVINA_CACHES_CLEARED' && item.version === 604));

const failures = checks.filter(item => !item.pass);
console.log(JSON.stringify({
  release:'V604',
  work:'WORK13',
  macroStage:'4-of-10 / layered-daily-reading-service-worker-runtime',
