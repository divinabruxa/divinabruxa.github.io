/* DIVINA BRUXA — WORK13 V609 · QA DO BOOT RECUPERÁVEL */
...
function harness() {
  ...
  vm.runInContext(bootScript, vm.createContext(context), { filename:'work13-inline-boot-v609.js' });
...
check('boot:work13-meta-v609', indexSource.includes('name="divina-work13" content="V609"'));
check('boot:work12-meta-v600', indexSource.includes('name="divina-work12" content="V600"'));
check('boot:app-v609', indexSource.includes('app-v208.js?v=609-work13-media-skins'));
check('boot:worker-v609', indexSource.includes("register('./sw.js?v=609'"));
check('boot:v609-bootstrap', indexSource.includes("__divinaSWBootstrap='v609-work13-media-skins-inline'"));
check('boot:v609-style', indexSource.includes('living-media-skins-v609.css?v=609-work13-media-skins'));
...
check('boot:watchdog-work13-v609', watchdog.root.dataset.work13Boot === 'released-v609');
...
check('boot:manual-work13', manual.root.dataset.work13Boot === 'released-v609');
...
check('boot:healthy-work13-ready', healthy.root.dataset.work13Boot === 'ready-v609');
check('boot:healthy-never-bypassed', !healthy.home.classList.contains('active'));
healthy.fire('load');
await Promise.resolve();
check('boot:registers-v609', healthy.registrations.some(item => item.url === './sw.js?v=609'));
...
console.log(JSON.stringify({
  release:'V609',
  work:'WORK13',
  macroStage:'9-of-10 / media-skins-boot-runtime',
