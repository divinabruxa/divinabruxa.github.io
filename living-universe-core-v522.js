/* DIVINA BRUXA — UNIVERSO VIVO · MACROETAPA 1/4 · CÉU RETINA ESTÁVEL V522
   Um único céu procedural e tátil respira atrás de todas as realidades.
   A resolução permanece estável durante a sessão; nuvens orgânicas carregam o
   movimento e as estrelas preservam luz serena, sem pisca-pisca. */

const VERSION = 522;
const STYLE_ID = 'divinaLivingUniverseV522Styles';
const ROOT_ID = 'divinaLivingUniverseV522';
...
function installStyle() {
  ...
  link.id = STYLE_ID;
  link.rel = 'stylesheet';
  link.href = './living-universe-core-v522.css?v=522';
...
class LivingUniverseCoreV522 {
  constructor() {
    ...
    this.root.className = 'db522-universe';
    this.root.setAttribute('aria-hidden', 'true');
    this.root.innerHTML = '<canvas data-living-universe-canvas></canvas><span class="db522-universe__veil"></span>';
    ...
    document.documentElement.dataset.livingUniverse = 'v522';
    document.body.classList.remove('db516-universe-active','db519-universe-active','db520-universe-active','db521-universe-active');
    document.body.classList.add('db522-universe-active');
    ...
    try {
      ...
      const fragment = this.createShader(this.gl.FRAGMENT_SHADER, `
        ...
        float galaxy(vec2 p,vec2 center,float size,float phase,float time){
          ...
          float breathing=0.985+0.015*sin(time*0.065+phase*2.0);
          float spiral=0.5+0.5*cos(angle*2.0+radius*9.8-phase-time*0.021);
          float arm=pow(max(spiral,0.0),8.2);
          float disk=exp(-radius*2.82)*breathing;
          float core=exp(-radius*radius*22.0);
          float dust=0.72+0.28*noise2(d*15.0+phase+time*0.002);
          return (disk*(0.055+arm*0.48)*dust+core*0.62)
        ...
        float starLayer(vec2 p,float scale,float seed,float time,out float giant){
          ...
          float rareBreath=1.0+giant*0.035*sin(time*0.18+random*29.0);
          float luminance=mix(0.78,1.0,hash21(id+19.7))*rareBreath;
          ...
          return core*luminance+halo*0.095+(vertical+horizontal)*0.34*rareBreath;
        }
        ...
        void livingConstellations(vec2 point,float aspect,float time,out float lines,out float stars){
          ...
          if(abs(crown.x)<0.18*constellationScale.x+0.02
            ...
            stars+=constellationStar(crown,c0,0.97+0.03*sin(time*0.18+0.2));
            stars+=constellationStar(crown,c1,0.97+0.03*sin(time*0.16+1.3));
            stars+=constellationStar(crown,c2,0.97+0.03*sin(time*0.19+2.5));
            stars+=constellationStar(crown,c3,0.97+0.03*sin(time*0.15+3.4));
            stars+=constellationStar(crown,c4,0.97+0.03*sin(time*0.17+4.6));
          }
          ...
          if(abs(river.x)<0.145*constellationScale.x+0.02
            ...
            stars+=constellationStar(river,r0,0.97+0.03*sin(time*0.16+0.8));
            stars+=constellationStar(river,r1,0.97+0.03*sin(time*0.19+1.7));
            stars+=constellationStar(river,r2,0.97+0.03*sin(time*0.17+2.8));
            stars+=constellationStar(river,r3,0.97+0.03*sin(time*0.18+3.9));
            stars+=constellationStar(river,r4,0.97+0.03*sin(time*0.15+4.8));
            stars+=constellationStar(river,r5,0.97+0.03*sin(time*0.17+5.7));
          }
        }
        ...
        void main(){
          ...
          float touchField=exp(-touchDistance*3.15)*u_touch_energy;
          float touchWave=sin(touchDistance*8.5-time*0.44+u_seed*9.0);
          vec2 touchFlow=touchTangent*touchField*(0.013+velocityStrength*0.038)
            *(0.78+touchWave*0.22)
            +touchVelocity*touchField*0.15
            +touchNormal*touchWave*touchField*0.0045;
          vec2 p=base-camera*0.052-vec2(0.0,u_scroll*0.026)+gravity+touchFlow;
          vec2 slow=vec2(time*0.0032*u_drift,-time*0.0024*u_drift);

          vec2 flow=vec2(
            fbm(p*1.24+slow+u_seed*9.1),
            fbm(p*1.19-slow+vec2(4.8,1.7)+u_seed*5.3)
          )-0.5;
          vec2 fluid=p+flow*0.31;
          ...
          float ribbonLeft=exp(-abs(fluid.x+aspect*0.38
            +sin(fluid.y*4.6+flow.x*2.0+time*0.014)*0.062)*11.0);
          float ribbonRight=exp(-abs(fluid.x-aspect*0.40
            +cos(fluid.y*4.1-flow.y*1.8-time*0.012)*0.068)*10.5);
          float ribbons=(ribbonLeft+ribbonRight)*(0.38+detail*0.62);
          float plumePathA=abs(fluid.x
            +sin(fluid.y*3.35+flow.x*3.1+time*0.026)*aspect*0.28);
          float plumePathB=abs(fluid.x
            -cos(fluid.y*2.85-flow.y*2.7-time*0.021)*aspect*0.31);
          ...
          float voiceThread=(1.0-smoothstep(0.006,0.024,plumePathA))
            ...
            *(0.32+filament*0.68);
          vec2 cloudA=fluid-vec2(-aspect*0.47+sin(time*0.041+u_seed)*0.035,-0.34);
          vec2 cloudB=fluid-vec2(aspect*0.49+cos(time*0.036+u_seed)*0.032,-0.19);
          vec2 cloudC=fluid-vec2(-aspect*0.51,0.09+sin(time*0.033)*0.040);
          vec2 cloudD=fluid-vec2(aspect*0.50,0.26+cos(time*0.038)*0.038);
          vec2 cloudE=fluid-vec2(-aspect*0.38+sin(time*0.029)*0.042,0.43);
          ...
          float cloudResponse=1.0+touchField*(0.16+velocityStrength*0.10);
          float nebula=(veil*0.62+filament*0.35+ribbons*0.28+silk*0.15+voiceCloud*0.18)
            *mix(0.48,1.12,edge)*u_density*(1.0-darkDust*0.28)*breath*cloudResponse;
          ...
          vec3 color=vec3(0.0016,0.00045,0.0044);
          color+=u_accent*nebula*(0.18+filament*0.40+silk*0.18)
            *(0.76+micro*0.24);
          color+=mix(u_accent,u_light,0.42)*hotDust*nebula
            *(0.12+u_warmth*0.18+silk*0.15);
          color+=u_gold*pow(max(hotDust*silk,0.0),2.15)*0.24*u_warmth;
          color+=mix(u_accent,u_light,0.56)*ribbons*filament*0.10;
          color+=mix(u_accent,u_light,0.30+u_warmth*0.18)
            *voiceCloud*(0.022+filament*0.055);
          color+=mix(u_accent,u_gold,0.34+u_warmth*0.24)
            *voiceThread*(0.15+hotDust*0.11);
          color+=mix(u_accent,u_light,0.52)*fineWisps*(0.12+hotDust*0.14);
          color+=mix(u_accent,u_gold,0.26+u_warmth*0.18)
            *cloudGrain*(0.055+filament*0.075);
          color+=u_accent*liquidBody*(0.032+filament*0.075);
          color+=mix(u_accent,u_light,0.38)*liquidRim*(0.055+hotDust*0.075);
          ...
          color+=mix(u_accent,u_gold,0.38)*g1*0.21*u_depth;
          color+=mix(u_accent,u_light,0.28)*g2*0.17*u_depth;
          ...
          color+=farColor*starsFar*0.18*u_density*u_depth;
          color+=midColor*starsMid*0.31*u_density*u_depth;
          color+=nearColor*starsNear*0.43*u_density*u_depth;
          ...
          float constellationBreath=0.96+0.04*sin(time*0.12+u_seed*6.283185);
          vec3 constellationColor=mix(u_light,u_gold,0.48+u_warmth*0.18);
          color+=constellationColor*constellationLines*0.085*u_depth*constellationBreath;
          ...
          color+=mix(u_accent,u_light,0.5)*touchField*(0.035+velocityStrength*0.055);
  ...
  setPerformanceBudget({ fps, scale, profile } = {}) {
    ...
    const nextScale = clamp(Number(scale) || this.qualityCeiling, 1.20, 1.95);
  ...
  resize({ force=false }={}) {
    ...
    const pixelBudget = constrained() || profile === 'protected'
      ? 620000
      : profile === 'cinematic'
        ? (mobile ? 1250000 : 2200000)
        : (mobile ? 1010000 : 1750000);
    ...
    const renderRatio = Math.max(mobile ? 1.20 : 0.82, Math.min(ratioCeiling, budgetRatio));
  ...
  drawCanvasConstellations(context, time) {
    ...
    for (const [groupIndex,definition] of definitions.entries()) {
      ...
      const breathing = 0.94+Math.sin(time*0.12+definition.phase)**2*0.06;
      ...
      points.forEach((point,index) => {
        const pulse = 0.97+Math.sin(time*(0.16+index*0.004)+index+definition.phase)*0.03;
        context.globalAlpha = breathing*(0.90+pulse*0.10);
        context.fillStyle = index % 3 === 0
          ? `rgb(${light[0]} ${light[1]} ${light[2]})`
          : `rgb(${color[0]} ${color[1]} ${color[2]})`;
        context.beginPath();
        context.arc(point.x,point.y,1.0+pulse*0.42,0,Math.PI*2);
        context.fill();
      });
  ...
  drawCanvas(timestamp) {
    ...
    for (const star of this.fallbackStars) {
      ...
      const pulse = star.breath ? 0.985 + Math.sin(time * 0.16 + star.phase) * 0.015 : 1;
  ...
  status() {
    return Object.freeze({
      ...
      staticUniverseImage:false,
      globalAcrossRoutes:true,
      oneUniverseCanvas:document.querySelectorAll('#divinaLivingUniverseV522 canvas').length === 1,
      targetFps:this.targetFps,
...
export function createLivingUniverseV522() {
  if (globalThis.divinaLivingUniverseV522?.status) return globalThis.divinaLivingUniverseV522;
  const former = globalThis.divinaLivingUniverseV521 || globalThis.divinaLivingUniverseV520 || globalThis.divinaLivingUniverseV519 || globalThis.divinaLivingUniverseV516;
  ...
  document.body?.classList.remove('db516-universe-active','db519-universe-active','db520-universe-active','db521-universe-active');
  const universe = new LivingUniverseCoreV522();
