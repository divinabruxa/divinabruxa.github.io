...
check(cacheStores.has('divina-bruxa-v554-shell'),'install-cache-v554');
check((await cacheStores.get('divina-bruxa-v554-shell').keys()).length>=123,'install-shell-atomico');
check(!cacheStores.has('divina-bruxa-v554-offline-core'),'install-sem-cache-opcional');
...
check(!cacheStores.has('divina-bruxa-v554-premium-static'),'premium-sem-cache-na-negacao');
...
online=true;
const shellCache=cacheStores.get('divina-bruxa-v554-shell');
await shellCache.delete(new Request(`${scope}security-privacy-core-v547.js`));
result=await message({type:'VERIFY_SHELL'});
check(result?.complete===false&&result?.missing?.includes('./security-privacy-core-v547.js'),'corrupcao-v554-detectada');
result=await message({type:'REPAIR_SHELL'});
check(result?.complete===true,'corrupcao-v554-reparada');

result=await message({type:'CLEAR_OPTIONAL_OFFLINE'});
check(result?.ok===true,'cache-opcional-limpo');
check(!cacheStores.has('divina-bruxa-v554-offline-core')&&!cacheStores.has('divina-bruxa-v554-premium-static'),'stores-opcionais-removidas');
check(result?.authorityDataRemoved===false,'nao-inventa-exclusao-privada');

const contentCache=await caches.open('divina-bruxa-v554-content');contentCache.failNextPut=true;
...
console.log(`V554 Service Worker runtime: ${passed}/${total} verificações aprovadas.`);
