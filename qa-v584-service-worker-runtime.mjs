/* DIVINA BRUXA 2.0 — FLUIDEZ SUPREMA · SERVICE WORKER RUNTIME V584 */
...
check(cacheStores.has('divina-bruxa-v584-shell'),'install:cache-v584');
const shellCache=cacheStores.get('divina-bruxa-v584-shell');
...
const oldCache=await caches.open('divina-bruxa-v583-shell');
...
check(!cacheStores.has('divina-bruxa-v583-shell'),'activate:remove-v583-cache');

let result=await message({type:'VERIFY_SHELL'});
check(result?.complete===true&&result?.atomic===true&&result?.version===584,'verify:complete-v584');
...
response=await fetchEvent({
  url:`${scope}index.html?v584=1`,method:'GET',mode:'navigate',destination:'document',
  cache:'default',headers:new Headers()
});
check(response?.status===200&&(await response.text()).includes('app-v208.js?v=584-menu-lendario'),'navigation:v584-shell-valid');
const contentCache=cacheStores.get('divina-bruxa-v584-content');
check(Boolean(await contentCache?.match(new Request(`${scope}index.html?v584=1`))),'navigation:valid-shell-cached');
...
console.log(JSON.stringify({
  release:'V584',base:'V583',state:failures.length?'FAIL':'PASS',
